...
errno_t mk_zer_cli()
{
    if (CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_INT64) +
        ...
        0)
    {
        mk_zer(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
...
{
    RegisterCLIcommand("mkzer", __FILE__, mk_zer_cli, "create Zernike polynomial",
                       "<output image> <size> <zern index> <rpix>", "mkzer z43 512 43 100.0",
                       "mk_zer(const char *ID_name, long SIZE, long zer_nb, float rpix)");

    RegisterCLIcommand("rmcpiston", __FILE__, ZERNIKEPOLYN_rmPiston_cli,
                       "remove piston term from WF cube", "<WF cube> <aperture mask>",
                       "rmcpiston wfc mask",
...
// continue Zernike exp. beyond nominal radius, using the same polynomial
// expression
