...
imageID IMG_REDUCE_cubesimplestat(const char *IDin_name)
{
    ...
    for (ii = 0; ii < xysize; ii++)
    {
        ...
        dcimg[IDrms].array.F[ii] =
            sqrt(dcimg[IDrms].array.F[ii] - dcimg[IDave].array.F[ii] * dcimg[IDave].array.F[ii]);
    }
                    ...
                    for (i = 0; i < 3; i++)
                    {
                        for (j = 0; j < 3; j++)
                        {
                            for (kk = 0; kk < zsize; kk++)
                            {
                                pix[kk * 9 + j * 3 + i] =
                                    dcimg[IDin]
                                        .array.F[kk * xysize + (jj - 1 + j) * xsize + (ii - 1 + i)];
                            }
                            bpix[i][j] =
                                dcimg[IDbadpix].array.F[(jj - 1 + j) * xsize + (ii - 1 + i)];
                            sum_bpix += bpix[i][j];
                            for (kk = 0; kk < zsize; kk++)
                            {
        ...
        for (jj = 0; jj < ysize; jj++)
        {
            if (dcimg[IDbadpix].array.F[jj * xsize + ii] > 0.5f)
            {
                    ...
                    for (ii1 = ii - distmax; ii1 < ii + distmax + 1; ii1++)
                    {
                        for (jj1 = jj - distmax; jj1 < jj + distmax + 1; jj1++)
                        {
                            if ((ii1 > -1) && (ii1 < xsize) && (jj1 > -1) && (jj1 < ysize) &&
                                (dcimg[IDbadpix].array.F[jj1 * xsize + ii1] < 0.5f))
                            {
                                if ((ii1 != ii) || (jj1 != jj))
                                {
                                    ...
                                    nearbypix_array_coeff[k] =
                                        pow(1.0 / nearbypix_array_dist2[k], 2.0);
                ...
                for (k = 0; k < NBnearbypix; k++)
                {
                    ...
                    if (NBop > xysize - 1)
                    {
                        printf("ERROR: TOO MANY BAD PIXELS .... sorry... you need "
                               "a better detector.\n");
                        exit(0);
                    }
                }
                                    ...
                                    int         streamMode)
{
    ...
    if (IDout == -1)
    {
        ...
        if (streamMode == 1)
        {
            ...
            {
                imgout_tmp.mdt->size[i] = sizearray[i];
            }
    ...
    while (OKloop == 1)
    {
        ...
        memcpy(dcimg[IDout].array.F, dcimg[ID].array.F, sizeof(float) * xysize * zsize);
        ...
        for (kk = 0; kk < zsize; kk++)
        {
            if (IDdark != -1)
            {
                for (ii = 0; ii < xysize; ii++)
                {
                    dcimg[IDout].array.F[kk * xysize + ii] -= dcimg[IDdark].array.F[ii];
                }
            }

            for (k = 0; k < badpixclean_NBbadpix; k++)
            {
                dcimg[IDout].array.F[kk * xysize + badpixclean_indexlist[k]] = 0.0f;
            }
            ...
            for (k = 0; k < badpixclean_NBop; k++)
            {
                ...
                //   fflush(stdout);
                dcimg[IDout].array.F[kk * xysize + badpixclean_array_indexout[k]] +=
                    badpixclean_array_coeff[k] *
                    dcimg[IDout].array.F[kk * xysize + badpixclean_array_indexin[k]];
                //  printf("\n");
                //  fflush(stdout);
            }
        }
    ...
    if (IDout == -1)
    {
        ...
        else
        {
            ...
            imsizearray[1] = ysize;
            {
                ...
                imgout_tmp.mdt->size[0]  = xsize;
                imgout_tmp.mdt->size[1]  = ysize;
                imgout_tmp.mdt->datatype = _DATATYPE_FLOAT;
                imgout_tmp.mdt->shared   = 1;
    ...
    while (loopOK == 1)
    {
        ...
        for (ii = 0; ii < xcentsize; ii++)
        {
            for (jj = 0; jj < ycentsize; jj++)
            {
                ...
                dcimg[IDcent].array.F[jj * xcentsize + ii] = dcimg[IDin].array.F[jj0 * xsize + ii0];
            }
        }
        ...
        memcpy(dcimg[IDtin].array.F, dcimg[IDin].array.F, sizeof(float) * xsize * ysize);
        ...
        else
        {
            ...
            for (ii = 0; ii < xsize; ii++)
            {
                for (jj = 0; jj < ysize; jj++)
                {
                    dcimg[IDout].array.F[jj * xsize + ii] = dcimg[IDtout].array.F[jj * xsize + ii];
                }
            }
            dcimg[IDout].md[0].write = 0;
            dcimg[IDout].md[0].cnt0++;
...
errno_t IMG_REDUCE_cubeprocess(const char *IDin_name)
{
    ...
    if ((IDdark = image_ID("calib_dark", dcimg, dcnimg)) != -1)
    {
        ...
        for (kk = 0; kk < zsize; kk++)
        {
            for (ii = 0; ii < xysize; ii++)
            {
                dcimg[IDin].array.F[kk * xysize + ii] -= dcimg[IDdark].array.F[kk1 * xysize + ii];
            }
            kk1++;
            if (kk1 == zsized)
            {
    ...
    for (kk = 0; kk < zsize; kk++)
    {
        ...
        fft_image_translate("translin", "translout", xcent[kk] - 0.5 * xsize1,
                            ycent[kk] - 0.5 * ysize1);
        IDt2 = image_ID("translout", dcimg, dcnimg);

        for (ii = 0; ii < xysize1; ii++)
        {
            dcimg[ID1].array.F[kk * xysize1 + ii] = dcimg[IDt2].array.F[ii];
        }
    ...
    for (ii = 0; ii < xysize1; ii++)
    {
        dcimg[ID1].array.F[ii] = dcimg[ID].array.F[kk1min * xysize1 + ii];
        dcimg[ID2].array.F[ii] = dcimg[ID].array.F[kk2min * xysize1 + ii];
        dcimg[IDdiff].array.F[ii] =
            dcimg[ID].array.F[kk1min * xysize1 + ii] - dcimg[ID].array.F[kk2min * xysize1 + ii];
    }
