...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 3) + CLI_checkarg(4, 2) +
            CLI_checkarg(5, 2) + CLI_checkarg(6, 2) + CLI_checkarg(7, 2) + CLI_checkarg(8, 2) +
            CLI_checkarg(9, 2) ==
        0)
    {
        IMG_REDUCE_centernormim(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand("rmbadpixfast", __FILE__, IMG_REDUCE_cleanbadpix_fast_cli,
                       "remove bad pixels (fast algo)", "<image> <badpixmap> <output>",
    ...
    RegisterCLIcommand("cubesimplestat", __FILE__, IMG_REDUCE_cubesimplestat_cli,
                       "simple data cube stats", "<image>", "cubesimplestat",
    ...
    RegisterCLIcommand("imgcubeprocess", __FILE__, IMG_REDUCE_cubeprocess_cli, "data cube process",
                       "<image>", "imgcubeprocess",
                ...
                if (data.image[IDbadpix].array.F[jj * xsize + ii] > 0.5)
                {
                    ...
                    for (i = 0; i < 3; i++)
                    {
                        for (j = 0; j < 3; j++)
                        {
                            ...
                            bpix[i][j] =
                                data.image[IDbadpix].array.F[(jj - 1 + j) * xsize + (ii - 1 + i)];
                            sum_bpix += bpix[i][j];
                            for (kk = 0; kk < zsize; kk++)
                            {
        ...
        for (kk = 0; kk < zsize; kk++)
        {
            ...
            for (k = 0; k < badpixclean_NBop; k++)
            {
                ...
                data.image[IDout].array.F[kk * xysize + badpixclean_array_indexout[k]] +=
                    ...
                    data.image[IDout].array.F[kk * xysize + badpixclean_array_indexin[k]];
                //  printf("\n");
                //  fflush(stdout);
            }
        }
...
errno_t IMG_REDUCE_cubeprocess(const char *IDin_name)
{
    ...
    for (ii = 0; ii < xysize1; ii++)
    {
        ...
        data.image[IDdiff].array.F[ii] = data.image[ID].array.F[kk1min * xysize1 + ii] -
