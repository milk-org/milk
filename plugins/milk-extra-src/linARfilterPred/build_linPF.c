...
static CLICMDARGDEF farg[] = { { // input telemetry
                               ...
                               { // temporal order of filter: number of time steps in state
                                 CLIARG_UINT32, ".PForder", "predictive filter order", "10",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &PForder, &fpi_PForder },
                               { // latency: how far ahead to predict
                                 CLIARG_FLOAT32, ".PFlatency", "time latency [frame]", "2.7",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &PFlatency, &fpi_PFlatency },
                               { // SVD limit
                                 CLIARG_FLOAT64, ".SVDeps", "SVD cutoff", "0.001",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &SVDeps, &fpi_SVDeps },
                               { // Regularization
                                 CLIARG_FLOAT64, ".reglambda", "regularization coefficient",
                                 "0.001", CLIARG_HIDDEN_DEFAULT, (void **) &reglambda,
                                 &fpi_reglambda },
                               { CLIARG_STR, ".outPFname", "output filter", "outPF",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outPFname, NULL },
                               { CLIARG_FLOAT32, ".loopgain", "loop gain", "0.2",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &loopgain, &fpi_loopgain },
                               { CLIARG_ONOFF, ".out3Dfilt", "write output 3D filter", "0",
...
static errno_t compute_function()
{
    ...
    imageID IDoutPF2D;
    {
        ...
        create_image_ID(outPFname, 2, imsizearray, _DATATYPE_FLOAT, 1, 1, 0, &IDoutPF2D);
            ...
            for (long dt = 0; dt < PForderval; dt++)
            {
                ...
                for (long PFpix = 0; PFpix < NBpixout; PFpix++)
                {
                    ...
                    for (long m = 0; m < NBmvec; m++)
                    {
                        ...
                        val += matCtrans[ind2t] * data.image[IDfm].array.F[PFpix * NBmvec + m];
                    }
