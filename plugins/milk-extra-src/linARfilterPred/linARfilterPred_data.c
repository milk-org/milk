                                    ...
                                    const char *IDoutname)
{
    ...
    while (fOK == 1)
    {
        snprintf(fname, sizeof(fname), "seq%03ld.dat", NBfiles);
    ...
    {
        snprintf(fname, sizeof(fname), "seq%03ld.dat", fcnt);
    ...
    for (fr = 0; fr < NBfr; fr++)
    {
        snprintf(imoutname, sizeof(imoutname), "%s_%03ld", IDoutname, fr);
                                    ...
                                    const char *IDout_name)
{
...

    {
        ...
        for (uint8_t a = 0; a < naxis; a++)
        {
            imgout_tmp.mdt->size[a] = sizearray[a];
        }
            ...
            for (uint32_t ii = 0; ii < mmax; ii++)
            {
                if (dcimg[IDblknb].array.UI16[ii] == blkNB)
                {
                    ...
                    dcimg[IDout].array.F[cnt] =
                        dcimg[IDin].array.F[kk * xsize * ysize + jj * ysize + ii];
                    cnt++;
                }
            }
        }
...
imageID linARfilterPred_repeat_shift_X(const char *IDin_name, long NBstep, const char *IDout_name)
{
    ...
    imsizeout[1] = ysizeout;
    {
        ...
        imgout_tmp.mdt->size[0]  = imsizeout[0];
        imgout_tmp.mdt->size[1]  = imsizeout[1];
        imgout_tmp.mdt->datatype = _DATATYPE_FLOAT;
    ...
    for (step = 0; step < NBstep; step++)
    {
        for (uint32_t ii = 0; ii < xsize; ii++)
        {
            ...
            {
                dcimg[IDout].array.F[jjout * xsizeout + step * xsize + ii] =
                    dcimg[IDin].array.F[(jjout + NBstep - step - 1) * xsize + ii];
            }
        }
    }
