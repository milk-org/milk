...
{
    if (CLI_checkarg(1, 1) + CLI_checkarg(2, 1) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) +
            CLI_checkarg(5, 5) ==
        0)
    {
...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 1) + CLI_checkarg(4, 1) +
            CLI_checkarg(5, 1) + CLI_checkarg(6, 3) + CLI_checkarg(7, 2) + CLI_checkarg(8, 1) +
            CLI_checkarg(9, 2) ==
        0)
    {
        LINARFILTERPRED_Build_LinPredictor(
            ...
            data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.string, 1,
...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 4) +
            CLI_checkarg(5, 2) + CLI_checkarg(6, 5) + CLI_checkarg(7, 2) + CLI_checkarg(8, 2) +
            CLI_checkarg(9, 2) + CLI_checkarg(10, 2) + CLI_checkarg(11, 1) + CLI_checkarg(12, 2) ==
        0)
    {
        LINARFILTERPRED_PF_RealTimeApply(
            ...
            data.cmdargtoken[11].val.numf, data.cmdargtoken[12].val.numl);
...
{
    RegisterCLIcommand("pfloadascii", __FILE__, LINARFILTERPRED_LoadASCIIfiles_cli,
                       "load ascii files to PF input", "<tstart> <dt> <NBpt> <NBfr> <output>",
    ...
    RegisterCLIcommand("mselblock", __FILE__, LINARFILTERPRED_SelectBlock_cli,
    ...
    RegisterCLIcommand("imrepshiftx", __FILE__, linARfilterPred_repeat_shift_X_cli,
    ...
    RegisterCLIcommand("mkARpfilt", __FILE__, LINARFILTERPRED_Build_LinPredictor_cli,
    ...
    */
    ...
    RegisterCLIcommand("linARPFMupdate", __FILE__, LINARFILTERPRED_PF_updatePFmatrix_cli,
                                           ...
                                           int                         testmode)
{
    ...
    case 2:
        ...
        create_2Dimage_ID("PFin_cp", data.image[IDin].md[0].size[0], data.image[IDin].md[0].size[1],
                          &IDincp);
    ...
    case 3:
        ...
        xsize = data.image[IDin].md[0].size[0];
        ysize = data.image[IDin].md[0].size[1];
        create_3Dimage_ID("PFin_copy", data.image[IDin].md[0].size[0],
                          data.image[IDin].md[0].size[1], data.image[IDin].md[0].size[2], &IDincp);
    ...
    for (iter = 0; iter < NBiter; iter++)
    {
        ...
        for (PFpix = 0; PFpix < NBpixout; PFpix++)
        {
            for (m = 0; m < NBmvec; m++)
            {
                ...
                data.image[IDfm].array.F[PFpix * NBmvec + m] =
                    (1.0 - alpha) *
                        data.image[IDincp].array.F[(k0) *xysize + outpixarray_xy[PFpix]] +
...
#ifdef HAVE_MAGMA
        printf("Using magma ...\n");
        LINALGEBRA_magma_compute_SVDpseudoInverse("PFmatD", "PFmatC", SVDeps_run, NB_SVD_Modes,
                                                  "PF_VTmat", LOOPmode, testmode, 64,
                                                  0, // GPU device
                                                  NULL);
#else
        ...
        else
        {
            if (iter == 0) // create 2D and 3D filters as shared memory
            {
                ...
                create_image_ID(IDoutPF_name_raw, 2, imsizearray, _DATATYPE_FLOAT, 1, 1, 0,
                                &IDoutPF2Draw);
                free(imsizearray);
                ...
                for (pix = 0; pix < NBpixin; pix++)
                {
                    for (dt = 0; dt < PForder; dt++)
                    {
                        ...
                        data.image[IDoutPF2Dn]
                            .array.F[PFpix * (PForder * NBpixin) + dt * NBpixin + pix] = val;
                    }
                }
            }
        }
                ...
                for (PFpix = 0; PFpix < NBpixout; PFpix++)
                {
                    for (dt = 0; dt < PForder; dt++)
                    {
                        val = data.image[IDoutPF2D]
                                  .array.F[PFpix * (PForder * NBpixin) + dt * NBpixin + pix];
                        data.image[IDoutPF3D]
                            .array.F[NBpixout * NBpixin * dt + NBpixin * PFpix + pix] = val;
                    }
                }
            }
    ...
    while (1)
    {
        ...
        for (uint32_t jj = 0; jj < NBpix_out; jj++)
        {
            for (uint32_t ii = 0; ii < NBpix_in; ii++)
            {
                ...
                {
                    outarray[jj] +=
                        data.image[IDfilt].array.F[kk * NBpix_in * NBpix_out + jj * NBpix_in + ii] *
    ...
    while (iter != NBiter)
    {
        ...
        for (mode = 0; mode < NBmodeIN; mode++)
        {
            data.image[IDINbuff].array.F[mode] =
                data.image[IDmodevalIN].array.F[IndexOffset + inmaskindex[mode]];
        }
...
float LINARFILTERPRED_ScanGain(char *IDin_name, float multfact, float framelag)
{
    ...
    printf("\noptimal block gain = %f     residual = %.6f -> %.6f\n\n", optgainblock,
           sqrt(residualblock0), sqrt(residualblock));
