...
static char fconv_in[FUNCTION_PARAMETER_STRMAXLEN]  = "imin";
static char fconv_ke[FUNCTION_PARAMETER_STRMAXLEN]  = "kernim";
static char fconv_out[FUNCTION_PARAMETER_STRMAXLEN] = "imout";

static FPS_APP_INFO FPS_app_info = { .fps_name    = "fconv",
                                     .cmdkey      = "fconv",
                                     .description = "Fourier-based convolution",
                                     .description_long =
                                         "Convolve two images using FFT-based multiplication in "
                                         "Fourier space. Efficient for large kernel sizes." };

#define FPS_PARAMS(X)                                                                   \
    X(".in_name", fconv_in, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image")  \
                  ...
                  const char *__restrict name_out)
{
    ...
    if ((naxes[0] != dcimg[IDke].md[0].size[0]) || (naxes[1] != dcimg[IDke].md[0].size[1]))
    {
        fprintf(stderr, "ERROR in function fconvolve: image and kernel have different "
                        "sizes\n");
        exit(0);
    }
                       ...
                       const char *__restrict name_out)
{
    ...
    for (ii = 0; ii < naxes[0]; ii++)
    {
        ...
        {
            dcimg[ID1].array.F[(jj + paddsize) * naxespadd[0] + (ii + paddsize)] =
                dcimg[IDin].array.F[jj * naxes[0] + ii];
            dcimg[ID2].array.F[(jj + paddsize) * naxespadd[0] + (ii + paddsize)] =
                dcimg[IDke].array.F[jj * naxes[0] + ii];
            dcimg[ID3].array.F[(jj + paddsize) * naxespadd[0] + (ii + paddsize)] = 1.0;
        }
    }
                ...
                for (jj = 0; jj < blocksize; jj++)
                {
                    if ((ii0 + ii < xsize) && (jj0 + jj < ysize))
                    {
                        dcimg[IDtmp].array.F[jj * blocksize + ii] =
                            dcimg[IDin].array.F[(jj0 + jj) * xsize + (ii0 + ii)];
                    }
                    else
                    {
            ...
            for (ii = 0; ii < blocksize; ii++)
            {
                for (jj = 0; jj < blocksize; jj++)
                {
                    if ((ii0 + ii < xsize) && (jj0 + jj < ysize))
                    {
                        ...
                        dcimg[IDout].array.F[(jj0 + jj) * xsize + (ii0 + ii)] +=
                            gain * dcimg[IDtmpout].array.F[jj * blocksize + ii];
                        dcimg[IDcnt].array.F[(jj0 + jj) * xsize + (ii0 + ii)] += gain * 1.0;
                    }
                }
            }
        }
