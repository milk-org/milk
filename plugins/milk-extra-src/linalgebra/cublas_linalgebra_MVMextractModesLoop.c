...
errno_t __attribute__((hot)) LINALGEBRA_MVMextractModesLoop_RUN();

int __attribute__((hot)) LINALGEBRA_MVMextractModesLoop(
...
static errno_t LINALGEBRA_MVMextractModesLoop_cli()
{
    ...
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR) + CLI_checkarg(3, CLIARG_IMG) +
            CLI_checkarg(4, CLIARG_STR) + CLI_checkarg(5, CLIARG_STR) +
            CLI_checkarg(6, CLIARG_STR) + CLI_checkarg(7, CLIARG_INT64) +
            CLI_checkarg(8, CLIARG_INT64) + CLI_checkarg(9, CLIARG_INT64) +
            CLI_checkarg(10, CLIARG_INT64) + CLI_checkarg(11, CLIARG_INT64) +
            CLI_checkarg(12, CLIARG_INT64) + CLI_checkarg(13, CLIARG_INT64) +
            CLI_checkarg(14, CLIARG_INT64) ==
        0)
    {
        LINALGEBRA_MVMextractModesLoop(
            ...
            data.cmdargtoken[7].val.numl, data.cmdargtoken[8].val.numl,
            data.cmdargtoken[9].val.numl, data.cmdargtoken[10].val.numl,
            data.cmdargtoken[11].val.numl, data.cmdargtoken[12].val.numl,
            data.cmdargtoken[13].val.numl, data.cmdargtoken[14].val.numl);
...
errno_t linalgebra_MVMextractModesLoop_addCLIcmd()
{
    RegisterCLIcommand("cudaextrmodes", __FILE__, LINALGEBRA_MVMextractModesLoop_cli,
                       ...
                       "0",
                       "int LINALGEBRA_MVMextractModesLoop(const char *in_stream, const char "
...
errno_t __attribute__((hot)) LINALGEBRA_MVMextractModesLoop_RUN()
{
    ...
    strncpy(in_stream, functionparameter_GetParamPtr_STRING(&fps, ".sname_in"),
            FUNCTION_PARAMETER_STRMAXLEN);

    char IDmodes_name[STRINGMAXLEN_IMGNAME];
    strncpy(IDmodes_name, functionparameter_GetParamPtr_STRING(&fps, ".sname_modes"),
            FUNCTION_PARAMETER_STRMAXLEN);

    char intot_stream[STRINGMAXLEN_IMGNAME];
    strncpy(intot_stream, functionparameter_GetParamPtr_STRING(&fps, ".option.sname_intot"),
            FUNCTION_PARAMETER_STRMAXLEN);

    char IDrefin_name[STRINGMAXLEN_IMGNAME];
    strncpy(IDrefin_name, functionparameter_GetParamPtr_STRING(&fps, ".option.sname_refin"),
            FUNCTION_PARAMETER_STRMAXLEN);

    char IDrefout_name[STRINGMAXLEN_IMGNAME];
    strncpy(IDrefout_name, functionparameter_GetParamPtr_STRING(&fps, ".option.sname_refout"),
            FUNCTION_PARAMETER_STRMAXLEN);

    char IDmodes_val_name[STRINGMAXLEN_IMGNAME];
    strncpy(IDmodes_val_name, functionparameter_GetParamPtr_STRING(&fps, ".sname_outmodesval"),
            FUNCTION_PARAMETER_STRMAXLEN);
    ...
    printf("IDrefin_name     : %16s  [optional] input reference  - to be "
           "subtracted\n",
           IDrefin_name);
    ...
    printf("axmode           : %16d  0 for normal mode extraction, 1 for "
           "expansion\n",
           axmode);
    printf("twait            : %16ld  if >0, insert time wait [us] at each "
           "iteration\n",
           *twait);
    printf("semwarn          : %16d  1 if warning when input stream semaphore "
           ">1\n",
           semwarn);
    printf("\n");
    ...
    {
        int slen =
            snprintf(pinfoname, STRINGMAXLEN_PROCESSINFO_NAME, "cudaMVMextract-%s", in_stream);
        if (slen < 1)
        {
    ...
    {
        int slen = snprintf(pinfodescr, STRINGMAXLEN_PROCESSINFO_DESCRIPTION, "%s->%s", in_stream,
                            IDmodes_val_name);
        if (slen < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
    ...
    if (IDin == -1)
    {
        ...
        fprintf(stderr,
                ...
                errtm->tm_hour, errtm->tm_min, errtm->tm_sec, errtime.tv_nsec, __FILE__,
                __FUNCTION__, __LINE__, in_stream);
        return 1;
    }
    ...
    if (IDref == -1)
    {
        create_2Dimage_ID("_tmprefin", data.image[IDin].md[0].size[0],
                          data.image[IDin].md[0].size[1], &IDref);
    ...
    else
    {
        ...
        printf("creating _tmpmodes  %ld %ld %ld\n", (long) data.image[IDin].md[0].size[0],
               (long) data.image[IDin].md[0].size[1], NBmodes);
        fflush(stdout);

        create_3Dimage_ID("_tmpmodes", data.image[IDin].md[0].size[0],
                          data.image[IDin].md[0].size[1], NBmodes, &IDmodes);
        ...
        for (ii = 0; ii < data.image[IDin].md[0].size[0]; ii++)
        {
            for (jj = 0; jj < data.image[IDin].md[0].size[1]; jj++)
            {
                ...
                {
                    data.image[IDmodes].array.F[kk * data.image[IDin].md[0].size[0] *
                                                    data.image[IDin].md[0].size[1] +
                                                jj * data.image[IDin].md[0].size[0] + ii] =
                        data.image[ID]
                            .array.F[NBmodes * (jj * data.image[IDin].md[0].size[0] + ii) + kk];
                }
            }
        }
    ...
    if (MODEVALCOMPUTE == 1)
    {
        ...
        cudaStat = cudaMemcpy(d_modes, data.image[IDmodes].array.F, sizeof(float) * m * NBmodes,
    ...
    if (PROCESS == 1)
    {
        ...
        if (imOK == 0)
        {
            create_image_ID(process_ave_name, 2, sizearraytmp, _DATATYPE_FLOAT, 1, 0, 0,
                            &IDprocave);
        }
        free(sizearraytmp);
        ...
        if (imOK == 0)
        {
            create_image_ID(process_rms_name, 2, sizearraytmp, _DATATYPE_FLOAT, 1, 0, 0,
                            &IDprocrms);
        }
        free(sizearraytmp);
    }
    ...
    if (MODEVALCOMPUTE == 1)
    {
        int slen = snprintf(pinfomsg, STRINGMAXLEN_PROCESSINFO_STATUSMSG,
                            "MVM %s %s -> %s TRACE=%d PROC=%d", IDmodes_name, in_stream,
                            IDmodes_val_name, TRACEMODE, PROCESS);
        if (slen < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
            abort(); // can't handle this error any other way
    ...
    {
        int slen =
            snprintf(pinfomsg, STRINGMAXLEN_PROCESSINFO_STATUSMSG,
                     "passthrough %s TRACE=%d PROC=%d", IDmodes_val_name, TRACEMODE, PROCESS);
        if (slen < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
            abort(); // can't handle this error any other way
    ...
    processinfo_waitoninputstream_init(processinfo, IDin, PROCESSINFO_TRIGGERMODE_SEMAPHORE, 5);
    ...
    while (loopOK == 1)
    {
        ...
        if (MODEVALCOMPUTE == 1)
        {
            ...
            if (doComputation == 1)
            {
                ...
                if (initref == 0)
                {
                    cudaStat = cudaMemcpy(d_in, data.image[IDref].array.F, sizeof(float) * m,
                                          cudaMemcpyHostToDevice);
                }
                else
                {
                    cudaStat = cudaMemcpy(d_in, data.image[IDin].array.F, sizeof(float) * m,
                                          cudaMemcpyHostToDevice);
                }
                ...
                {
                    beta     = -1.0;
                    cudaStat = cudaMemcpy(d_modeval, modevalarrayref, sizeof(float) * NBmodes,
                                          cudaMemcpyHostToDevice);
                }
                ...
                // compute
                cublas_status = cublasSgemv(cublasH, CUBLAS_OP_T, m, NBmodes, &alpha, d_modes, m,
                                            d_in, 1, &beta, d_modeval, 1);
                if (cublas_status != CUBLAS_STATUS_SUCCESS)
                {
            ...
            for (step = 0; step < NBaveSTEP; step++)
            {
                for (k = 0; k < NBmodes; k++)
                {
                    data.image[IDprocave].array.F[NBmodes * step + k] =
                        (1.0 - stepcoeff) * data.image[IDprocave].array.F[NBmodes * step + k] +
            ...
            for (step = 0; step < NBaveSTEP; step++)
            {
                for (k = 0; k < NBmodes; k++)
                {
                    ...
                    data.image[IDprocrms].array.F[NBmodes * step + k] =
                        (1.0 - stepcoeff) * data.image[IDprocrms].array.F[NBmodes * step + k] +
                        stepcoeff * tmpv;
                }
                stepcoeff *= stepcoeff0;
            }
...
)
{
    ...
    LINALGEBRA_MVMextractModesLoop_RUN();
