...
static CLICMDARGDEF farg[] = {
    { CLIARG_INT32, ".GPUindex", "GPU index, 99 for CPU", "0", CLIARG_VISIBLE_DEFAULT,
      (void **) &GPUindex, &fpi_GPUindex },
    { CLIARG_STREAM, ".insname", "input stream name", "inV", CLIARG_VISIBLE_DEFAULT,
      (void **) &insname, &fpi_insname },
    { CLIARG_STREAM, ".inmasksname", "nput mask stream name", "inV", CLIARG_VISIBLE_DEFAULT,
      (void **) &inmasksname, &fpi_inmasksname },
    { CLIARG_STREAM, ".immodes", "modes stream name", "mat", CLIARG_VISIBLE_DEFAULT,
      (void **) &immodes, &fpi_immodes },
    { CLIARG_STREAM, ".outcoeff", "output coefficients", "outV", CLIARG_VISIBLE_DEFAULT,
      (void **) &outcoeff, &fpi_outcoeff },
    { CLIARG_ONOFF, ".outinit", "output init mode", "0", CLIARG_HIDDEN_DEFAULT, (void **) &outinit,
      &fpi_outinit },
    { CLIARG_UINT32, ".option.axmode", "0 for normal mode extraction, 1 for expansion", "0",
      CLIARG_HIDDEN_DEFAULT, (void **) &axmode, &fpi_axmode },
    { CLIARG_ONOFF, ".option.PROCESS", "processing flag", "0", CLIARG_HIDDEN_DEFAULT,
      (void **) &PROCESS, &fpi_PROCESS },
    { CLIARG_ONOFF, ".option.TRACEMODE", "writing trace", "0", CLIARG_HIDDEN_DEFAULT,
      (void **) &TRACEMODE, &fpi_TRACEMODE },
    { CLIARG_ONOFF, ".option.MODENORM", "input modes normalization", "0", CLIARG_HIDDEN_DEFAULT,
      (void **) &MODENORM, &fpi_MODENORM },
    ...
    { CLIARG_STREAM, ".option.sname_refin", "optional input reference to be subtracted stream",
      "null", CLIARG_VISIBLE_DEFAULT, (void **) &inrefsname, &fpi_inrefsname },
    { CLIARG_STREAM, ".option.sname_refout", "optional output reference to be subtracted stream",
      "null", CLIARG_VISIBLE_DEFAULT, (void **) &outrefsname, &fpi_outrefsname },
    { CLIARG_UINT64, ".option.twait", "insert time wait [us] at each iteration", "0",
      CLIARG_HIDDEN_DEFAULT, (void **) &twait, &fpi_twait },
    { CLIARG_UINT32, ".option.mmax", "partial computation: max m index value", "100000",
      CLIARG_HIDDEN_DEFAULT, (void **) &mmax, &fpi_mmax },
    { CLIARG_UINT32, ".option.nmax", "partial computation: max n index value", "100000",
      CLIARG_HIDDEN_DEFAULT, (void **) &nmax, &fpi_nmax }
};
...
static CLICMDDATA CLIcmddata = { "MVMmextrmodes", "extract modes by MVM", CLICMD_FIELDS_DEFAULTS };
...
static errno_t compute_function()
{
  ...
  */
    ...
    if (imginref.ID == -1)
    {
        create_2Dimage_ID("_tmprefin", imgin.md->size[0], imgin.md->size[1], &IDinref);
    ...
    else
    {
        ...
        for (uint32_t ii = 0; ii < imgin.md->size[0]; ii++)
        {
            for (uint32_t jj = 0; jj < imgin.md->size[1]; jj++)
            {
                ...
                {
                    data.image[IDmodes].array.F[kk * imgin.md->size[0] * imgin.md->size[1] +
                ...
                for (uint32_t rr = 0; rr < nrows; ++rr)
                {
                    for (uint32_t cc = 0; cc < mask_npix; ++cc)
                    {
                        modesmat[rr * mask_npix + cc] =
                            data.image[IDmodes].array.F[rr * ncols + mask_idx[cc]];
                    }
                }
            }
            else
