...
static errno_t LINALGEBRA_Coeff2Map_Loop_cli()
{
    ...
    {
        LINALGEBRA_Coeff2Map_Loop(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand("cudacoeff2map", __FILE__, LINALGEBRA_Coeff2Map_Loop_cli,
    ...
    RegisterCLIcommand("cudacoeffo2map", __FILE__, LINALGEBRA_Coeff2Map_offset_Loop_cli,
                                  ...
                                  const char *IDoffset_name)
{
    ...
    fflush(stdout);
    list_image_ID();
    cudaStat = cudaMemcpy(d_modes, data.image[IDmodes].array.F, sizeof(float) * mdim * NBmodes,
    ...
    while (loopOK == 1)
    {
        ...
        if (semr == 0)
        {
            ...
            cudaStat = cudaMemcpy(d_coeff, data.image[IDcoeff].array.F, sizeof(float) * NBmodes,
                                  cudaMemcpyHostToDevice);
            if (cudaStat != cudaSuccess)
            {
                printf("cudaMemcpy returned error code %d, line(%d)\n", cudaStat, __LINE__);
            ...
            if (offsetmode == 1)
            {
                cudaStat = cudaMemcpy(d_outmap, data.image[IDoffset].array.F, sizeof(float) * mdim,
                                      cudaMemcpyHostToDevice);
                if (cudaStat != cudaSuccess)
                {
                    printf("cudaMemcpy returned error code %d, line(%d)\n", cudaStat, __LINE__);
            ...
            cudaStat = cudaMemcpy(data.image[IDoutmap].array.F, d_outmap, sizeof(float) * mdim,
