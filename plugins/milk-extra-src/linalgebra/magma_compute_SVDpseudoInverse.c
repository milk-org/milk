...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 3) + CLI_checkarg(3, 1) + CLI_checkarg(4, 2) +
            CLI_checkarg(5, 3) + CLI_checkarg(6, 2) + CLI_checkarg(7, 1) + CLI_checkarg(8, 1) ==
        0)
    {
        LINALGEBRA_magma_compute_SVDpseudoInverse(
            ...
            data.cmdargtoken[5].val.string, 0, 0, 64, 0, NULL);
...
{
    RegisterCLIcommand("linalgebrapsinv", __FILE__, LINALGEBRA_magma_compute_SVDpseudoInverse_cli,
                                                  ...
                                                  imageID    *outID)
{
    ...
    // times in sec
    double t01d, t12d, t23d, t34d, t45d, t56d, t67d, t78d, t89d, t910d, t1011d, t1112d, t1213d,
        t013d;

    long MaxNBmodes1, mode;

    // TESTING FLAGS
    int VERBOSE_LINALGEBRA_magma_compute_SVDpseudoInverse = 1;
   ...
   */
...

    {
        ...
        if (MAGMAfloat == 1)
        {
            magma_ssyrk(MagmaLower, MagmaTrans, N, M, 1.0, magmaf_d_A, M, 0.0, magmaf_d_AtA, N,
                        magmaqueue);
        ...
        {
            magma_dgemm(MagmaTrans, MagmaNoTrans, N, N, M, 1.0, magma_d_A, M, magma_d_A, M, 0.0,
                        magma_d_AtA, N, magmaqueue);
        }
            ...
            if (MAGMAfloat == 1)
            {
                ...
                if (magmaXmode == 1)
                {
                    magma_ssyevdx_gpu(MagmaVec, MagmaRangeI, MagmaLower, N, NULL, N, 0.0, 1.0,
                                      N - MaxNBmodes, N, NULL, NULL, NULL, N, auxf_work, -1,
        ...
        if (MAGMAfloat == 1)
        {
            ...
            if (magmaXmode == 1)
            {
                magma_ssyevdx_gpu(MagmaVec, MagmaRangeI, MagmaLower, N, magmaf_d_AtA, N, 0.0, 1.0,
                                  N - MaxNBmodes, N, &mout, magmaf_w1, magmaf_h_R, N, magmaf_h_work,
                                  magma_lwork, magma_iwork, magma_liwork, &info);
            }
            else
            {
                magma_ssyevd_gpu(MagmaVec, MagmaLower, N, magmaf_d_AtA, N, magmaf_w1, magmaf_h_R, N,
                                 magmaf_h_work, magma_lwork, magma_iwork, magma_liwork, &info);
            }
        }
        else
        {
        ...
        if (VERBOSE_LINALGEBRA_magma_compute_SVDpseudoInverse == 1)
        {
            printf("Keeping %ld modes  (SVDeps = %g -> %g, MaxNBmodes = %ld -> "
                   "%ld)\n",
        ...
        if (MAGMAfloat == 1)
        {
            magma_sgemm(MagmaTrans, MagmaTrans, N, N, N, 1.0, magmaf_d_VT1, N, magmaf_d_AtA, N, 0.0,
                        magmaf_d_M2, N, magmaqueue);
        }
        else
        {
            magma_dgemm(MagmaTrans, MagmaTrans, N, N, N, 1.0, magma_d_VT1, N, magma_d_AtA, N, 0.0,
                        magma_d_M2, N, magmaqueue);
        ...
        if (MAGMAfloat == 1)
        {
            magma_sgemm(MagmaNoTrans, MagmaNoTrans, M, N, N, 1.0, magmaf_d_A, M, magmaf_d_M2, N,
                        0.0, magmaf_d_Ainv, M, magmaqueue);
        }
        else
        {
            DEBUG_TRACEPOINT("double precision running magma_dgemm");
            magma_dgemm(MagmaNoTrans, MagmaNoTrans, M, N, N, 1.0, magma_d_A, M, magma_d_M2, N, 0.0,
                        magma_d_Ainv, M, magmaqueue);
    ...
    if (MAGMAloop_iter == 0)
    {
        ...
        FUNC_CHECK_RETURN(create_image_ID(ID_Cmatrix_name, data.image[ID_Rmatrix].md[0].naxis,
                                          arraysizetmp, datatype, 0, 0, 0, &ID_Cmatrix));
    ...
    if (testmode == 1) // compute product of Ainv with A
    {
        if (MAGMAfloat == 1)
        {
            magma_sgemm(MagmaTrans, MagmaNoTrans, N, N, M, 1.0, magmaf_d_A, M, magmaf_d_Ainv, M,
                        0.0, magmaf_d_AtA, N, magmaqueue);
        }
        else
        {
            magma_dgemm(MagmaTrans, MagmaNoTrans, N, N, M, 1.0, magma_d_A, M, magma_d_Ainv, M, 0.0,
                        magma_d_AtA, N, magmaqueue);
        }
