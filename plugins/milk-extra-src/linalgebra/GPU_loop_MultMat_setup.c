    ...
    if (gpumatmultconf[index].init == 0)
    {
        ...
        if (orientation == 0)
        {
            if (data.image[IDcontrM].md[0].naxis == 3)
            {
                ...
                gpumatmultconf[index].N =
                    data.image[IDcontrM].md[0].size[0] * data.image[IDcontrM].md[0].size[1];
                //   cmatdim = 3;
            }
            else
            {
        ...
        if (orientation == 0)
        {
            ...
            if ((uint32_t) (data.image[IDwfsim].md[0].size[0] *
                            data.image[IDwfsim].md[0].size[1]) != gpumatmultconf[index].N)
            {
                PRINT_ERROR(
        ...
        if ((gpumatmultconf[index].IDout = image_ID(IDoutdmmodes_name)) == -1)
        {
            ...
            sizearraytmp[1] = 1;
            create_image_ID(IDoutdmmodes_name, 2, sizearraytmp, _DATATYPE_FLOAT, 1, 10, 0,
        ...
        for (int device = 0; device < gpumatmultconf[index].NBstreams; device++)
        {
            ...
            sprintf(sname, "loop%02ld_i%02d_gpu%02d_sem1_%06d", loopnb, index, GPUdevice[device],
                    pid);
            ...
            sprintf(sname, "loop%02ld_i%02d_gpu%02d_sem2_%06d", loopnb, index, GPUdevice[device],
                    pid);
            ...
            sprintf(sname, "loop%02ld_i%02d_gpu%02d_sem3_%06d", loopnb, index, GPUdevice[device],
                    pid);
            ...
            sprintf(sname, "loop%02ld_i%02d_gpu%02d_sem4_%06d", loopnb, index, GPUdevice[device],
                    pid);
            ...
            sprintf(sname, "loop%02ld_i%02d_gpu%02d_sem5_%06d", loopnb, index, GPUdevice[device],
                    pid);
