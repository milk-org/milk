...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Apply a linear transformation defined by a modal mapping matrix. Remaps "
                        "coefficients from one modal basis to another."
};
...
#define FPS_PARAMS(X)                                                                  \
    X(".inM", &inM, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image")         \
    X(".inU0", &inU0, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input space mode")  \
    X(".inU1", &inU1, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "output space mode") \
    X(".outM", &outM, FPTYPE_STRING, 1, FPFLAG_DEFAULT_INPUT, "output M")
...
errno_t ModalRemap(IMGID imgM0, IMGID imgU0, IMGID imgU1, IMGID *imgM1, int GPUdev)
{
    ...
    // evaluate fit quality
    {
        ...
        fprintf(fp, "# AVERAGE  %20g  %20g   MEDIAN  %20g  %20g\n", res0_average, res1_average,
                res0array[NBframe / 2], res1array[NBframe / 2]);
