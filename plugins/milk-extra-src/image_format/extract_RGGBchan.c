...
static CLICMDARGDEF farg[] = { { CLIARG_STR, ".inim", "input RGGB image", "inim",
                               ...
                               { CLIARG_STR, ".outimR", "output R image", "inim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outimR, &fpi_outimR },
                               { CLIARG_STR, ".outimG1", "output G1 image", "outimG1",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outimG1, &fpi_outimG1 },
                               { CLIARG_STR, ".outimG2", "output G2 image", "outimG2",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outimG2, &fpi_outimG2 },
...
//
errno_t image_format_extract_RGGBchan(IMGID imgin,
                                      IMGID imgoutR,
                                      IMGID imgoutG1,
                                      IMGID imgoutG2,
                                      IMGID imgoutB)
    ...
    case _DATATYPE_FLOAT:
        for (uint32_t ii = 0; ii < imgoutR.size[0]; ii++)
        {
            for (uint32_t jj = 0; jj < imgoutR.size[1]; jj++)
            {
                ...
                imgoutR.im->array.F[pixi]  = imgin.im->array.F[(jj1 + 1) * xsize + ii1];
                imgoutG1.im->array.F[pixi] = imgin.im->array.F[jj1 * xsize + ii1];
                imgoutG2.im->array.F[pixi] = imgin.im->array.F[(jj1 + 1) * xsize + (ii1 + 1)];
                imgoutB.im->array.F[pixi]  = imgin.im->array.F[jj1 * xsize + (ii1 + 1)];
            }
        }
        break;
    ...
    case _DATATYPE_DOUBLE:
        for (uint32_t ii = 0; ii < imgoutR.size[0]; ii++)
        {
            for (uint32_t jj = 0; jj < imgoutR.size[1]; jj++)
            {
                ...
                imgoutR.im->array.D[pixi]  = imgin.im->array.D[(jj1 + 1) * xsize + ii1];
                imgoutG1.im->array.D[pixi] = imgin.im->array.D[jj1 * xsize + ii1];
                imgoutG2.im->array.D[pixi] = imgin.im->array.D[(jj1 + 1) * xsize + (ii1 + 1)];
                imgoutB.im->array.D[pixi]  = imgin.im->array.D[jj1 * xsize + (ii1 + 1)];
            }
        }
        break;
    ...
    case _DATATYPE_UINT16:
        for (uint32_t ii = 0; ii < imgoutR.size[0]; ii++)
        {
            for (uint32_t jj = 0; jj < imgoutR.size[1]; jj++)
            {
                ...
                imgoutR.im->array.UI16[pixi]  = imgin.im->array.UI16[(jj1 + 1) * xsize + ii1];
                imgoutG1.im->array.UI16[pixi] = imgin.im->array.UI16[jj1 * xsize + ii1];
                imgoutG2.im->array.UI16[pixi] = imgin.im->array.UI16[(jj1 + 1) * xsize + (ii1 + 1)];
                imgoutB.im->array.UI16[pixi]  = imgin.im->array.UI16[jj1 * xsize + (ii1 + 1)];
            }
        }
        break;
    }
