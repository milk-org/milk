...
{
    RegisterCLIcommand("loadcr2torgb", __FILE__, IMAGE_FORMAT_loadCR2toFITSRGB_cli,
                       "load CR2 file into R G B images", "<input image> <imR> <imG> <imB>",
                         ...
                         const char *__restrict fnameFITSb)
{
    ...
    if (CR2toFITS_NORM == 1)
    {
        ...
        EXECUTE_SYSTEM_COMMAND("dcraw -i -v %s | grep \"Shutter\"| awk '{print $2}' > "
                               "shutter_tmp.txt",
                               fnameCR2);
        ...
        EXECUTE_SYSTEM_COMMAND("dcraw -i -v %s | grep \"Aperture\"| awk '{print $2}' > "
                               "aperture_tmp.txt",
                               fnameCR2);
