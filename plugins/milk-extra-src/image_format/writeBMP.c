...
static CLICMDARGDEF farg[] = { { CLIARG_STR, ".bmp_fname", "BMP file name", "out.bmp",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &BMPfname, NULL },
                               { CLIARG_IMG, ".imRname", "Red channel image", "imR",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &imRname, NULL },
                               ...
                               { CLIARG_IMG, ".imBname", "Blue channel image", "imB",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &imBname, NULL } };
                       ...
                       char *__restrict outname)
{
    ...
    for (ii = 0; ii < width; ii++)
    {
        ...
        {
            array[(jj * width + ii) * 3] =
                (unsigned char) (data.image[IDB].array.F[(height - jj - 1) * width + ii]);

            array[(jj * width + ii) * 3 + 1] =
                (unsigned char) (data.image[IDG].array.F[(height - jj - 1) * width + ii]);

            array[(jj * width + ii) * 3 + 2] =
                (unsigned char) (data.image[IDR].array.F[(height - jj - 1) * width + ii]);
        }
    }
