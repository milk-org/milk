...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".inim", "input image", "im1", CLIARG_VISIBLE_DEFAULT,
                                 (void **) &inimname, NULL },
                               { CLIARG_IMG, ".modes", "modes image cube", "imcmode",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &modesimname, NULL },
                               { CLIARG_IMG, ".mask", "mask image", "immask",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &maskimname, NULL },
                               { CLIARG_FLOAT64, ".SVDeps", "SVD cutoff", "0.001",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &SVDeps, NULL },
                               { CLIARG_STR, ".outimcoeff", "output coeff image", "immask",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outcoeffimname, NULL },
                               { CLIARG_INT64, ".reuse", "reuse configuration flag", "0",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &reuse, NULL } };

static CLICMDDATA CLIcmddata = { "imfitmodes", "fit image as sum of modes",
                                      ...
                                      imageID    *outIDcoeff)
{
    ...
    if (0) // testing
    {
        printf("========  %s  %s  %s  %lf  %s  %d  ====\n", ID_name, IDmodes_name, IDmask_name,
               SVDeps, IDcoeff_name, reuse);
        list_image_ID();
