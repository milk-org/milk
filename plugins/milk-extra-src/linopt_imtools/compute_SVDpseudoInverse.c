...
#define FPS_PARAMS(X) \
    ...
    X(".outim", &outimname, \
      ...
      FPFLAG_DEFAULT_INPUT, \
      "output image") \
    X(".svdeps", &SVD_epsilon, \
      FPTYPE_FLOAT64, 1, \
      FPFLAG_DEFAULT_INPUT, \
      "SVD cutoff") \
    X(".maxNBmode", &max_NBmodes, \
      FPTYPE_INT64, 1, \
      FPFLAG_DEFAULT_INPUT, \
      "Maximum NB modes") \
    X(".outimVT", &outimVTmatname, \
      FPTYPE_STRING, 1, \
      FPFLAG_DEFAULT_INPUT, \
      "output VT matrix")
    ...
    imageID    *outID)
{
    ...
    ID_Rmatrix =
        image_ID(ID_Rmatrix_name, dcimg, dcnimg);
    if (ID_Rmatrix == -1) {
    ...
    double *Ds =
        calloc((size_t) m * n, sizeof(double));
    double *DtD =
    ...
    if (testmode == 1) {
        FUNC_CHECK_RETURN(
            create_2Dimage_ID("AtA", m, m,
                              &ID_AtA));
        for (long ii = 0; ii < m; ii++) {
            for (long jj = 0; jj < m; jj++) {
                dcimg[ID_AtA].array.F[
                    jj * m + ii] =
                    (float) DtD[ii + jj * m];
            }
        }
    ...
    /* Eigenvalue decomposition of DtD.
     * After dsyev, DtD columns = eigenvectors,
     * eval = eigenvalues in ascending order. */
    {
        /* dsyev destroys input, so work on DtD */
    ...
    if (testmode == 1) {
        save_fits(ID_VTmatrix_name,
                  "test_VT.fits");
    }

    /* Build diagonal inverse matrix (tmp1) */
    nbmodesremoved = 0;
    memset(tmp1, 0,
           (size_t) m * m * sizeof(double));
    for (long ii = 0; ii < m; ii++) {
        if (ii > MaxNBmodes1 - 1) {
            nbmodesremoved++;
        } else {
