...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".inim", "input image", "", CLIARG_VISIBLE_DEFAULT,
                                 (void **) &inimname, NULL },
                               { CLIARG_STR, ".outpixi", "output index image", "out1",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outpixiimname, NULL },
                               { CLIARG_STR, ".outpixm", "output mask image", "out1",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outpixmimname, NULL } };

static CLICMDDATA CLIcmddata = { "mask2pixtable", "make pixel tables from mask",
                                        ...
                                        long       *outNBpix)
{
    ...
    FUNC_CHECK_RETURN(
        create_image_ID(IDpixindex_name, 2, sizearray, _DATATYPE_INT64, 0, 0, 0, &IDpixindex));

    FUNC_CHECK_RETURN(
        create_image_ID(IDpixmult_name, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDpixmult));
    free(sizearray);
