...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".modes", "modes image cube", "imcmode",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &modesimname, NULL },
                               { CLIARG_IMG, ".invec", "input vector", "imvec",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &invecname, NULL },
                               { CLIARG_STR, ".outim", "output image", "outim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outimname, NULL } };

static CLICMDDATA CLIcmddata = { "imlinconstruct", "construct image as linear sum of modes",
        ...
        for (uint32_t kk = 0; kk < zsize; kk++)
        {
            for (uint64_t ii = 0; ii < sizexy; ii++)
            {
                data.image[ID].array.F[ii] +=
                    data.image[IDcoeff].array.F[kk] * data.image[IDmodes].array.F[kk * sizexy + ii];
            }
        }
    }
    else
        ...
        for (uint32_t kk = 0; kk < zsize; kk++)
        {
            for (uint64_t ii = 0; ii < sizexy; ii++)
            {
                data.image[ID].array.D[ii] +=
                    data.image[IDcoeff].array.D[kk] * data.image[IDmodes].array.D[kk * sizexy + ii];
            }
        }
    }
