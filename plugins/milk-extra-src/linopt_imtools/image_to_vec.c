...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".inim", "input image", "im", CLIARG_VISIBLE_DEFAULT,
                                 (void **) &inimname, NULL },
                               { CLIARG_IMG, ".inpixi", "input pixel index image", "pixi",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &inpixiname, NULL },
                               { CLIARG_IMG, ".inpixmult", "input pixel mult image", "pixmult",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &inpixmultname, NULL },
                               { CLIARG_STR, ".outvec", "output vector image", "vecim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outvecname, NULL } };

static CLICMDDATA CLIcmddata = { "im2vec", "remap image to vector", CLICMD_FIELDS_DEFAULTS };
            ...
            for (uint32_t kk = 0; kk < data.image[ID].md[0].size[2]; kk++)
            {
                for (long k = 0; k < NBpix; k++)
                {
                    data.image[IDvec].array.F[kk * NBpix * 2 + 2 * k] =
                        data.image[IDpixmult].array.F[k] *
                        data.image[ID]
                            .array.CF[kk * sizexy + data.image[IDpixindex].array.SI64[k]]
                            .re;
