...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".inimname", "input image", "im",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &inputimname, NULL },
                               { CLIARG_IMG, ".inmaskname", "mask image", "mask",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &inmaskname, NULL },
                               { CLIARG_IMG, ".mrespimname", "measured response images", "mresp",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &mrespimname, NULL },
                               { CLIARG_STR, ".outRM", "output RM image", "ourRM",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outRMimname, NULL } };

static CLICMDDATA CLIcmddata = { "lincRMiter", "estimate response matrix from input and output",
    ...
    for (spl = 0; spl < insize; spl++)
    {
        for (act = 0; act < NBact; act++)
        {
            data.image[IDpokeM].array.F[NBact * spl + act] =
                data.image[IDin].array.F[spl * xsizein * ysizein + inpixarray[act]];
        }
    }
                ...
                for (ii_out = 0; ii_out < xsizeout; ii_out++)
                {
                    for (jj_out = 0; jj_out < ysizeout; jj_out++)
                    {
                        data.image[IDtmp].array.F[jj_out * xsizeout + ii_out] +=
                            data.image[IDin]
                                .array.F[kk * xsizein * ysizein + jj_in * xsizein + ii_in] *
                            data.image[IDRM]
                                .array.F[(jj_in * xsizein + ii_in) * xsizeout * ysizeout +
