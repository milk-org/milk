...
{
    RegisterCLIcommand("dofft", __FILE__, fft_do2dfft_cli, "perform FFT", "<input> <output>",
                       "fofft in out", "int do2dfft(const char *in_name, const char *out_name)");

    RegisterCLIcommand("do1Dfft", __FILE__, fft_do1dfft_cli, "perform 1D complex->complex FFT",
                       "<input> <output>", "do1Dfft in out",
                       "int do1dfft(const char *in_name, const char *out_name)");

    RegisterCLIcommand("do1Drfft", __FILE__, fft_do1drfft_cli, "perform 1D real->complex FFT",
                       "<input> <output>", "do1drfft in out",
...
imageID FFT_do1dfft(const char *__restrict in_name, const char *__restrict out_name, int dir)
{
    ...
    create_image_ID(out_name, naxis, naxesl, datatype, data.SHARED_DFT, NB_KEYWNODE_MAX, 0, &IDout);
        ...
        if (array_index(naxes[0]) != 100)
        {
            ...
            if (datatype == _DATATYPE_COMPLEX_FLOAT)
            {
                plan = fftwf_plan_dft_1d(naxes[0], (fftwf_complex *) data.image[IDin].array.CF,
                                         (fftwf_complex *) data.image[IDout].array.CF, dir,
                                         FFTWOPTMODE);
                fftwf_execute(plan);
            ...
            {
                plan_double =
                    fftw_plan_dft_1d(naxes[0], (fftw_complex *) data.image[IDin].array.CD,
...
imageID do1drfft(const char *__restrict in_name, const char *__restrict out_name)
{
    ...
    {
        create_image_ID(out_name, naxis, naxesout, _DATATYPE_COMPLEX_DOUBLE, data.SHARED_DFT,
                        NB_KEYWNODE_MAX, 0, &IDout);
    }
    else
    {
        create_image_ID(out_name, naxis, naxesout, _DATATYPE_COMPLEX_FLOAT, data.SHARED_DFT,
                        NB_KEYWNODE_MAX, 0, &IDout);
    }
    ...
    if (naxis == 2)
    {
        if ((naxes[1] == 1) && (array_index(naxes[0]) != 100))
        {
            ...
            {
                plan_double =
                    fftw_plan_dft_r2c_1d(naxes[0], data.image[IDin].array.D,
        ...
        else
        {
            ...
            if (datatype == _DATATYPE_FLOAT)
            {
                ...
                {
                    memcpy((char *) inptr,
                           (char *) data.image[IDin].array.F + sizeof(float) * jj * naxes[0],
    ...
    if (naxis == 3)
    {
        ...
        if (datatype == _DATATYPE_FLOAT)
        {
            ...
            for (uint32_t ii = 0; ii < xysize; ii++)
            {
                ...
                {
                    data.image[IDout].array.CF[i * xysize + ii].re = outptr[i][0];
                    data.image[IDout].array.CF[i * xysize + ii].im = outptr[i][1];
                }
            }
            free(inptr);
            free(outptr);
        ...
        if (datatype == _DATATYPE_UINT16)
        {
            ...
            for (uint32_t ii = 0; ii < xysize; ii++)
            {
                for (int i = 0; i < naxes[2]; i++)
                {
                    inptr[i] = 1.0 * data.image[IDin].array.UI16[i * xysize + ii];
                }
                fftwf_execute(plan);
                for (uint32_t i = 0; i < naxesout[2]; i++)
                {
        ...
        if (datatype == _DATATYPE_UINT32)
        {
            ...
            for (uint32_t ii = 0; ii < xysize; ii++)
            {
                for (int i = 0; i < naxes[2]; i++)
                {
                    inptr[i] = 1.0 * data.image[IDin].array.UI32[i * xysize + ii];
                }
                fftwf_execute(plan);
                for (uint32_t i = 0; i < naxesout[2]; i++)
                {
        ...
        if (datatype == _DATATYPE_UINT64)
        {
            ...
            for (uint32_t ii = 0; ii < xysize; ii++)
            {
                for (int i = 0; i < naxes[2]; i++)
                {
                    inptr[i] = 1.0 * data.image[IDin].array.UI64[i * xysize + ii];
                }
                fftwf_execute(plan);
                for (uint32_t i = 0; i < naxesout[2]; i++)
                {
...
imageID FFT_do2dfft(const char *in_name, const char *out_name, int dir)
{
    ...
    if (naxis == 2)
    {
        ...
        {
            plan =
                fftwf_plan_dft_2d(naxes[0], naxes[1], (fftwf_complex *) data.image[IDin].array.CF,
            ...
            if (plan == NULL)
            {
                ...
                plan = fftwf_plan_dft_2d(
                    naxes[0], naxes[1], (fftwf_complex *) data.image[IDin].array.CF,
                    (fftwf_complex *) data.image[IDout].array.CF, dir, FFTWOPTMODE);
        ...
        {
            plan_double =
                fftw_plan_dft_2d(naxes[0], naxes[1], (fftw_complex *) data.image[IDin].array.CD,
                                 (fftw_complex *) data.image[IDout].array.CD, dir, FFTWOPTMODE);
            if (plan_double == NULL)
            {
                //	  if ( Debug > 2)
    ...
    if (naxis == 3)
    {
        ...
        if (datatype == _DATATYPE_COMPLEX_FLOAT)
        {
            plan = fftwf_plan_many_dft(
                2, naxes, naxes[2], (fftwf_complex *) data.image[IDin].array.CF, NULL, 1,
        ...
        else
        {
            plan_double = fftw_plan_many_dft(
                2, naxes, naxes[2], (fftw_complex *) data.image[IDin].array.CD, NULL, 1,
...
imageID FFT_do2drfft(const char *__restrict in_name, const char *__restrict out_name, int dir)
{
    ...
    create_image_ID(ffttmpname, naxis, naxestmp, datatypeout, data.SHARED_DFT, NB_KEYWNODE_MAX, 0,
                    &IDtmp);

    create_image_ID(out_name, naxis, naxesl, datatypeout, data.SHARED_DFT, NB_KEYWNODE_MAX, 0,
                    &IDout);
    ...
    if (naxis == 2)
    {
        ...
        {
            plan_double =
                fftw_plan_dft_r2c_2d(naxes[1], naxes[0], data.image[IDin].array.D,
    ...
    if (naxis == 3)
    {
        ...
        if (datatype == _DATATYPE_FLOAT)
        {
            plan = fftwf_plan_many_dft_r2c(2, naxes, naxes[2], data.image[IDin].array.F, NULL, 1,
                                           naxes[0] * naxes[1],
                                           (fftwf_complex *) data.image[IDout].array.CF, NULL, 1,
            ...
            if (plan == NULL)
            {
                ...
                plan = fftwf_plan_many_dft_r2c(2, naxes, naxes[2], data.image[IDin].array.F, NULL,
                                               1, naxes[0] * naxes[1],
                                               (fftwf_complex *) data.image[IDout].array.CF, NULL,
            ...
            if (dir == -1)
            {
                ...
                for (uint32_t ii = 0; ii < (uint32_t) (naxes[0] / 2 + 1); ii++)
                {
                    for (uint32_t jj = 0; jj < (uint32_t) naxes[1]; jj++)
                    {
                        ...
                        {
                            data.image[IDout]
                                .array.CF[naxes[0] * naxes[1] * kk + jj * naxes[0] + ii] =
                                data.image[IDtmp].array.CF[naxestmp[0] * naxestmp[1] * kk +
                                                           jj * naxestmp[0] + ii];
                            if (ii != 0)
                            {
                                data.image[IDout].array.CF[naxes[0] * naxes[1] * kk +
        ...
        {
            plan_double = fftw_plan_many_dft_r2c(2, naxes, naxes[2], data.image[IDin].array.D, NULL,
                                                 1, naxes[0] * naxes[1],
                                                 (fftw_complex *) data.image[IDout].array.CD, NULL,
                                                 1, naxes[0] * naxes[1], FFTWOPTMODE);
            if (plan == NULL)
            {
                //	  if ( Debug > 2) fprintf(stdout,"New FFT size [do2drfft %d x %d
                ...
                plan_double = fftw_plan_many_dft_r2c(2, naxes, naxes[2], data.image[IDin].array.D,
            ...
            if (dir == -1)
            {
                ...
                for (uint32_t ii = 0; ii < (uint32_t) (naxes[0] / 2 + 1); ii++)
                {
                    for (uint32_t jj = 0; jj < (uint32_t) naxes[1]; jj++)
                    {
                        ...
                        {
                            data.image[IDout]
                                .array.CD[naxes[0] * naxes[1] * kk + jj * naxes[0] + ii] =
                                data.image[IDtmp].array.CD[naxestmp[0] * naxestmp[1] * kk +
                                                           jj * naxestmp[0] + ii];
                            if (ii != 0)
                            {
                                data.image[IDout].array.CD[naxes[0] * naxes[1] * kk +
                                                           jj * naxes[0] + (naxes[0] - ii)] =
                                    data.image[IDtmp].array.CD[naxestmp[0] * naxestmp[1] * kk +
                                                               jj * naxestmp[0] + ii];
                            }
