...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 3) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) +
            CLI_checkarg(5, 2) ==
        0)
    {
...
{
    RegisterCLIcommand("imcontract", __FILE__, image_basic_contract_cli, "image binning",
                       "<inim> <outim> <binx> <biny>", "imcontract im1 outim 4 4",
                       "long basic_contract(const char *ID_name, const char "
                       "*ID_name_out, int n1, int n2)");

    RegisterCLIcommand("imcontract3D", __FILE__, image_basic_contract3D_cli, "image binning (3D)",
                       "<inim> <outim> <binx> <biny> <binz>", "imcontracteD im1 outim 4 4 1",
            ...
            for (i = 0; i < n1; i++)
            {
                for (j = 0; j < n2; j++)
                {
                    data.image[ID_out].array.F[jj * naxes_out[0] + ii] +=
                        data.image[ID].array.F[(jj * n2 + j) * naxes[0] + ii * n1 + i];
                }
            }
        }
    }
...
imageID basic_contract3D(const char *ID_name, const char *ID_name_out, int n1, int n2, int n3)
{
    ...
    else
    {
        printf("(%ld x %ld x %ld)  ->  (%ld x %ld x %ld)\n", (long) naxes[0], (long) naxes[1],
               (long) naxes[2], (long) naxes_out[0], (long) naxes_out[1], (long) naxes_out[2]);
                ...
                for (uint32_t kk = 0; kk < naxes_out[2]; kk++)
                {
                    for (int i = 0; i < n1; i++)
                    {
                        for (int j = 0; j < n2; j++)
                        {
                            for (int k = 0; k < n3; k++)
                            {
                                data.image[ID_out].array.F[kk * naxes_out[0] * naxes_out[1] +
                                                           jj * naxes_out[0] + ii] +=
                ...
                for (uint32_t kk = 0; kk < naxes_out[2]; kk++)
                {
                    for (int i = 0; i < n1; i++)
                    {
                        for (int j = 0; j < n2; j++)
                        {
                            for (int k = 0; k < n3; k++)
                            {
                                data.image[ID_out].array.D[kk * naxes_out[0] * naxes_out[1] +
                                                           jj * naxes_out[0] + ii] +=
                    ...
                    for (int i = 0; i < n1; i++)
                    {
                        for (int j = 0; j < n2; j++)
                        {
                            for (int k = 0; k < n3; k++)
                            {
                                data.image[ID_out]
                                    .array
                                    .CF[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
                                    .re += data.image[ID]
                                               .array
                                               .CF[(kk * n3 + k) * naxes[0] * naxes[1] +
                                                   (jj * n2 + j) * naxes[0] + ii * n1 + i]
                                               .re;
                                data.image[ID_out]
                                    .array
                    ...
                    for (int i = 0; i < n1; i++)
                    {
                        for (int j = 0; j < n2; j++)
                        {
                            for (int k = 0; k < n3; k++)
                            {
                                data.image[ID_out]
                                    .array
                                    .CD[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
                                    .re += data.image[ID]
                                               .array
                                               .CD[(kk * n3 + k) * naxes[0] * naxes[1] +
                                                   (jj * n2 + j) * naxes[0] + ii * n1 + i]
                                               .re;
                                data.image[ID_out]
                                    .array
