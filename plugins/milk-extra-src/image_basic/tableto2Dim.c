                          ...
                          float convsize)
{
        ...
        for (jj = 0; jj < ysize; jj++)
        {
            ...
            if (radius < 0.99 * radiusmax)
            {
                ...
                while (kd_res_end(presults) == 0)
                {
                    ...
                    pt_coeff1[i] =
                        pow(dist / radius0, 2.0) * (1.0 + cos(M_PI * dist / radius0)) / 2.0;
                ...
                dcimg[IDxerr].array.F[jj * xsize + ii] = (float) (x - xave);
                dcimg[IDyerr].array.F[jj * xsize + ii] = (float) (y - yave);
    ...
    if (presults != NULL)
    {
        PRINT_ERROR("presults was not freed by kd_res_free. Attempting free(presults) "
                    "and continuing.");
        free(presults);
    }
    kd_free(ptree);
    if (ptree != NULL)
    {
        PRINT_ERROR("ptree was not freed by kd_free. Attempting free(ptree) and "
                    "continuing.");
        free(ptree);
    }
