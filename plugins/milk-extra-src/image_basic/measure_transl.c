                            ...
                            long tmax)
{
    ...
    resolveIMGID(&img2, ERRMODE_WARN, dcimg, dcnimg);
    if (img2.ID == -1)
    {
        return RETURN_FAILURE;
    }
    ...
    IMGID imgout       = imgid_make_from_name_2D("TranslMap", sx_out, sy_out);
    imgout.mdt->shared = 0;
    imgout.im          = (IMAGE *) calloc(1, sizeof(IMAGE));
    imgid_mkimage(&imgout);

    IMGID imgcnt       = imgid_make_from_name_2D("TranslMapcnt", sx_out, sy_out);
    imgcnt.mdt->shared = 0;
    imgcnt.im          = (IMAGE *) calloc(1, sizeof(IMAGE));
    imgid_mkimage(&imgcnt);

    for (long iio = 0; iio < sx_out; iio++)
    {
        for (long jjo = 0; jjo < sy_out; jjo++)
        {
            imgout.im->array.F[jjo * sx_out + iio] = 0.0;
            imgcnt.im->array.F[jjo * sx_out + iio] = 0.0;
        }
    }
    ...
    double vlim = (double) img_percentile("_im1mask", 0.8);
    printf("vlim = %g\n", vlim);
    save_fl_fits("_im1mask", "_im1mask.0.fits");

    IMGID imgmask = imgid_make_from_name("_im1mask");
    resolveIMGID(&imgmask, ERRMODE_ABORT, dcimg, dcnimg);
    long xsizemask = imgmask.md->size[0];
    long ysizemask = imgmask.md->size[1];

    for (long ii = 0; ii < xsizemask * ysizemask; ii++)
    {
        if (imgmask.im->array.F[ii] > vlim)
        {
            imgmask.im->array.F[ii] = 1.0;
        }
        else
        {
            imgmask.im->array.F[ii] = 0.0;
        }
    }
    ...
    while (SCALE != 0)
    {
        ...
        for (long ii1 = ii1min; ii1 < ii1max; ii1 += step1)
        {
            for (long jj1 = jj1min; jj1 < jj1max; jj1 += step1)
            {
                long ii1m = (long) (ii1 / contractfactor);
                long jj1m = (long) (jj1 / contractfactor);
                if (imgmask.im->array.F[jj1m * xsizemask + ii1m] > Mlim)
                {
                    double v1 = img1.im->array.F[jj1 * size1x + ii1];

                    long ii2min = ii1 + dxmin - dsize;
                    long ii2max = ii1 + dxmin + dsize;
                    while (ii2min < 0)
                    {
                        ii2min += step2;
                    }
                    ...
                    long jj2min = jj1 + dymin - dsize;
                    long jj2max = jj1 + dymin + dsize;
                    while (jj2min < 0)
                    {
                        jj2min += step2;
                    }
                    ...
                    for (long ii2 = ii2min; ii2 < ii2max; ii2 += step2)
                    {
                        for (long jj2 = jj2min; jj2 < jj2max; jj2 += step2)
                        {
                            long dx  = ii2 - ii1;
                            long dy  = jj2 - jj1;
                            long dx1 = dx - dxmin;
                            ...
                            if (dx1 * dx1 + dy1 * dy1 < 1.0 * dsize * dsize)
                            {
                                ...
                                {
                                    double v2  = img2.im->array.F[jj2 * size2x + ii2];
                                    double tmp = (v1 - v2);
                                    imgout.im->array.F[jjo * sx_out + iio] += tmp * tmp;
                                    imgcnt.im->array.F[jjo * sx_out + iio] += 1.0;
                                }
                            }
                        }
                    }
        ...
        for (long iio = 0; iio < sx_out; iio++)
        {
            for (long jjo = 0; jjo < sy_out; jjo++)
            {
                if (imgcnt.im->array.F[jjo * sx_out + iio] > 0.1)
                {
                    double val = imgout.im->array.F[jjo * sx_out + iio] /
                                 imgcnt.im->array.F[jjo * sx_out + iio];
                    if (val < vmin)
                    {
                        vmin    = val;
                        vmincnt = imgcnt.im->array.F[jjo * sx_out + iio];
                        vdx     = 1.0 * iio - tmax;
                        vdy     = 1.0 * jjo - tmax;
                    }
                }
    ...
    for (long iio = 0; iio < sx_out; iio++)
    {
        for (long jjo = 0; jjo < sy_out; jjo++)
        {
            if (imgcnt.im->array.F[jjo * sx_out + iio] > 0.1)
            {
                imgout.im->array.F[jjo * sx_out + iio] /= imgcnt.im->array.F[jjo * sx_out + iio];
            }
        }
    }

    imageID ID = gauss_filter("TranslMap", "TranslMapg", 5.0, 10);

    vmin = 1.0e100;

    long iiomin = sx_out / 2 + dxmin - 20;
    if (iiomin < 0)
    {
        iiomin = 0;
    }
    long iiomax = sx_out / 2 + dxmin + 20;
    if (iiomax > sx_out - 1)
    {
        iiomax = sx_out - 1;
    }

    long jjomin = sy_out / 2 + dymin - 20;
    if (jjomin < 0)
    {
        jjomin = 0;
    }
    long jjomax = sy_out / 2 + dymin + 20;
    if (jjomax > sy_out - 1)
    {
        jjomax = sy_out - 1;
    }

    for (long iio = iiomin; iio < iiomax; iio++)
    {
        for (long jjo = jjomin; jjo < jjomax; jjo++)
        {
            if (imgcnt.im->array.F[jjo * sx_out + iio] > 0.1)
            {
                double val = dcimg[ID].array.F[jjo * sx_out + iio];
                if (val < vmin)
                {
                    vmin = val;
                    vdx  = 1.0 * iio - tmax;
