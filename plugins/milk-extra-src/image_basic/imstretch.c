    ...
    for (step = 0; step < NBstep; step++)
    {
        ...
        for (uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            for (uint32_t jj = 0; jj < naxes[1]; jj++)
            {
                ...
                {
                    tmp = (1.0 - u) * (1.0 - t) * data.image[IDin].array.F[j * naxes[0] + i];
                    tmp += (1.0 - u) * t * data.image[IDin].array.F[(j + 1) * naxes[0] + i];
                    tmp += u * (1.0 - t) * data.image[IDin].array.F[j * naxes[0] + i + 1];
                    tmp += u * t * data.image[IDin].array.F[(j + 1) * naxes[0] + i + 1];
                    data.image[IDout].array.F[jj * naxes[0] + ii] += mcoeff * tmp / coeff / coeff;
                }
            }
        }
    }
