...
{
    RegisterCLIcommand("imexpand", __FILE__, image_basic_expand_cli, "expand 2D image",
                       "<image in> <output image> <x factor> <y factor>", "imexpand im1 im2 2 2",
                       "long basic_expand(const char *ID_name, const char "
                       "*ID_name_out, int n1, int n2)");

    RegisterCLIcommand("imexpand3D", __FILE__, image_basic_expand3D_cli, "expand 3D image",
...
imageID basic_expand(const char *ID_name, const char *ID_name_out, int n1, int n2)
{
    ...
    FUNC_CHECK_RETURN(create_2Dimage_ID(ID_name_out, naxes_out[0], naxes_out[1], &ID_out));
...
imageID basic_expand3D(const char *ID_name, const char *ID_name_out, int n1, int n2, int n3)
{
    ...
    printf(" %ld %ld %ld -> %ld %ld %ld\n", naxes[0], naxes[1], naxes[2], naxes_out[0],
           naxes_out[1], naxes_out[2]);

    create_3Dimage_ID(ID_name_out, naxes_out[0], naxes_out[1], naxes_out[2], &ID_out);
    list_image_ID();
