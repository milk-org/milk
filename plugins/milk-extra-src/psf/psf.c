...
errno_t PSF_sequence_measure_cli()
{
    ...
    {
        PSF_sequence_measure(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numf,
...
{
    RegisterCLIcommand("psfseqmeas", __FILE__, PSF_sequence_measure_cli, "measure PSF sequence",
    ...
    for (int k = 0; k < nbiter; k++)
    {
        ...
        for (uint32_t jj = jjstart; jj < jjend; jj++)
        {
            for (uint32_t ii = iistart; ii < iiend; ii++)
            {
                ...
                {
                    centerx +=
                        1.0 * ii * (data.image[ID].array.F[jj * naxes[0] + ii] - 1.0 * back_cont);
                    centery +=
                        1.0 * jj * (data.image[ID].array.F[jj * naxes[0] + ii] - 1.0 * back_cont);
    ...
    for (uint32_t jj = 0; jj < naxes[1]; jj++)
    {
        for (uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            ...
            data.image[IDout].array.F[jj * naxes[0] + ii] =
                exp(-pow(dist / radius, index)) * data.image[ID1].array.F[jj * naxes[0] + ii] +
...
imageID PSF_coaddbest(const char *IDcin_name, const char *IDout_name, float r_pix)
{
    ...
    for (kk = 0; kk < ksize; kk++)
    {
        ...
        {
            flux_array[kk] -=
                data.image[IDcin].array.F[kk * xsize * ysize + ii] * data.image[IDmask].array.F[ii];
        }
    }
