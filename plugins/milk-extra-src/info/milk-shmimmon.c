...
#define SHMIMMON_DESCRIPTION "monitor a shared-memory image stream"
 ...
 */
static void compute_stats(IMAGE *img, StreamStats *stats)
{
...
#define PIXEL_PRINT(FMT, FIELD)                              \
    ...
    case _DATATYPE_UINT64:
        PIXEL_PRINT("%5lu", UI64);
        break;
    case _DATATYPE_INT64:
        PIXEL_PRINT("%5ld", SI64);
        break;
    default:
        break;
    }
 ...
 */
static void draw_frame(IMAGE             *img,
...
#define _TPRINT_IMPL(s) \
    ...
    /* — header — */
    {
        ...
        char shape[128];
        snprintf(shape, sizeof(shape), "%s [%u", ImageStreamIO_typename(md->datatype),
                 (unsigned) md->size[0]);
    ...
    TPRINT("[cnt0 %10u] [cnt1 %10u] [%5.1f Hz meas] [%5.1f Hz tgt]\n", (unsigned) md->cnt0,
           (unsigned) md->cnt1, hz_meas, hz_target);
    ...
    /* — circular buffer — */
    {
        ...
        TPRINT("[semlog %3d]  [circbuff %3u/%3u  %6lu]\n", semlogval, (unsigned) md->CBindex,
               (unsigned) md->CBsize, (unsigned long) md->CBcycle);
    }
    ...
    else
    {
        ...
        for (int h = 0; h < SHMIMMON_HIST_NBINS; h++)
        {
            ...
            char label[64];
            snprintf(label, sizeof(label), "[%12.4e - %12.4e] %7ld", (double) lo, (double) hi,
                     stats->hist[h]);
...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n\n", mh_color ? MH_OPT : "", "-f <hz>", mh_color ? MH_RST : "",
           "Refresh rate in Hz (default: 10)");

    milk_help_section("Keys (while running)", mh_color);
    printf("  %s%-15s%s %s\n", mh_color ? MH_OPT : "", "q / x", mh_color ? MH_RST : "", "Quit");
    printf("  %s%-15s%s %s\n", mh_color ? MH_OPT : "", "SPACE", mh_color ? MH_RST : "",
           "Reset RMS exponential filter");
...
int main(int argc, char *argv[])
{
    ...
    int action = milk_help_init(
        argc, argv, SHMIMMON_DESCRIPTION,
    ...
    {
        char shmpath[512];
        snprintf(shmpath, sizeof(shmpath), "%s/%s.im.shm", get_shm_dir(), stream);
