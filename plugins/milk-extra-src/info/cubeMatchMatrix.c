...
static char p_in[FUNCTION_PARAMETER_STRMAXLEN]  = "incube";
static char p_out[FUNCTION_PARAMETER_STRMAXLEN] = "outim";

static FPS_APP_INFO FPS_app_info = {
    .fps_name         = "cubeslmatch",
    .cmdkey           = "cubeslmatch",
    .description      = "compute sqsum diffs between slices",
    .description_long = "Compute a matrix of sum-of-squared differences between all pairs of "
                        "slices in an image cube. Used for similarity analysis."
};
...
imageID info_cubeMatchMatrix(const char *IDin_name, const char *IDout_name)
{
    ...
    for (kk1 = 0; kk1 < zsize; kk1++)
    {
        for (kk2 = kk1 + 1; kk2 < zsize; kk2++)
        {
            ...
            {
                array_matchV[ii]  = (double) dcimg[IDout].array.F[kk2 * zsize + kk1];
    ...
    if (ID0 != -1)
    {
        ...
        for (unsigned long ii = 0; ii < xysize; ii++)
        {
            dcimg[IDrmsim].array.F[ii] = sqrt(dcimg[IDrmsim].array.F[ii] / kmax);
        }
