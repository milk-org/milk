...
errno_t info_profile_cli()
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) +
        ...
        0)
    {
        profile(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
                data.cmdargtoken[3].val.numf, data.cmdargtoken[4].val.numf,
                data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.numl);
...
{
    RegisterCLIcommand("profile", __FILE__, info_profile_cli, "radial profile",
