...
errno_t info_cubestats_cli()
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_IMG) +
        ...
        0)
    {
        info_cubestats(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand("cubestats", __FILE__, info_cubestats_cli, "image cube stats",
                       "<3Dimage> <mask> <output file>", "cubestats imc immask imc_stats.txt",
...
imageID info_cubestats(const char *ID_name, const char *IDmask_name, const char *outfname)
{
    ...
    if (COMPUTE_CORR == 1)
    {
        ...
        {
            vcorr = 0.0;
            for (unsigned long kk = 0; kk < (unsigned long) (data.image[ID].md[0].size[2] - kc);
                 kk++)
            {
                k1    = kk;
                k2    = kk + kc;
