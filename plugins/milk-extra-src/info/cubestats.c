...
static char p_cube[FUNCTION_PARAMETER_STRMAXLEN] = "imc";
static char p_mask[FUNCTION_PARAMETER_STRMAXLEN] = "immask";
static char p_out[FUNCTION_PARAMETER_STRMAXLEN]  = "imc_stats.txt";

static FPS_APP_INFO FPS_app_info = { .fps_name    = "cubestats",
                                     .cmdkey      = "cubestats",
                                     .description = "image cube stats",
                                     .description_long =
                                         "Compute per-slice statistics (mean, rms, min, max) of a "
                                         "3D image cube. Outputs a summary table." };

#define FPS_PARAMS(X)                                                                   \
    X(".in_cube", p_cube, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input 3D image") \
...
imageID info_cubestats(const char *ID_name, const char *IDmask_name, const char *outfname)
{
    ...
    for (unsigned long kk = 0; kk < dcimg[ID].md[0].size[2]; kk++)
    {
        ...
        fprintf(fp, "%5ld  %20f  %20f  %20f  %20f  %20f  %20f\n", kk, min, max, tot, tot / mtot,
                tot2, sqrt((tot2 - tot * tot / mtot) / mtot));
    }
    fclose(fp);
    ...
    if (COMPUTE_CORR == 1)
    {
        ...
        {
            vcorr = 0.0;
            for (unsigned long kk = 0; kk < (unsigned long) (dcimg[ID].md[0].size[2] - kc); kk++)
            {
                k1    = kk;
                k2    = kk + kc;
                valn1 = 0.0;
