...
int main(int argc, char *argv[]) {
    ...
    while (loop) {
        ...
        int hist_len = 50;
        if (hist_len > smon->size) hist_len = smon->size;
        ...
        for (int i = 0; i < hist_len; i++) {
            ...
            if (val < min_flux) min_flux = val;
            if (val > max_flux) max_flux = val;
        }

        if (max_flux == min_flux) max_flux = min_flux + 1.0;
        ...
        if (nbins > STREAM_MON_MAX_HIST_BINS) nbins = STREAM_MON_MAX_HIST_BINS;
        ...
        if (available_rows < 5) available_rows = 5; // Minimum space
        ...
        for (uint32_t i = 0; i < nbins; i += bin_factor) {
            ...
            if (end_idx > nbins) end_idx = nbins;
