...
static CLICMDARGDEF farg[] = {
    { CLIARG_IMG, ".in_name", "input image cube", "imc1", CLIARG_VISIBLE_DEFAULT,
      (void **) &farg_inimname, NULL },
    { CLIARG_STR, ".outdname", "output directory name", "outd", CLIARG_VISIBLE_DEFAULT,
      (void **) &farg_outdname, NULL },
    { CLIARG_FLOAT32, ".T", "threshold", "1.0", CLIARG_HIDDEN_DEFAULT, (void **) &threshold,
      &fpi_threshold },
    { CLIARG_UINT32, ".B", "branch number", "10", CLIARG_HIDDEN_DEFAULT, (void **) &branchB,
      &fpi_branchB },
    { CLIARG_UINT32, ".leafposmode", "leaf position mode (0:fixed, 1:dyn)", "1",
      CLIARG_HIDDEN_DEFAULT, (void **) &leafposmode, &fpi_leafposmode },
    { CLIARG_UINT32, ".NBCFmax", "max number of CFs", "2048", CLIARG_HIDDEN_DEFAULT,
      (void **) &NBCFmax, &fpi_NBCFmax },
    { CLIARG_ONOFF, ".opt.rebuild", "rebuild tree after scan", "1", CLIARG_HIDDEN_DEFAULT,
      (void **) &optrebuild, &fpi_optrebuild },
    { CLIARG_ONOFF, ".opt.condense", "condense tree after scan", "1", CLIARG_HIDDEN_DEFAULT,
      (void **) &optcondense, &fpi_optcondense }
};
        ...
        if (ctree->CFarray[cfi].type != CLUSTER_CF_TYPE_UNUSED)
        {
            if (ctree->CFarray[cfi].N < 1)
            {
                FUNC_RETURN_FAILURE("node %ld type %d at level %d has N = %ld\n", cfi,
                                    ctree->CFarray[cfi].type, ctree->CFarray[cfi].level,
                                    ctree->CFarray[cfi].N);
            }
            ...
            if (ctree->CFarray[cfi].type == CLUSTER_CF_TYPE_NODE)
            {
                ...
                {
                    FUNC_RETURN_FAILURE("node %ld at level %d number of childred %d exceeds "
                                        "limit %d",
                                        cfi, ctree->CFarray[cfi].level, ctree->CFarray[cfi].NBchild,
                                        ctree->B);
                }
            }
        }
...
static errno_t findleafnode(CLUSTERTREE *ctree, double *datavec, long *nodeindex, double *distance)
{
    ...
    if (findleadnodecnt != 0)
    {
        ...
        {
            double distval = 0.0;
            FUNC_CHECK_RETURN(compute_imdistance_double(ctree, ctree->CFarray[CFindex].dataposvec,
                                                        1, datavec, 1, &distval));
                ...
                if (skipflag == 0)
                {
                    ...
                    if (ctree->leafposmode == CLUSTER_CFPOS_DYNAMIC)
                    {
                        FUNC_CHECK_RETURN(compute_imdistance_double(
                            ctree, ctree->CFarray[CFindex1].datasumvec, ctree->CFarray[CFindex1].N,
                            datavec, 1, &distval));
                    }
                    else
                    {
                        FUNC_CHECK_RETURN(compute_imdistance_double(
                            ctree, ctree->CFarray[CFindex1].dataposvec, 1, datavec, 1, &distval));
                    }
...
static errno_t imcube_makecluster(IMGID img, const char *__restrict outdname)
{
    ...
    for (long frame = 0; frame < NBframe; frame++)
    {
        ...
        if (img.im->md->datatype == _DATATYPE_FLOAT)
        {
            ...
            {
                datarray[ii] = pixgain[ii] * img.im->array.F[frameC[frame] * xysize + pixmap[ii]];
        ...
        else if (img.im->md->datatype == _DATATYPE_DOUBLE)
        {
            for (long ii = 0; ii < CF_npix; ii++)
            {
                datarray[ii] = pixgain[ii] * img.im->array.D[frameC[frame] * xysize + pixmap[ii]];
                ssqr += datarray[ii] * datarray[ii];
            }
        }
        ...
        else
        {
            ...
            if (addOK == 1)
            {
                ...
                DEBUG_TRACEPOINT("Added entry %ld to leaf index %ld", frame, lCFindex);
            }
            else
            {
                DEBUG_TRACEPOINT("Radius condition not met for leaf index %ld", lCFindex);
            ...
            if (lCFindex == -1)
            {
                ...
                DEBUG_TRACEPOINT("ATTACHED LEAF %ld to %ld", nCFindex,
                ...
                {
                    DEBUG_TRACEPOINT("MAX BRANCH NUMBER REACHED -> SPLIT NODE");
                    ...
                    DEBUG_TRACEPOINT(
                        "NODE %ld(%d) -> %ld(%d) %ld(%d)", CFindex, ctree.CFarray[CFindex].NBchild,
                    ...
                    while (splitupstream == 1)
                    {
                        ...
                        DEBUG_TRACEPOINT("NODE %ld(%d) -> %ld(%d) %ld(%d)", CFindex,
                                         ctree.CFarray[CFindex].NBchild, CFi0,
                                         ctree.CFarray[CFi0].NBchild, CFi1,
    ...
    write_clustleafsummary(&ctree, img, pixmap, pixgain, frameleafCFindex, NBframe, outdname);
