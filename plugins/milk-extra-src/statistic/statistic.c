...
errno_t statistic_putgaussnoise_cli()
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) +
        ...
        0)
    {
        put_gauss_noise(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand("putphnoise", __FILE__, statistic_putphnoise_cli,
                       "add photon noise to image", "input output", "putphnoise im0 im1",
                       "int put_poisson_noise(const char *ID_in_name, const "
                       "char *ID_out_name)");

    RegisterCLIcommand("putgaussnoise", __FILE__, statistic_putgaussnoise_cli,
                       "add gaussian noise to image", "input output amplitude",
