...
## Handling Dual-Mode Compilation (CLI vs Standalone)

A compute unit's source file is compiled twice: once into the module's regular library (CLI enabled, via `add_subdirectory()`), and once directly into the standalone executable's own translation unit via `add_milk_standalone()` / `add_cacao_standalone()`, which applies `-DMILK_NO_CLI` to that executable target only.
...
1. **Mark `compute_function` as unused** in Section 6:
   ...
   _(Because it becomes orphaned when both CLI and standalone sections are excluded -- causes a warning in DEBUG builds.)._
