...
- **Native CLI Scripts** (`*.milk`): Scripts authored in the native scripting language designed to be evaluated by the CLI orchestrator MUST use the `.milk` extension (e.g., `makecircleofdisks.milk`). They MUST NOT be executed directly, but should be run by passing them to the CLI orchestrator natively: `milk-cli -s scriptname.milk`.
