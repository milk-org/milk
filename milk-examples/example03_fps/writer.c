 ...
 */
void writer03_compute(FUNCTION_PARAMETER_STRUCT *fps, PROCESSINFO *processinfo, IMAGE *output_image)
{
    ...
    if (!width_ptr || !height_ptr || !freq_x_ptr || !freq_y_ptr)
    {
        return;
    }
    ...
    for (uint32_t y = 0; y < height; y++)
    {
        ...
        {
            out_data[y * width + x] =
                0.5 * sin((x + counter) * freq_x) + 0.5 * cos((y + counter) * freq_y);
        }
    }
}
...
{
    if (width_ptr && *width_ptr < 1)
    {
        *width_ptr = 1;
    }
    if (height_ptr && *height_ptr < 1)
    {
        *height_ptr = 1;
    }
}
 ...
 */
int FPSINIT_writer(const char *fps_name, const char *keywords, const char *description)
{
 ...
 */
int FPSCONF_writer(const char *fps_name, int loop)
{
    FUNCTION_PARAMETER_STRUCT fps;
...
int FPSRUN_writer(const char *fps_name)
{
    ...
    if (ImageStreamIO_createIm_gpu(&output_image, out_name_ptr, 2, dims, _DATATYPE_FLOAT, -1, 1, 10,
                                   0, 0, 0) != 0)
    {
        return 1;
    }

    PROCESSINFO *processinfo = processinfo_setup((char *) fps_name, "Ex03 Writer", "Looping",
