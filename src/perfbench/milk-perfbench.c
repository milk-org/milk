...
#define N_PERF_EVS ((int) (sizeof(PERF_EVS) / sizeof(PERF_EVS[0])))
...
{
    printf("Usage: %s [options] <fpsexec> <nbiter>\n\n"
           "Options:\n"
 ...
 */
static int parse_args(int argc, char *argv[], bench_cfg_t *cfg)
{
    static const struct option long_opts[] = {
        { "warmup", required_argument, 0, 'w' },  { "outdir", required_argument, 0, 'o' },
        { "fpsargs", required_argument, 0, 'a' }, { "setup", required_argument, 0, 's' },
        { "help", no_argument, 0, 'h' },          { 0, 0, 0, 0 }
    };

    int c;
    while ((c = getopt_long(argc, argv, "w:o:a:s:h", long_opts, NULL)) != -1)
    {
        switch (c)
        {
        case 'w':
        ...
        case 'o':
            ...
            break;
        case 'a':
            strncpy(cfg->fpsargs, optarg, sizeof(cfg->fpsargs) - 1);
            break;
        case 's':
            strncpy(cfg->setupcmd, optarg, sizeof(cfg->setupcmd) - 1);
            break;
        case 'h':
            usage(argv[0]);
            exit(0);
    ...
    if (optind + 2 > argc)
    {
        fprintf(stderr, "Error: fpsexec and nbiter required\n");
        usage(argv[0]);
        return 1;
    }

    strncpy(cfg->fpsexec, argv[optind], sizeof(cfg->fpsexec) - 1);
    cfg->nbiter = atoi(argv[optind + 1]);

    if (cfg->nbiter <= 0)
    {
        fprintf(stderr, "Error: nbiter must be positive\n");
        return 1;
    }
    if (cfg->warmup >= cfg->nbiter)
    {
        fprintf(stderr, "Error: warmup must be < nbiter\n");
        return 1;
    }
 ...
 */
static void read_build_tags(const char *exe, char *out, size_t outsz)
{
    out[0] = '\0';
 ...
 */
static void fps_set(const char *fpsname, const char *param_tag, const char *value)
{
 ...
 */
static int find_proc_shm(const char *procdir, pid_t pid, char *out, size_t outsz)
{
    ...
    while ((de = readdir(d)) != NULL)
    {
        ...
        snprintf(out, outsz, "%s/%s", procdir, de->d_name);
        found = 1;
        break;
    }
    closedir(d);
 ...
 *        /proc/PID/status.
 */
static void read_proc_mem(pid_t pid,
                          long *vmpeak_kb,
                          long *vmhwm_kb,
                          long *vmrss_kb,
                          long *vol_ctxt,
 ...
 * is absent.
 */
static void read_cpu_freq(long *freq_min_khz, long *freq_max_khz)
{
    *freq_min_khz = -1;
    *freq_max_khz = -1;
    long fmin     = LONG_MAX;
...
{
    const char *rapl = "/sys/class/powercap/intel-rapl/"
 ...
 */
static void read_procinfo_stats(const char *shm_path,
                                pid_t       child_pid,
                                pi_stats_t *out,
                                long long   rapl_start)
{
    ...
    PROCESSINFO *pi = (PROCESSINFO *) mmap(NULL, (size_t) st.st_size, PROT_READ, MAP_SHARED, fd, 0);
    if (pi == MAP_FAILED)
    {
        close(fd);
        return;
    ...
    {
        long dt_exec = (pi->texecend[i].tv_sec - pi->texecstart[i].tv_sec) * 1000000000L +
                       (pi->texecend[i].tv_nsec - pi->texecstart[i].tv_nsec);
        long dt_iter = (pi->texecstart[i].tv_sec - pi->texecstart[i - 1].tv_sec) * 1000000000L +
                       (pi->texecstart[i].tv_nsec - pi->texecstart[i - 1].tv_nsec);
        /* Reject negative, zero, or implausibly
         * large values (stale ring buffer entries
         * from a previous FPS session). */
        if (dt_exec > 0 && dt_iter > 0 && dt_exec < 10000000000L && dt_iter < 10000000000L)
        {
            exec_ns[nv] = dt_exec;
            iter_ns[nv] = dt_iter;
            nv++;
    ...
    read_proc_mem(child_pid, &out->vmpeak_kb, &out->vmhwm_kb, &out->vmrss_kb, &out->vol_ctxt,
                  &out->nvol_ctxt);
    ...
    read_cpu_freq(&out->cpu_freq_min_khz, &out->cpu_freq_max_khz);
    ...
    /* RAPL energy delta */
    {
        ...
        if (rapl_start >= 0 && rapl_end >= 0)
        {
            ...
            out->rapl_uj = (rapl_end >= rapl_start) ? (rapl_end - rapl_start)
    ...
    qsort(exec_ns, (size_t) nv, sizeof(long), cmp_long);
    qsort(iter_ns, (size_t) nv, sizeof(long), cmp_long);

    /* Compute percentile index, clamped to [0,nv-1] */
#define PCTILE(arr, pct) (arr)[((nv * (pct) / 100) < nv ? (nv * (pct) / 100) : (nv - 1))]
    /* p99.9: need 1000-based arithmetic */
#define PCTILE999(arr) (arr)[((nv * 999 / 1000) < nv ? (nv * 999 / 1000) : (nv - 1))]
...
static int perf_open_all(pid_t pid, int *fds)
{
    ...
    for (int i = 0; i < N_PERF_EVS; i++)
    {
        ...
        fds[i] = (int) perf_event_open(&attr, pid, -1, -1, 0);
        if (fds[i] >= 0)
        {
 ...
 */
static void perf_read_close(int *fds, hw_phase_t *phase)
{
    phase->valid = 0;
    for (int i = 0; i < N_PERF_EVS; i++)
    {
 ...
 */
static void run_phase(const bench_cfg_t *cfg,
                      int                iters,
                      hw_phase_t        *phase,
                      pi_stats_t        *pi,
                      int                collect_pi,
                      long long         *wall_ns)
{
    ...
    char runarg[MAX_CMD];
    snprintf(runarg, sizeof(runarg), "%s:runstart", cfg->fpsname);
    ...
    if (child == 0)
    {
        ...
        execlp(cfg->fpsexec, cfg->fpsexec, runarg, NULL);
        _exit(127);
    }
    ...
    if (collect_pi && pi)
    {
        ...
        while (total_poll_ms < POLL_TIMEOUT_MS)
        {
            ...
            if (ret == child)
            {
                ...
                if (find_proc_shm(cfg->procdir, child, shm_path, sizeof(shm_path)))
                {
                    read_procinfo_stats(shm_path, child, pi, rapl_start);
                }
                perf_read_close(fds, phase);
                clock_gettime(CLOCK_MONOTONIC, &ts_end);
                *wall_ns = (long long) (ts_end.tv_sec - ts_start.tv_sec) * 1000000000LL +
            ...
            if (find_proc_shm(cfg->procdir, child, shm_path, sizeof(shm_path)))
            {
                ...
                read_procinfo_stats(shm_path, child, &tmp, rapl_start);
                if (tmp.valid)
                {
                    *pi = tmp;
                }
            ...
            poll_ts.tv_sec  = 0;
            poll_ts.tv_nsec = POLL_INTERVAL_MS * 1000000L;
 ...
 */
static void write_json(const bench_cfg_t *cfg,
                       const hw_phase_t  *t,
                       const hw_phase_t  *w,
                       int                measured,
                       long long          t_ns,
 ...
 */
static void print_summary(const bench_cfg_t *cfg,
                          int                measured,
                          const hw_phase_t  *t,
                          const hw_phase_t  *w,
                          long long          t_ns,
    ...
    {                                                                                              \
        ...
        if (hw && pi_w && pi_w->valid)                                                             \
            printf("  %s%-*s%s %s%*ld%s %-3s%*ld %-3s\n", CD, COL1W, (label), CR, CWH, COL2W,      \
                   (long) (tot), CR, (unit), COL2W - 1, _wv, (unit));                              \
        else                                                                                       \
            printf("  %s%-*s%s %s%*ld%s %-3s\n", CD, COL1W, (label), CR, CWH, COL2W, (long) (tot), \
                   CR, (unit));                                                                    \
    } while (0)
        ...
        const char *tunit = (scale == 1000L) ? "µs" : "ns";
...
#define print_timing_row(lbl, tot, wrm)                                                            \
    ...
    {                                                                                              \
        ...
        if (hw && pi_w && pi_w->valid)                                                             \
            printf("  %s%-*s%s %s%*ld%s %-3s%*ld %-3s\n", CD, COL1W, (lbl), CR, CWH, COL2W, _t,    \
                   CR, tunit, COL2W - 1, _w, tunit);                                               \
        else                                                                                       \
            printf("  %s%-*s%s %s%*ld%s %-3s\n", CD, COL1W, (lbl), CR, CWH, COL2W, _t, CR, tunit); \
    } while (0)
        ...
        printf("  %s%-*s%s %s%14ld%s %-3s%14ld %-3s"
               "%s%14ld%s %-3s\n",
               CD, COL1W, "  Iter time", CR, CD, (long) pi->p50_iter / scale, CR, tunit,
               (long) pi->p95_iter / scale, tunit, CWH, (long) pi->p99_iter / scale, CR, tunit);
        /* Exec row */
        printf("  %s%-*s%s %s%14ld%s %-3s%14ld %-3s"
               "%s%14ld%s %-3s\n",
               CD, COL1W, "  Exec time", CR, CD, (long) pi->p50_exec / scale, CR, tunit,
               (long) pi->p95_exec / scale, tunit, CWH, (long) pi->p99_exec / scale, CR, tunit);
        /* Jitter + max */
        printf("  %s%-*s%s %s%14ld%s %-3s   "
               "%s%-*s%s %s%14ld%s %-3s\n",
               CD, COL1W, "  Iter jitter (p99-p50)", CR, CWH, (long) pi->jitter_iter / scale, CR,
               tunit, CD, COL1W, "  Exec jitter (p99-p50)", CR, CWH, (long) pi->jitter_exec / scale,
               CR, tunit);
        printf("  %s%-*s%s %s%14ld%s %-3s   "
               "%s%-*s%s %s%14ld%s %-3s\n",
               CD, COL1W, "  Iter max", CR, CWH, (long) pi->max_iter / scale, CR, tunit, CD, COL1W,
               "  Exec max", CR, CWH, (long) pi->max_exec / scale, CR, tunit);
#undef print_timing_row

        print_section("Memory & OS");
        print_sep();
        print_pi_row("  Peak RSS", pi->vmhwm_kb, 0L, "kB");
        print_pi_row("  Virtual peak", pi->vmpeak_kb, 0L, "kB");
        if (pi->anon_huge_kb > 0)
        {
            print_pi_row("  Anon huge pages", pi->anon_huge_kb, 0L, "kB");
        }
        print_pi_row("  Vol ctx switches", pi->vol_ctxt, (pi_w ? pi_w->vol_ctxt : 0L), "");
        print_pi_row("  Nonvol ctx switches", pi->nvol_ctxt, (pi_w ? pi_w->nvol_ctxt : 0L), "");
        if (pi->cpu_freq_min_khz > 0)
        {
            print_pi_row("  CPU freq (min)", pi->cpu_freq_min_khz / 1000L,
                         (pi_w ? pi_w->cpu_freq_min_khz / 1000L : 0L), "MHz");
            print_pi_row("  CPU freq (max)", pi->cpu_freq_max_khz / 1000L,
                         (pi_w ? pi_w->cpu_freq_max_khz / 1000L : 0L), "MHz");
        }
        if (pi->rapl_uj >= 0)
        {
            print_pi_row("  RAPL (package)", (long) (pi->rapl_uj / 1000LL),
                         (pi_w && pi_w->rapl_uj >= 0 ? (long) (pi_w->rapl_uj / 1000LL) : 0L), "mJ");
        }
        else
        {
            printf("  %s%-*s%s %sN/A%s "
...
int main(int argc, char *argv[])
{
    ...
    /* Defaults */
    strncpy(cfg->outdir, "./perfresults", sizeof(cfg->outdir) - 1);
    ...
    make_fpsname(cfg->fpsname, sizeof(cfg->fpsname));
    ...
    /* Output path */
    {
        ...
        strncpy(exe_base, cfg->fpsexec, sizeof(exe_base) - 1);
        /* basename in-place */
        char *slash = strrchr(exe_base, '/');
        if (slash)
        {
            memmove(exe_base, slash + 1, strlen(slash));
        }
        snprintf(cfg->result_file, sizeof(cfg->result_file), "%s/%s-%s.json", cfg->outdir, exe_base,
                 ts);
    }
    ...
    read_build_tags(cfg->fpsexec, cfg->build_tags, sizeof(cfg->build_tags));
    ...
    printf("  Command   : %s\n", cfg->fpsexec);
    printf("  FPS name  : %s\n", cfg->fpsname);
    printf("  Build     : %s\n", cfg->build_tags[0] ? cfg->build_tags : "default (no PGO/LTO)");
    printf("  Iterations: %d\n", cfg->nbiter);
    if (cfg->warmup > 0)
    {
        printf("  Warmup    : %d\n", cfg->warmup);
    ...
    if (cfg->warmup > 0)
    {
        ...
        run_phase(cfg, cfg->warmup, &ph_warmup, &pi_warmup, 1, &w_ns);

        /* Re-init FPS for 2nd run */
        run_cmd("%s %s:fpsinit -procinfo"
                " >/dev/null 2>&1",
                cfg->fpsexec, cfg->fpsname);
        run_cmd("%s %s:confstep >/dev/null 2>&1", cfg->fpsexec, cfg->fpsname);
        fps_set(cfg->fpsname, "procinfo.enabled", "ON");
        fps_set(cfg->fpsname, "procinfo.MeasureTiming", "ON");
        fps_set(cfg->fpsname, "procinfo.triggermode", "0");

        printf("  [total ] %d iterations ...\n", cfg->nbiter);
        run_phase(cfg, cfg->nbiter, &ph_total, &pi, 1, &t_ns);
    }
    else
    {
    ...
    /* JSON */
    write_json(cfg, &ph_total, &ph_warmup, measured, t_ns, w_ns, &pi, &pi_warmup, sz);

    /* Summary */
    print_summary(cfg, measured, &ph_total, &ph_warmup, t_ns, w_ns, &pi, &pi_warmup, sz);
