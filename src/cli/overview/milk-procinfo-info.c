...
static void print_proc_info(const OV_MODEL *m, int pi)
{
    ...
    printf(C_LABEL " %-20s" C_RST ": " C_VAL "%d" C_RST " [%s]\n", "PID", (int) p->PID,
    ...
    if (p->mem_rss_kb > 0)
    {
        printf("   %-18s: " C_VAL "%" PRId64 " KB" C_RST "\n", "RSS memory",
    ...
    if (p->dtmedian_iter_ns > 0)
    {
        ...
        printf("   %-18s: " C_VAL "%.1f µs" C_RST "  (%.1f Hz)\n", "Iter (median)", iter_us,
               p->loop_hz);
    }
    else
    {
    ...
    if (p->dtmedian_exec_ns > 0)
    {
        ...
        printf("   %-18s: " C_VAL "%.1f µs" C_RST "  (duty: %.1f%%)\n", "Exec (median)", exec_us,
               duty);
    }
    else
    {
    ...
    printf("   %-18s: %s\n", "Measure timing",
           p->MeasureTiming ? C_ALIVE "ON" C_RST : C_DIM "OFF" C_RST);

    /* ---- Trigger ---- */
    printf("\n" C_HDR " Trigger" C_RST "\n");
    printf("   %-18s: " C_VAL "%s (%d)" C_RST "\n", "Mode", trigmode_str(p->triggermode),
           p->triggermode);
    ...
    printf("   %-18s: " C_VAL "%d" C_RST " (cumul: %" PRIu64 ")\n", "Missed frames",
    ...
    else
    {
        ...
        for (int e = 0; e < m->nb_edges; e++)
        {
            ...
            if (ni < 0 || ni >= m->nb_nodes || m->nodes[ni].type != OV_NODE_STREAM)
            {
                continue;
            }
        ...
        for (int e = 0; e < m->nb_edges; e++)
        {
            ...
            if (m->edges[e].type != OV_EDGE_FPS_RUNS_PROC)
            {
                continue;
            }
            int ni = m->edges[e].src_node;
...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "--pid PID", mh_color ? MH_RST : "",
           "Look up process by PID instead of name");
    ...
    printf("  %s$ milk-procinfo-info%s %s--pid%s %s1234%s\n\n", mh_color ? MH_CMD : "",
           mh_color ? MH_RST : "", mh_color ? MH_OPT : "", mh_color ? MH_RST : "",
           mh_color ? MH_ARG : "", mh_color ? MH_RST : "");
    const char *see_also[] = { "milk-procinfo-list:list active processinfo instances",
...
static int find_proc_by_name(const OV_MODEL *m, const char *name)
{
    ...
    {
        if (m->procs[i].valid && strcmp(m->procs[i].name, name) == 0)
        {
            return i;
        }
    }
