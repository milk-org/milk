    ...
    ov_perf_counters_t *out)
{
    ...
    if(s_perf_pid != pid)
    {
        ...
        s_perf_fd_l1d = open_perf_fd(pid, PERF_TYPE_HW_CACHE,
                                     (PERF_COUNT_HW_CACHE_L1D) | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_MISS <<
                                             16));
        s_perf_fd_llc = open_perf_fd(pid, PERF_TYPE_HW_CACHE,
                                     (PERF_COUNT_HW_CACHE_LL) | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_MISS <<
                                             16));
        s_perf_fd_dtlb = open_perf_fd(pid, PERF_TYPE_HW_CACHE,
                                      (PERF_COUNT_HW_CACHE_DTLB) | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_MISS
                                              << 16));
