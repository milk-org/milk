...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < m.nb_edges; i++)
    {
        ...
        const char *tnames[] = {
            "PROC_WRITES_STREAM", "STREAM_TRIGGERS_PROC", "FPS_RUNS_PROC",       "FPS_INPUT_STREAM",
            "FPS_OUTPUT_STREAM",  "PROC_TRIGGER_STREAM",  "STREAM_READ_BY_PROC",
        };
    ...
    for (int a = 0; a < lin.nb_ancestors; a++)
    {
        printf("  depth=%d stream[%d]=%s "
               "via=%s loop=%d\n",
               lin.ancestors[a].depth, lin.ancestors[a].stream_idx,
               m.streams[lin.ancestors[a].stream_idx].name, lin.ancestors[a].via_name,
    ...
    for (int d = 0; d < lin.nb_descendants; d++)
    {
        printf("  depth=%d stream[%d]=%s "
               "via=%s loop=%d\n",
               lin.descendants[d].depth, lin.descendants[d].stream_idx,
               m.streams[lin.descendants[d].stream_idx].name, lin.descendants[d].via_name,
