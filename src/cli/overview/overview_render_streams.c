            ...
            /* DEBUG: dump to file once */
            {
                ...
                if (!dbg_done)
                {
                    ...
                    if (df)
                    {
                        ...
                        const char *etnames[] = { "PROC_WRITES_STREAM", "STREAM_TRIGGERS_PROC",
                                                  "FPS_RUNS_PROC",      "FPS_INPUT_STREAM",
                                                  "FPS_OUTPUT_STREAM",  "PROC_TRIGGER_STREAM",
                                                  "STREAM_READ_BY_PROC" };
                        for (int ei = 0; ei < m->nb_edges; ei++)
                        {
                            const OV_EDGE *e  = &m->edges[ei];
                            const char    *sn = (e->src_node >= 0 && e->src_node < m->nb_nodes)
                                                    ? m->nodes[e->src_node].name
                                                    : "??";
                            const char    *tn = (e->tgt_node >= 0 && e->tgt_node < m->nb_nodes)
                        ...
                        for (int a = 0; a < lin.nb_ancestors; a++)
                        {
                            fprintf(df,
                                    ...
                                    "si=%d name=%s "
                                    "via=%s\n",
                                    a, lin.ancestors[a].depth, lin.ancestors[a].stream_idx,
                                    m->streams[lin.ancestors[a].stream_idx].name,
                        ...
                        for (int di = 0; di < lin.nb_descendants; di++)
                        {
                            fprintf(df,
                                    ...
                                    "si=%d name=%s "
                                    "via=%s\n",
                                    di, lin.descendants[di].depth, lin.descendants[di].stream_idx,
                                    m->streams[lin.descendants[di].stream_idx].name,
                                    lin.descendants[di].via_name);
                        }
                        fclose(df);
                    }
                }
