                              ...
                              struct streamCTRL_TUI_state *state)
{
    ...
    snprintf(monstring, monstrlen,
             "[%d x %d] [PID %d] STREAM MONITOR: PRESS (x) TO STOP, (h) FOR HELP", wrow, wcol,
             getpid());
    ...
    {
        streamCTRL__render_header_streams(streamCTRLdata, state, &NBsinfodisp, &lastindex,
                                          &frame_t_sec, &frame_color_level);
        streamCTRL__render_stream_rows(streamCTRLdata, state, NBsinfodisp, frame_t_sec,
                                       frame_color_level);
    }
