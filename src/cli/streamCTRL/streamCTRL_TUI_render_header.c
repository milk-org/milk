...
#undef RENDER_ONE_TAB
    ...
    if (sTUIparam.DisplayMode == DISPLAY_MODE_FUSER)
    {
        if (sTUIparam.fuserScan == 1)
        {
            TUI_printfw("Last scan on  %02d:%02d:%02d  - Press "
                        ...
                        "scan",
                        sTUIparam.uttime_lastScan->tm_hour, sTUIparam.uttime_lastScan->tm_min,
    ...
    else
    {
        ...
        static const char *sort_col_names[] = { "",     "NAME", "TYPE", "SIZE",
                                                "CNT0", "CPID", "OPID", "FREQ" };

        if (sTUIparam.sort_col > 0 && sTUIparam.sort_col <= STREAM_NB_SORT_COLS)
        {
            screenprint_setcolor(6);
            TUI_printfw("[SORT: %s %s]  ", sort_col_names[sTUIparam.sort_col],
                        sTUIparam.sort_dir ? "DESC" : "ASC");
...

    {
        ...
        TUI_printfw("%4d streams    Currently displaying %4ld-%4ld   "
                    "Selected %d  ID = %d  inode = %d",
                    sTUIparam.NBsindex, (long) doffsetindex, (long) lastindex,
                    sTUIparam.dindexSelected, ssIDselected, (int) inodeselected);
    }
    ...
    TUI_printfw("%*s  %-*s  %-*s  %*s   %*s %*s %*s", 9, "inode", DispName_NBchar, "name",
                DispSize_NBchar, "type", Dispcnt0_NBchar, "cnt0", DispPID_NBchar, "creaPID",
    ...
    if (sTUIparam.sort_col > STREAM_SORT_NONE && sTUIparam.SORTING == 0)
    {
    ...
    if ((sTUIparam.SORTING == 2) || (sTUIparam.SORTING == 3))
    {
        long   *larray;
        double *varray;
        ...
        {
            for (long i = 0; i < sTUIparam.NBsindex; i++)
            {
            ...
            if (sTUIparam.SORTING == 3)
            {
                ...
                {
                    long si = sTUIparam.ssindex[i];
                    streaminfo[si].updatevalue_frozen +=
                        10000.0 * streaminfo[si].streamOpenPID_cnt1;
                }
            }
    ...
    // Clamp scroll margins for small terminals
    {
        ...
        while (sTUIparam.dindexSelected - doffsetindex > NBsinfodisp - 1 - margin_dn)
        {
            doffsetindex++;
        }

        while (sTUIparam.dindexSelected < doffsetindex + margin_up)
        {
            doffsetindex--;
        }
    }
    ...
    if (sTUIparam.dindexSelected >= doffsetindex + NBsinfodisp)
    {
