...
{
    if (starts_with(p, "pushd ") || starts_with(p, "pushd\t"))
    {
        p += 5;
        p = strip_ws(p);
        char cwd[1024];
        if (getcwd(cwd, sizeof(cwd)) != NULL)
        {
            ...
            {
                if (cli_vars[k].used && strncmp(cli_vars[k].name, "_ds_", 4) == 0)
                {
                    dcnt++;
                }
            }
...
int cli_intercept_cmd_seq(const char *p)
{
    if (starts_with(p, "seq "))
    {
        ...
        if (step > 0.0)
        {
            for (double v = s1; v <= s2 + 1e-12; v += step)
            {
                printf("%g\n", v);
            }
        }
...
int cli_intercept_cmd_waitfor_stream(const char *p)
{
    if (starts_with(p, "waitfor_stream "))
    {
        ...
        int  si = 0;
        while (*p != '\0' && *p != ' ' && *p != '\t' && si < CLI_VAR_NAMELEN - 1)
        {
            sname[si++] = *p++;
        }
        sname[si]   = '\0';
        ...
        while (1)
        {
            ...
            double elapsed = (double) (wnow.tv_sec - wstart.tv_sec) +
                             (double) (wnow.tv_nsec - wstart.tv_nsec) / 1e9;
            if (elapsed >= tout)
            {
                break;
            }
...
int cli_intercept_cmd_waitfor_fps(const char *p)
{
    if (starts_with(p, "waitfor_fps "))
    {
        ...
        int  fi = 0;
        while (*p != '\0' && *p != ' ' && *p != '\t' && fi < CLI_VAR_NAMELEN - 1)
        {
            fname[fi++] = *p++;
        }
        fname[fi]   = '\0';
