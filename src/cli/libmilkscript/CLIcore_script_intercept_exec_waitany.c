    ...
    int             nevents)
{
    ...
    for(int i = 0; i < nevents; i++)
    {
        ...
        {
            if(ImageStreamIO_read_sharedmem_image_toIMAGE(ev->name, &ev->img) == IMAGESTREAMIO_SUCCESS)
            {
    ...
    double          timeout)
{
    ...
    while(!cli_break_flag)
    {
        for(int i = 0; i < nevents; i++)
        {
            ...
            {
                char vstr[512];
                functionparameter_GetParamValueString(&ev->fps.parray[ev->fps_pindex], vstr, sizeof(vstr));
