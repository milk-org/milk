...
errno_t setSHMdir()
{
    ...
    else
    {
        ...
        {
            printf("%c[%d;%dm", (char) 27, 1, 31); // set color red
            printf("    WARNING: Environment variable MILK_SHM_DIR not "
                   "specified -> falling back to default %s\n",
                   SHAREDMEMDIR);
            printf("    BEWARE : Other milk users may be using the same "
                   "SHM directory on this machine, and could see "
                   "your milk session data and temporary files\n");
            printf("    BEWARE : Some scripts may rely on MILK_SHM_DIR to "
                   "find/access shared memory and temporary "
                   "files, and WILL not run.\n");
            printf("             Please set MILK_SHM_DIR and restart CLI "
                   "to set up user-specific shared memory and "
                   "temporary files\n");
            printf("             Example: Add \"export "
    ...
    if (shmdirOK == 0)
    {
        ...
        else
        {
            ...
            if (dcquiet == 0)
            {
                ...
                printf("        NOTE: Consider creating tmpfs "
                       "directory and setting env var MILK_SHM_DIR "
                       "for improved "
                       "performance :\n");
                printf("        $ echo \"tmpfs %s tmpfs "
