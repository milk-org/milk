...
errno_t termview_screen(const char *imagename, termview_options_t options)
{
    ...
    tv_cell_t *screen = NULL;
    tv_cell_t *prev_screen = NULL;
    int screen_size = 0;
    ...
    uint64_t last_cnt0_for_fps = img.md[0].cnt0;
    double current_stream_fps = 0.0;
    
    struct timespec last_time_real, last_time_cpu;

    clock_gettime(CLOCK_MONOTONIC, &last_time_real);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &last_time_cpu);
    
    struct timespec last_render_real = last_time_real;

    int popup_type = 0; // 0: none, 1: cmap, 2: scale
    struct timespec popup_expiry_time = {0, 0};

    int mouse_is_dragging = 0;
    int roi_is_dragging = 0;
    int last_mouse_x = 0;
    int last_mouse_y = 0;
    int roi_start_x = 0, roi_start_y = 0;
    int roi_end_x = 0, roi_end_y = 0;
    int input_mode = 0;
    char input_buf[64] = "";
    int input_len = 0;
    ...
    while(loop) {
        ...
        long elapsed_since_last = (now_real.tv_sec - last_render_real.tv_sec) * 1000000L + 
                                  (now_real.tv_nsec - last_render_real.tv_nsec) / 1000L;
        long timeout_us = target_us - elapsed_since_last;
        if (timeout_us < 0) timeout_us = 0;

        if (popup_type > 0) {
            struct timespec now;
            clock_gettime(CLOCK_MONOTONIC, &now);
            long popup_remain_us = (popup_expiry_time.tv_sec - now.tv_sec) * 1000000L + 
                                   (popup_expiry_time.tv_nsec - now.tv_nsec) / 1000L;
            if (popup_remain_us <= 0) {
                popup_type = 0;
                force_redraw = 1;
            } else if (popup_remain_us < timeout_us) {
                timeout_us = popup_remain_us;
            }
        }

        // Process keys until timeout, or process all pending if timeout is 0
        int ch;
        int first_wait = 1;
        while (1) {
            long wait_us = first_wait ? timeout_us : 0;
            ch = tv_read_key(wait_us);
            first_wait = 0;
            if (ch == -1) break;

            if (input_mode > 0) {
                if (ch == 10 || ch == 13 || ch == 27) { // ENTER or ESC
                    if (ch != 27 && input_len > 0) {
                        input_buf[input_len] = '\0';
                        if (input_mode == 1) {
                            current_options.manual_min = atof(input_buf);
                            current_options.range_locked = 1;
                        } else if (input_mode == 2) {
                            current_options.manual_max = atof(input_buf);
                            current_options.range_locked = 1;
                        }
                    }
                    input_mode = 0;
                ...
                } else if (ch >= 32 && ch <= 126 && input_len < 63) {
            ...
            switch(ch) {
                ...
                case 'c': force_redraw = 1; 
                    ...
                    popup_type = 1;
                    clock_gettime(CLOCK_MONOTONIC, &popup_expiry_time);
                    popup_expiry_time.tv_sec += 2;
                    break;
                ...
                case 'l': force_redraw = 1;
                    ...
                    break;
                case 'm': force_redraw = 1; input_mode = 1; input_len = 0; input_buf[0] = '\0'; break;
                case 'M': force_redraw = 1; input_mode = 2; input_len = 0; input_buf[0] = '\0'; break;
                ...
                case '.': force_redraw = 1;
                    ...
                    break;
                case TV_KEY_MOUSE: { force_redraw = 1;
                    ...
                    if (last_mouse_event.button == 64) {
                        ...
                        if (last_mouse_event.is_press && !last_mouse_event.is_drag) {
                            ...
                            last_mouse_x = mx;
                            last_mouse_y = my;
                        } else if (last_mouse_event.is_press && last_mouse_event.is_drag && mouse_is_dragging) {
                        ...
                        if (last_mouse_event.is_press && !last_mouse_event.is_drag) {
                            ...
                            roi_end_x = mx;
                            roi_end_y = my;
                        } else if (last_mouse_event.is_press && last_mouse_event.is_drag && roi_is_dragging) {
                            roi_end_x = mx;
                            roi_end_y = my;
                        } else if (!last_mouse_event.is_press && roi_is_dragging) {
                            roi_end_x = mx;
                            roi_end_y = my;
                            roi_is_dragging = 0;
                            
                            int min_x = (roi_start_x < roi_end_x) ? roi_start_x : roi_end_x;
                            int max_x = (roi_start_x > roi_end_x) ? roi_start_x : roi_end_x;
                            int min_y = (roi_start_y < roi_end_y) ? roi_start_y : roi_end_y;
                            int max_y = (roi_start_y > roi_end_y) ? roi_start_y : roi_end_y;
        ...
        clock_gettime(CLOCK_MONOTONIC, &last_render_real);
        
        double real_diff = (last_render_real.tv_sec - last_time_real.tv_sec) + 
                           (last_render_real.tv_nsec - last_time_real.tv_nsec) / 1e9;
        if (real_diff >= 1.0) {
            struct timespec now_cpu;
            clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &now_cpu);
            double cpu_diff = (now_cpu.tv_sec - last_time_cpu.tv_sec) + 
                              (now_cpu.tv_nsec - last_time_cpu.tv_nsec) / 1e9;
            current_cpu_usage = (cpu_diff / real_diff) * 100.0;
            
            uint64_t current_cnt0 = img.md[0].cnt0;
            current_stream_fps = (current_cnt0 - last_cnt0_for_fps) / real_diff;
            last_cnt0_for_fps = current_cnt0;
            
            last_time_real = last_render_real;
            last_time_cpu = now_cpu;
        }

        
        // Skip rendering if no UI changes and no new frame
        if (!force_redraw && img.md[0].cnt0 == last_cnt0) {
            continue;
        }
        last_cnt0 = img.md[0].cnt0;
        force_redraw = 0;
        ...
        if (wrow * wcol > screen_size) {
            screen_size = wrow * wcol;
            screen = (tv_cell_t*)realloc(screen, screen_size * sizeof(tv_cell_t));
            prev_screen = (tv_cell_t*)realloc(prev_screen, screen_size * sizeof(tv_cell_t));
            memset(prev_screen, 0, screen_size * sizeof(tv_cell_t)); // force initial redraw
        }
        // clear screen to spaces with black bg
        for (int i = 0; i < wrow * wcol; i++) {
            screen[i].bg_r = 0; screen[i].bg_g = 0; screen[i].bg_b = 0;
            screen[i].fg_r = 255; screen[i].fg_g = 255; screen[i].fg_b = 255;
            screen[i].ch[0] = ' '; screen[i].ch[1] = 0;
        }
        ...
        // Half-block mode gives 2 image rows per character row
        int disp_img_rows = disp_char_rows * 2;

        if (disp_img_rows * disp_cols > buffer_size) {
            buffer_size = disp_img_rows * disp_cols;
            display_buffer = (double*)realloc(display_buffer, buffer_size * sizeof(double));
        }

        size_t needed_fb = tv_framebuf_size(wrow, wcol);
        if (needed_fb > frame_buffer_size) {
            frame_buffer_size = needed_fb;
            frame_buffer = (char*)realloc(frame_buffer, frame_buffer_size);
        }

        // Aspect ratio preserving mapping
        double view_w_img = (double)xsize / view_zoom;
        double view_h_img = (double)ysize / view_zoom;
        
        // Use the larger step to ensure the image fits within the view
        double step = fmax(view_w_img / disp_cols, view_h_img / disp_img_rows);
        ...
        if (img.md[0].datatype == _DATATYPE_FLOAT) {
            ...
            for (int i = 0; i < disp_img_rows; i++) {
                for (int j = 0; j < disp_cols; j++) {
                    ...
                    if (img_x >= 0 && img_x < (int)xsize && img_y >= 0 && img_y < (int)ysize) {
                        display_buffer[buf_idx++] = (double)data[img_y * xsize + img_x];
                    } else {
                        display_buffer[buf_idx++] = 0.0;
                    }
                }
            }
            ...
            for (int i = 0; i < disp_img_rows; i++) {
                for (int j = 0; j < disp_cols; j++) {
                    ...
                    int img_x = (int)(center_img_x + sign_x * (j - center_disp_x) * step);
                    if (img_x >= 0 && img_x < (int)xsize && img_y >= 0 && img_y < (int)ysize) {
                        display_buffer[buf_idx++] = get_pixel_value(&img, img_x, img_y);
                    } else {
                        display_buffer[buf_idx++] = 0.0;
                    }
                }
      ...
      ((int)(center_img_y + sign_y * ((yy) - center_disp_y) * step) < (int)ysize) )
        ...
        for (int i = 0; i < disp_char_rows; i++) {
            for (int j = 0; j < disp_cols; j++) {
                ...
                tv_cell_t *cell = &screen[i * wcol + j];

                // Check if the current whole block (top+bot) and its immediate neighbors are all out of bounds.
                // If so, we can skip it. But we must allow border drawing if it's adjacent to the image.
                ...
                double val_top = in_bounds_top ? display_buffer[(i*2)*disp_cols + j] : 0.0;
                double val_bot = in_bounds_bot ? display_buffer[(i*2+1)*disp_cols + j] : 0.0;
                ...
                if (roi_is_dragging) {
                    int rx_min = (roi_start_x < roi_end_x) ? roi_start_x : roi_end_x;
                    int rx_max = (roi_start_x > roi_end_x) ? roi_start_x : roi_end_x;
                    int ry_min = (roi_start_y < roi_end_y) ? roi_start_y : roi_end_y;
                    int ry_max = (roi_start_y > roi_end_y) ? roi_start_y : roi_end_y;
...
#undef IS_IN_BOUNDS
        ...
        for (int i = 0; i < disp_char_rows; i++) {
            ...
            tv_cell_t *c1 = &screen[i * wcol + bar_col_start - 1];
            tv_cell_t *c2 = &screen[i * wcol + bar_col_start];
            ...
            for(int k = 0; k < label_len; k++) {
                if (bar_col_start + 2 + k < wcol) {
                    tv_cell_t *lc = &screen[i * wcol + bar_col_start + 2 + k];
        ...
        len = snprintf(info, sizeof(info), "Image: %s [%d x %d] Type: %d  |  CPU: %5.1f%%  UI: %3d FPS  Stream: %5.1f Hz", 
                           img.md[0].name, xsize, ysize, img.md[0].datatype, current_cpu_usage, target_fps, current_stream_fps);
        for(int k=0; k<len && k<wcol; k++) { 
            tv_cell_t *c = &screen[info_row * wcol + k];
            c->ch[0] = info[k]; 
            c->ch[1] = '\0';
            // Highlight stream name (Starts at index 7: "Image: ")
            int name_len = strlen(img.md[0].name);
            if (k >= 7 && k < 7 + name_len) {
        ...
        for(int k=0; k<len && k<wcol; k++) { screen[(info_row+1) * wcol + k].ch[0] = info[k]; screen[(info_row+1) * wcol + k].ch[1] = '\0'; }

        if (input_mode > 0) {
            len = snprintf(info, sizeof(info), "Enter Manual %s: %s_", input_mode == 1 ? "Min" : "Max", input_buf);
            for(int k=0; k<len && k<wcol; k++) { 
                tv_cell_t *c = &screen[(info_row+2) * wcol + k];
                c->ch[0] = info[k]; c->ch[1] = '\0';
                c->fg_r = 255; c->fg_g = 255; c->fg_b = 0; // Yellow text
            }
            for(int k=len; k<wcol; k++) {
                screen[(info_row+2) * wcol + k].ch[0] = ' '; screen[(info_row+2) * wcol + k].ch[1] = '\0';
            }
        } else {
            char info1[128];
        ...
        if (popup_type > 0) {
            ...
            for (int i = 0; i < box_h; i++) {
                ...
                for (int j = 0; j < box_w; j++) {
                    ...
                    tv_cell_t *cell = &screen[r * wcol + c];
                    ...
                    if (i == 0) {
                        ...
                        if (popup_type == 1 && j >= 16 && j < box_w - 2) {
                            // Compute position 0.0 to 1.0 along the gradient bar
            ...
            for (int c = 0; c < wcol; c++) {
                ...
                if (memcmp(cell, prev, sizeof(tv_cell_t)) != 0) {
                    ...
                    if (cell->fg_r != last_fg_r || cell->fg_g != last_fg_g || cell->fg_b != last_fg_b) {
                        tv_fg(frame_buffer, &fb_pos, cell->fg_r, cell->fg_g, cell->fg_b);
        ...
        // reset at end so normal terminal isn't broken on crash
    ...
    if (prev_screen) free(prev_screen);
