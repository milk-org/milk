...
static errno_t streamCTRL_keyinput_process(int ch, streamCTRLarg_struct *streamCTRLdata)
{
...
errno_t streamCTRL_CTRLscreen()
{
    ...
    while (sTUIparam.loopOK == 1)
    {
        ...
        else
        {
            ...
            TUI_printfw("PIDmax = %d    Update frequ = %2d Hz  fscan=%5.2f Hz "
                        "( %5.2f Hz %5.2f %% busy ) ",
                        PIDmax, (int) (sTUIparam.frequ + 0.5), 1.0 / streaminfoproc.dtscan,
            ...
            if (sTUIparam.DisplayMode == DISPLAY_MODE_FUSER)
            {
                ...
                {
                    TUI_printfw("Last scan on  %02d:%02d:%02d  - Press "
                ...
                {
                    TUI_printfw("Last scan on  XX:XX:XX  - Press F6 "
...

            {
                ...
                TUI_printfw("%4d streams    Currently displaying %4d-%4d   "
                            "Selected %d  ID = %d  inode = %d",
                            sTUIparam.NBsindex, doffsetindex, lastindex, sTUIparam.dindexSelected,
                            ssIDselected, (int) inodeselected);
            }
            ...
            TUI_printfw("%*s  %-*s  %-*s  %*s   %*s %*s %*s %8s", 9, "inode", DispName_NBchar,
                        "name", DispSize_NBchar, "type", Dispcnt0_NBchar, "cnt0", DispPID_NBchar,
                        "creaPID", DispPID_NBchar, "ownPID", Dispfreq_NBchar, "   frequ ", "#sem");
            ...
            case DISPLAY_MODE_SPTRACE:
                TUI_printfw("     stream process traces:   \"(INODE "
                            "TYPE/SEM PID)>\"");
                TUI_newline();
                break;
            ...
            while (sTUIparam.dindexSelected - doffsetindex > NBsinfodisp - 5) // scroll down
            {
                doffsetindex++;
            }

            while (sTUIparam.dindexSelected - doffsetindex < NBsinfodisp - 10) // scroll up
            {
                doffsetindex--;
            }
                ...
                else
                {
                    ...
                    else
                    {
                        DEBUG_TRACEPOINT(
                            ...
                            streaminfo[sindex].sname, streaminfo[sindex].ID,
            ...
            else
            {
                TUI_printfw(" ");
                    ...
                    if ((sTUIparam.DisplayMode < DISPLAY_MODE_FUSER) && (DisplayFlag == 1))
                    {
                        ...
                        snprintf(string, stringlen, "%-*.*s ", DispSize_NBchar, DispSize_NBchar,
                                 str);
                        TUI_printfw(string);
                        ...
                        {
                            snprintf(string, stringlen, " %*.2f Hz", Dispfreq_NBchar,
                    ...
                    if (streamCTRLimages[streaminfo[sindex].ID].md != NULL)
                    {
                        ...
                        {
                            snprintf(string, stringlen, " %3d sems ",
                                     streamCTRLimages[ID].md[0].sem);
                            TUI_printfw(string);

                            int s;
                            int max_s =
                                sTUIparam.DISPLAY_ALL_SEMS ? streamCTRLimages[ID].md[0].sem : 3;
                            for (s = 0; s < max_s; s++)
                            {
                    ...
                    if (streamCTRLimages[streaminfo[sindex].ID].md != NULL)
                    {
                            ...
                            (DisplayFlag == 1)) // sem write PIDs
                        {
                            ...
                            if (sTUIparam.DisplayDetailLevel == 1)
                            {
                                ...
                                double *dtarray = (double *) malloc(
                                    sizeof(double) * (IMAGESTRUCT_FRAMEWRITEMDSIZE - 2));
                                     ...
                                     wioffset++)
                                {
                                    ...
                                    double tdouble = 1.0 * streamCTRLimages[ID]
                                                               .writehist[windex]
                                                               .writetime.tv_sec +
                                                     1.0e-9 * streamCTRLimages[ID]
                                                                  .writehist[windex]
                                                                  .writetime.tv_nsec;
                                    double deltat  = 0.0;
                                    ...
                                    {
                                        TUI_printfw(
                                            "%4d  cnt0 %8d  PID %6d  ts %9ld.%09ld   %.9f s ago  "
                                            "delta = %9.3f us\n",
                                ...
                                double tave =
                                    1.0e6 * deltatsum / (IMAGESTRUCT_FRAMEWRITEMDSIZE - 2);
                                TUI_printfw("AVERAGE =        %9.3f us\n", tave);

                                double trms = deltatsum2 - deltatsum * deltatsum /
                                ...
                                double p10us =
                                    1.0e6 *
                                    dtarray[(int) (0.1 * (IMAGESTRUCT_FRAMEWRITEMDSIZE - 2))];
                                TUI_printfw("  p10          : %9.3f us    %9.3f us\n", p10us,
                                            p10us - tave);

                                double p50us =
                                    1.0e6 * dtarray[(IMAGESTRUCT_FRAMEWRITEMDSIZE - 2) / 2];
                                TUI_printfw("  p50 (median) : %9.3f us    %9.3f us\n", p50us,
                                            p50us - tave);

                                double p90us =
                                    1.0e6 *
                                    dtarray[(int) (0.9 * (IMAGESTRUCT_FRAMEWRITEMDSIZE - 2))];
                    ...
                    if (streamCTRLimages[streaminfo[sindex].ID].md != NULL)
                    {
                        if ((sTUIparam.DisplayMode == DISPLAY_MODE_SPTRACE) && (DisplayFlag == 1))
                        {
                            ...
                            DEBUG_TRACEPOINT("NBproctrace = %d",
                        ...
                        if ((sTUIparam.DisplayMode == DISPLAY_MODE_SUMMARY) && (DisplayFlag == 1))
                        {
                            if (sTUIparam.DisplayDetailLevel == 1)
                            {
                                ...
                                TUI_printfw("md.atime            %10ld.%09d\n",
                                            ...
                                            streamCTRLimages[ID].md->atime.tv_nsec);
                        ...
                        (DisplayFlag == 1)) // list processes that are accessing streams
                    {
                        ...
                        case 1:
                            ...
                            for (pidIndex = 0; pidIndex < streaminfo[sindex].streamOpenPID_cnt;
                                 pidIndex++)
                            {
