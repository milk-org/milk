...
int PIDcollectSystemInfo(PROCESSINFODISP *pinfodisp, int level)
{
    ...
    for (int spindex = 0; spindex < pinfodisp->NBsubprocesses; spindex++)
    {
        ...
        unsigned long stat_minflt;  // (10) The number of minor faults the process has made
                                    // which have not required loading a memory page from disk
        unsigned long stat_cminflt; // (11) The number of minor faults that the
                                    // process's waited-for children have made
        unsigned long stat_majflt;  // (12) The number of major faults the process has made
        ...
        unsigned long stat_stime;   // (15) Amount of time that this process has been scheduled
        ...
        long          stat_rss;        // (24) Resident Set Size: number of pages the process has in
                                       // real memory
        unsigned long stat_rsslim;     // (25) Current soft limit in bytes on the rss of
                                       // the process
        ...
        unsigned long stat_startstack; // (28) The address of the start (i.e.,
                                       // bottom) of the stack
        unsigned long stat_kstkesp;    // (29) The current value of ESP (stack pointer), as found
        ...
        unsigned long stat_blocked;    // (32) The bitmap of blocked signals, displayed
                                       // as a decimal number.  Obsolete, because it
                                       // does not provide information on real-time
                                       // signals; use /proc/[pid]/status instead.
        unsigned long stat_sigignore;  // (33) The bitmap of ignored signals, displayed as a
                                       // decimal number.  Obsolete, because it does not
                                       // provide information on real-time signals; use
                                       // /proc/[pid]/status instead.
        unsigned long stat_sigcatch;   // (34) The bitmap of ignored signals, displayed as a
                                       // decimal number.  Obsolete, because it does not provide
                                       // information on real-time signals; use
                                       // /proc/[pid]/status instead.
        unsigned long stat_wchan;      // (35) This is the "channel" in which the process is
        ...
        unsigned long stat_cnswap;     // (37) Cumulative nswap for child processes (not
                                       // maintained)
        int          stat_exit_signal; // (38) Signal to be sent to parent when we die
        int          stat_processor;   // (39) CPU number last executed on
        unsigned int stat_rt_priority; // (40) Real-time scheduling priority, a number in the
        ...
        unsigned long long stat_delayacct_blkio_ticks; // (42) Aggregated block I/O delays,
                                                       // measured in clock ticks
        unsigned long stat_guest_time;                 // (43) Guest time of the process (time spent
                                                       // running a virtual CPU for a guest
                                                       // operating system), measured in clock ticks
        long stat_cguest_time;         // (44) Guest time of the process's children, measured
                                       // in clock ticks (divide by sysconf(_SC_CLK_TCK)).
        unsigned long stat_start_data; // (45) Address above which program initialized and
        ...
        Nfields = fscanf(fp,
                         ...
                         &stat_num_threads, // 20
                         &stat_itrealvalue, &stat_starttime, &stat_vsize, &stat_rss, &stat_rsslim,
                         &stat_startcode, &stat_endcode, &stat_startstack, &stat_kstkesp,
                         &stat_kstkeip, // 30
                         &stat_signal, &stat_blocked, &stat_sigignore, &stat_sigcatch, &stat_wchan,
                         &stat_nswap, &stat_cnswap, &stat_exit_signal, &stat_processor,
                         &stat_rt_priority, // 40
