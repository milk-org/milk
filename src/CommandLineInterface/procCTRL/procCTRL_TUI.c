        ...
        if (fw != NULL)
        {
            ...
            if (d != NULL)
            {
                ...
                while ((ent = readdir(d)) != NULL)
                {
                    ...
                    char cpufile[512];
                    snprintf(cpufile, sizeof(cpufile), "%s/%s/%s", cgroot, ent->d_name,
...
errno_t processinfo_CTRLscreen()
{
    ...
    */
    ...
    procinfoproc.pinfodisp =
        (PROCESSINFODISP *) malloc(sizeof(PROCESSINFODISP) * procinfoproc.NBpinfodisp);
    ...
    for (int pinfodispindex = 0; pinfodispindex < procinfoproc.NBpinfodisp; pinfodispindex++)
    {
        ...
        for (int spi = 0; spi < MAXNBSUBPROCESS; spi++)
        {
            ...
            procinfoproc.pinfodisp[pinfodispindex].sampletimearray_prev[spi] = 0.0;
            ...
            procinfoproc.pinfodisp[pinfodispindex].ctxtsw_voluntary_prev[spi]    = 0;
            procinfoproc.pinfodisp[pinfodispindex].ctxtsw_nonvoluntary_prev[spi] = 0;

            procinfoproc.pinfodisp[pinfodispindex].cpuloadcntarray[spi]                  = 0;
            procinfoproc.pinfodisp[pinfodispindex].cpuloadcntarray_prev[spi]             = 0;
            procinfoproc.pinfodisp[pinfodispindex].subprocCPUloadarray[spi]              = 0.0;
            procinfoproc.pinfodisp[pinfodispindex].subprocCPUloadarray_timeaveraged[spi] = 0.0;
    ...
    while (loopOK == 1)
    {
        ...
        if (freeze == 0)
        {
            ...
            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_HELP)
            {
                ...
                attroff(attrval);
                TUI_printfw("   control / resources / triggering / timing "
                            "screens");
                TUI_newline();
                ...
                attroff(attrval);
                TUI_printfw("   htop (F10 to exit) / iotop (q to exit) / "
                            "atop (q to exit)");
                TUI_newline();
                ...
                TUI_printfw("+ - f");
                attroff(attrval);
                TUI_printfw("    Increase/decrease display frequency, "
                            "(f)reeze display");
                TUI_newline();
                ...
                TUI_printfw("  r R");
                attroff(attrval);
                TUI_printfw("    Remove selected (r) or all (R) inactive "
                            "process(es) log");
                TUI_newline();
                ...
                TUI_printfw("SPACE; u");
                attroff(attrval);
                TUI_printfw("    Select this process; (u)nselect all "
                            "processes");
                TUI_newline();
                ...
                TUI_printfw("p c s");
                attroff(attrval);
                TUI_printfw("    (p)ause (toggle C0 - C1); (c)ompute "
                ...
                TUI_printfw("e T K I");
                attroff(attrval);
                TUI_printfw("    clean (e)xit; SIG(T)ERM; SIG(K)ILL; "
                            "SIG(I)NT");
                TUI_newline();
                ...
                TUI_printfw(" z Z");
                attroff(attrval);
                TUI_printfw("    zero this (z) or all (Z) selected "
                            "counter");
                TUI_newline();
                ...
                TUI_printfw(" > <");
                attroff(attrval);
                TUI_printfw("    Move to other CPU set / back to same CPU "
                            "set");
                TUI_newline();
            }
            else
            {
                ...
                TUI_printfw("[PID %d   SCAN TID %d]  %2d cpus   %2d "
                ...
                TUI_printfw("Display frequ = %2d Hz  [%ld] fscan=%5.2f Hz "
                            "( %5.2f Hz %5.2f %% busy )",
                ...
                if ((pindexSelected >= 0) && (pindexSelected < PROCESSINFOLISTSIZE))
                {
                    if (procinfoproc.pinfommapped[pindexSelected] == 1)
                    {
                        ...
                        TUI_printfw("Source Code: %s line %d "
                                    "(function %s)",
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_RESOURCES)
                {
                    ...
                    TUI_printfw(" %*.*s %*.*s %-*.*s    %-*.*s         "
                                "     ",
                                pstrlen_status, pstrlen_status, " ", pstrlen_pid, pstrlen_pid, " ",
                                pstrlen_pname, pstrlen_pname, " ", pstrlen_cset, pstrlen_cset, " ");
                    ...
                    // Print CPU LOAD
                    TUI_printfw(" %*.*s %*.*s %-*.*s PR %-*.*s  #T  "
                                "ctxsw   ",
                                pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid, pstrlen_pid,
                                "PID", pstrlen_pname, pstrlen_pname, "pname", pstrlen_cset,
                    ...
                    for (cpusocket = 0; cpusocket < procinfoproc.NBcpusocket; cpusocket++)
                    {
                        ...
                        for (cpu = 0; cpu < procinfoproc.NBcpus; cpu++)
                        {
                            ...
                            {
                                int vint =
                                    (int) (100.0 * procinfoproc.CPUload[procinfoproc.CPUids[cpu]]);
                                if (vint > 99)
                                {
                                    vint = 99;
                                }
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL)
                {
                    ...
                    TUI_newline();
                    TUI_newline();
                    TUI_printfw(" %*.*s %*.*s %-*.*s %-*.*s C# tstart  "
                                "     %-*.*s %-*.*s   %-*.*s   %-*.*s",
                                pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid, pstrlen_pid,
                                "PID", pstrlen_pname, pstrlen_pname, "pname", pstrlen_state,
                                pstrlen_state, "state", pstrlen_tmux, pstrlen_tmux, "tmux sess",
                                pstrlen_loopcnt, pstrlen_loopcnt, "loopcnt", pstrlen_descr,
                                pstrlen_descr, "Description", pstrlen_msg, pstrlen_msg, "Message");
                    TUI_newline();
                    TUI_newline();
                    DEBUG_TRACEPOINT(" ");
                }
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TRIGGER)
                {
                    ...
                    TUI_newline();
                    TUI_newline();
                    TUI_printfw("%*.*s %*.*s %-*.*s %*.*s %*.*s mode "
                                "sem %*.*s  %*.*s  %*.*s",
                                pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid, pstrlen_pid,
                                "PID", pstrlen_pname, pstrlen_pname, "pname", pstrlen_inode,
                                pstrlen_inode, "inode", pstrlen_trigstreamname,
                ...
                for (dispindex = 0; dispindex < dispindexMax; dispindex++)
                {
                    ...
                    if (dispindex < procinfoproc.NBpinfodisp)
                    {
                        ...
                        DEBUG_TRACEPOINT("procinfoproc.selectedarray["
                                         "pindex] = %d",
                                         procinfoproc.selectedarray[pindex]);

                        if (pinfolist->active[pindex] == 1)
                        {
                            snprintf(string, stringlen, "%-*.*s", pstrlen_status, pstrlen_status,
                                     "ACTIVE");
                            attron(COLOR_PAIR(2));
                        ...
                        DEBUG_TRACEPOINT("pinfolist->active[pindex] = "
                                         "%d",
                        ...
                        if (pinfolist->active[pindex] == 2)
                        {
                            ...
                            {
                            case 3: // clean exit
                                snprintf(string, stringlen, "%-*.*s", pstrlen_status,
                                         pstrlen_status, "STOPPED");
                                attron(COLOR_PAIR(3));
                                TUI_printfw("%s", string);
                        ...
                        if (pinfolist->active[pindex] != 0)
                        {
                            ...
                            snprintf(string, stringlen, " %-*.*d", pstrlen_pid, pstrlen_pid,
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL)
                            {
                                ...
                                case 0:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "INIT");
                                    break;
                                ...
                                case 1:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "RUN");
                                    break;
                                ...
                                case 2:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "PAUS");
                                    break;
                                ...
                                case 3:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "TERM");
                                    break;
                                ...
                                case 4:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "ERR");
                                    break;
                                ...
                                case 5:
                                    snprintf(string, stringlen,
                                             " %-*.*"
                                             "s",
                                             pstrlen_state, pstrlen_state, "OFF");
                                    break;
                                ...
                                // Create time
                                {
                                    struct tm *createtm;
                                    createtm =
                                        gmtime(&procinfoproc.pinfoarray[pindex]->createtime.tv_sec);

                                    snprintf(string, stringlen,
                                             " %02d:%02d:"
                                             "%02d.%03d",
                                             createtm->tm_hour, createtm->tm_min, createtm->tm_sec,
                                             (int) (0.000001 * (procinfoproc.pinfoarray[pindex]
                                                                    ->createtime.tv_nsec)));
                                }
                                TUI_printfw("%s", string);

                                snprintf(string, stringlen, " %-*.*s", pstrlen_tmux, pstrlen_tmux,
                                         procinfoproc.pinfoarray[pindex]->tmuxname);
                                TUI_printfw("%s", string);

                                snprintf(string, stringlen, " %- *.*ld", pstrlen_loopcnt,
                                         pstrlen_loopcnt,
                                         procinfoproc.pinfoarray[pindex]->loopcnt -
                                             procinfoproc.loopcntoffsetarray[pindex]);
                                ...
                                snprintf(string, stringlen, "%-*.*s", pstrlen_descr, pstrlen_descr,
                                ...
                                snprintf(string, stringlen, "%-*.*s", pstrlen_msg, pstrlen_msg,
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_RESOURCES)
                            {
                                ...
                                else
                                {
                                         ...
                                         spindex++)
                                    {
                                        ...
                                        snprintf(string, stringlen, " %2d",
                                                 procinfoproc.pinfodisp[pindex].rt_priority);
                                        TUI_printfw("%s", string);

                                        snprintf(string, stringlen, " %-*.*s", pstrlen_cset,
                                                 pstrlen_cset,
                                                 procinfoproc.pinfodisp[pindex].cpuset);
                                        TUI_printfw("%s", string);

                                        snprintf(string, stringlen, " %2dx ",
                                        ...
                                        snprintf(string, stringlen, " +%02ld +%02ld",
                                        ...
                                        attron(COLOR_PAIR(cpuColor));
                                        snprintf(string, stringlen, "%5.1f %6.2f",
                                                 procinfoproc.pinfodisp[pindex]
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TIMING)
                            {
                                ...
                                if (procinfoproc.pinfoarray[pindex]->MeasureTiming == 1)
                                {
                                    ...
                                    TUI_printfw(" %3d "
                                                ...
                                                procinfoproc.pinfoarray[pindex]->timingbuffercnt %
                                                    100);
            ...
            TUI_printfw("procCTRL display thread load : %5.3f %%  @ %7.2f Hz",
                        100.0 * tdiffvloopexec / tdiffvloop, 1.0 / tdiffvloop);
            TUI_newline();
