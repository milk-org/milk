...
PROCESSINFO *processinfo_setup(
    ...
    const char *descriptionstring, const char *msgstring, const char *functionname, const char *filename,
    int linenumber)
{
...
PROCESSINFO *processinfo_shm_create(const char *pname, int CTRLval)
{
    ...
    {
        int slen = snprintf(pinfo->logfilename, STRINGMAXLEN_PROCESSINFO_LOGFILENAME, "%s/proc.%s.%06d.%09ld.logfile",
...
int processinfo_cleanExit(PROCESSINFO *processinfo)
{
    ...
    if (processinfo->loopstat != 4)
    {
        ...
        {
            sprintf(msgstring, "CTRLexit %02d:%02d:%02d.%03d", tstoptm->tm_hour, tstoptm->tm_min, tstoptm->tm_sec,
        ...
        {
            sprintf(msgstring, "Loop exit %02d:%02d:%02d.%03d", tstoptm->tm_hour, tstoptm->tm_min, tstoptm->tm_sec,
                    (int)(0.000001 * (tstop.tv_nsec)));
            strncpy(processinfo->statusmsg, msgstring, STRINGMAXLEN_PROCESSINFO_STATUSMSG - 1);
        }
...
int processinfo_SIGexit(PROCESSINFO *processinfo, int SignalNumber)
{
    ...
    sprintf(timestring, "%02d:%02d:%02d.%03d", tstoptm->tm_hour, tstoptm->tm_min, tstoptm->tm_sec,
            (int)(0.000001 * (tstop.tv_nsec)));
    processinfo->loopstat = 3; // clean exit
...
int processinfo_WriteMessage(PROCESSINFO *processinfo, const char *msgstring)
{
    ...
    fprintf(processinfo->logFile, "%02d:%02d:%02d.%06d  %8ld.%09ld  %06d  %s\n", tmnow->tm_hour, tmnow->tm_min,
            tmnow->tm_sec, (int)(0.001 * (tnow.tv_nsec)), tnow.tv_sec, tnow.tv_nsec, (int)processinfo->PID, msgstring);
    ...
    if (data.image[outstreamID].md[0].shared == 1)
    {
        ...
        if (processinfo != NULL)
        {
            ...
            if (IDin > -1)
            {
                ...
                memcpy(&data.image[outstreamID].streamproctrace[1], &data.image[IDin].streamproctrace[0],
    ...
    if (processinfo->MeasureTiming == 1)
    {
        ...
        if (processinfo->dtiter_limit_enable != 0)
        {
            ...
            if (dtiter > processinfo->dtiter_limit_value)
            {
                ...
                {
                    int slen =
                        snprintf(msgstring, STRINGMAXLEN_PROCESSINFO_STATUSMSG, "dtiter %4ld  %4d %6.1f us  > %6.1f us",
                                 processinfo->dtiter_limit_cnt, processinfo->timerindex, 0.001 * dtiter,
    ...
    if (processinfo->MeasureTiming == 1)
    {
        ...
        if (processinfo->dtexec_limit_enable != 0)
        {
            ...
            if (dtexec > processinfo->dtexec_limit_value)
            {
                ...
                {
                    int slen =
                        snprintf(msgstring, STRINGMAXLEN_PROCESSINFO_STATUSMSG, "dtexec %4ld  %4d %6.1f us  > %6.1f us",
                                 processinfo->dtexec_limit_cnt, processinfo->timerindex, 0.001 * dtexec,
...
static int GetCPUloads(PROCINFOPROC *pinfop)
{
    ...
    while (((read = getline(&line, &maxstrlen, fp)) != -1) && (cpu < pinfop->NBcpus))
    {

        sscanf(line, "%s %lld %lld %lld %lld %lld %lld %lld %lld %lld", string0, &vall0, &vall1, &vall2, &vall3, &vall4,
               &vall5, &vall6, &vall7, &vall8);
    ...
    EXECUTE_SYSTEM_COMMAND("{ if [ ! -f %s/_psOKlock ]; then touch %s/_psOKlock; ps -e -o pid,psr,cpu,cmd > %s; fi; rm "
...
static int PIDcollectSystemInfo(PROCESSINFODISP *pinfodisp, int level)
{
    ...
    for (int spindex = 0; spindex < pinfodisp->NBsubprocesses; spindex++)
    {
        ...
        Nfields = fscanf(
            fp,
            "%d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %llu %lu %ld %lu %lu %lu %lu "
            "%lu %lu %lu %lu %lu %lu %lu %lu %lu %d %d %u %u %llu %lu %ld %lu %lu %lu %lu %lu %lu %lu %ld\n",
            &stat_pid, //  1
            stat_comm, &stat_state, &stat_ppid, &stat_pgrp, &stat_session, &stat_tty_nr, &stat_tpgid, &stat_flags,
            &stat_minflt, //  10
            &stat_cminflt, &stat_majflt, &stat_cmajflt, &stat_utime, &stat_stime, &stat_cutime, &stat_cstime,
            &stat_priority, &stat_nice,
            &stat_num_threads, // 20
            &stat_itrealvalue, &stat_starttime, &stat_vsize, &stat_rss, &stat_rsslim, &stat_startcode, &stat_endcode,
            &stat_startstack, &stat_kstkesp,
            &stat_kstkeip, // 30
            &stat_signal, &stat_blocked, &stat_sigignore, &stat_sigcatch, &stat_wchan, &stat_nswap, &stat_cnswap,
            &stat_exit_signal, &stat_processor,
            &stat_rt_priority, // 40
            &stat_policy, &stat_delayacct_blkio_ticks, &stat_guest_time, &stat_cguest_time, &stat_start_data,
...
int processinfo_CPUsets_List(STRINGLISTENTRY *CPUsetList)
{
    ...
    EXECUTE_SYSTEM_COMMAND("cset set -l | awk '/root/{stop=1} stop==1{print $0}' > %s", fname);
        ...
        if (pinfop->NBpindexActive > 0)
        {
            ...
            for (index = 0; index < pinfop->NBpindexActive; index++)
            {
                ...
                if (pinfop->pinfommapped[pindex] == 1)
                {
                    ...
                    timearray[index] = -1.0 * pinfop->pinfoarray[pindex]->createtime.tv_sec -
    ...
    while (loopOK == 1)
    {
        ...
        case '<': // move to same cpuset
            ...
            if (pinfolist->active[pindex] == 1)
            {
                ...
                EXECUTE_SYSTEM_COMMAND("sudo cset proc --force -m %d %s &> /dev/null", pinfolist->PIDarray[pindex],
                                       procinfoproc.pinfodisp[pindex].cpuset);
        ...
        case 'M': // toggle time limit (exec)
            ...
            if (ToggleValue == 0)
            {
                ...
                procinfoproc.pinfoarray[pindex]->dtexec_limit_value =
                    (long)(1.5 * procinfoproc.pinfoarray[pindex]->dtmedian_exec_ns + 20000);
        ...
        if (freeze == 0)
        {
            ...
            else
            {
                ...
                TUI_printfw("pindexSelected = %d    %d", pindexSelected, pindexSelectedOK);
                TUI_newline();

                TUI_printfw("[PID %d   SCAN TID %d]  %2d cpus   %2d processes tracked    Display Mode %d", CLIPID,
                            (int)procinfoproc.scanPID, procinfoproc.NBcpus, procinfoproc.NBpindexActive,
                ...
                TUI_printfw("Display frequ = %2d Hz  [%ld] fscan=%5.2f Hz ( %5.2f Hz %5.2f %% busy )",
                ...
                if ((pindexSelected >= 0) && (pindexSelected < PROCESSINFOLISTSIZE))
                {
                    if (procinfoproc.pinfommapped[pindexSelected] == 1)
                    {
                        ...
                        TUI_printfw("Source Code: %s line %d (function %s)", pselected_FILE, pselected_LINE,
                                    pselected_FUNCTION);
                        TUI_newline();
                    }
                    else
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_RESOURCES)
                {
                    ...
                    // List CPUs
                    TUI_printfw(" %*.*s %*.*s %-*.*s    %-*.*s              ", pstrlen_status, pstrlen_status, " ",
                                pstrlen_pid, pstrlen_pid, " ", pstrlen_pname, pstrlen_pname, " ", pstrlen_cset,
                                pstrlen_cset, " ");
                    ...
                    TUI_printfw(" <- %2d sockets %2d CPUs", procinfoproc.NBcpusocket, procinfoproc.NBcpus);
                    TUI_newline();
                    ...
                    // Print CPU LOAD
                    TUI_printfw(" %*.*s %*.*s %-*.*s PR %-*.*s  #T  ctxsw   ", pstrlen_status, pstrlen_status, "STATUS",
                                pstrlen_pid, pstrlen_pid, "PID", pstrlen_pname, pstrlen_pname, "pname", pstrlen_cset,
                                pstrlen_cset, "cset", procinfoproc.NBcpus);
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL)
                {
                    ...
                    TUI_newline();
                    TUI_newline();
                    TUI_printfw(" %*.*s %*.*s %-*.*s %-*.*s C# tstart       %-*.*s %-*.*s   %-*.*s   %-*.*s",
                                pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid, pstrlen_pid, "PID",
                                pstrlen_pname, pstrlen_pname, "pname", pstrlen_state, pstrlen_state, "state",
                                pstrlen_tmux, pstrlen_tmux, "tmux sess", pstrlen_loopcnt, pstrlen_loopcnt, "loopcnt",
                                pstrlen_descr, pstrlen_descr, "Description", pstrlen_msg, pstrlen_msg, "Message");
                    TUI_newline();
                    TUI_newline();
                    DEBUG_TRACEPOINT(" ");
                }
                ...
                if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TRIGGER)
                {
                    ...
                    TUI_newline();
                    TUI_newline();
                    TUI_printfw("%*.*s %*.*s %-*.*s %*.*s %*.*s mode sem %*.*s  %*.*s  %*.*s", pstrlen_status,
                                pstrlen_status, "STATUS", pstrlen_pid, pstrlen_pid, "PID", pstrlen_pname, pstrlen_pname,
                                "pname", pstrlen_inode, pstrlen_inode, "inode", pstrlen_trigstreamname,
                                pstrlen_trigstreamname, "stream", pstrlen_missedfr, pstrlen_missedfr, "miss",
                                pstrlen_missedfrc, pstrlen_missedfrc, "misscumul", pstrlen_tocnt, pstrlen_tocnt,
                                "timeouts");
                    TUI_newline();
                    TUI_newline();
                    DEBUG_TRACEPOINT(" ");
                ...
                for (dispindex = 0; dispindex < dispindexMax; dispindex++)
                {
                    ...
                    {
                        DEBUG_TRACEPOINT("%d %d   %ld %ld", dispindex, dispindexMax, pindex, procinfoproc.NBpinfodisp);
                        ...
                        DEBUG_TRACEPOINT("procinfoproc.selectedarray[pindex] = %d", procinfoproc.selectedarray[pindex]);
                        ...
                        if (pinfolist->active[pindex] != 0)
                        {
                            ...
                            sprintf(string, " %-*.*d", pstrlen_pid, pstrlen_pid, pinfolist->PIDarray[pindex]);
                            TUI_printfw("%s", string);
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL)
                            {
                                ...
                                sprintf(string, " %02d:%02d:%02d.%03d", procinfoproc.pinfodisp[pindex].createtime_hr,
                                ...
                                else
                                {
                                         ...
                                         spindex++)
                                    {
                                        ...
                                        if (spindex > 0)
                                        {
                                            ...
                                            sprintf(string, " %*.*s %-*.*d %-*.*s", pstrlen_status, pstrlen_status,
                                        ...
                                        sprintf(string, " +%02ld +%02ld",
                                                labs(procinfoproc.pinfodisp[pindex].ctxtsw_voluntary[spindex] -
                                                     procinfoproc.pinfodisp[pindex].ctxtsw_voluntary_prev[spindex]) %
                                                    100,
                                                labs(procinfoproc.pinfodisp[pindex].ctxtsw_nonvoluntary[spindex] -
                                                     procinfoproc.pinfodisp[pindex].ctxtsw_nonvoluntary_prev[spindex]) %
                                                    100);
                                        TUI_printfw("%s", string);
                                    ...
                                    {
                                        TUI_printfw("  ERROR: procinfoproc.pinfodisp[pindex].NBsubprocesses = %d",
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TRIGGER)
                            {
                                TUI_printfw("%*d ", pstrlen_inode, procinfoproc.pinfoarray[pindex]->triggerstreaminode);
                                ...
                                case PROCESSINFO_TRIGGERMODE_SEMAPHORE:
                                    TUI_printfw("%2d:sem%01d ", procinfoproc.pinfoarray[pindex]->triggermode,
                                ...
                                TUI_printfw("  %*llu ", pstrlen_missedfrc,
                                            procinfoproc.pinfoarray[pindex]->triggermissedframe_cumul);
                                TUI_printfw("  %*llu ", pstrlen_tocnt,
                            ...
                            if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TIMING)
                            {
                                ...
                                if (procinfoproc.pinfoarray[pindex]->MeasureTiming == 1)
                                {
                                    ...
                                    TUI_printfw(" %3d ..%02ld  ", procinfoproc.pinfoarray[pindex]->timerindex,
                                    ...
                                    for (tindex = 0; tindex < PROCESSINFO_NBtimer - 1; tindex++)
                                    {
                                        ...
                                        dtiter_array[tindex] =
                                            (procinfoproc.pinfoarray[pindex]->texecstart[ti1].tv_nsec -
                                             procinfoproc.pinfoarray[pindex]->texecstart[ti0].tv_nsec) +
                                            1000000000 * (procinfoproc.pinfoarray[pindex]->texecstart[ti1].tv_sec -
                                                          procinfoproc.pinfoarray[pindex]->texecstart[ti0].tv_sec);

                                        dtexec_array[tindex] =
                                            (procinfoproc.pinfoarray[pindex]->texecend[ti0].tv_nsec -
                                             procinfoproc.pinfoarray[pindex]->texecstart[ti0].tv_nsec) +
                                            1000000000 * (procinfoproc.pinfoarray[pindex]->texecend[ti0].tv_sec -
                                                          procinfoproc.pinfoarray[pindex]->texecstart[ti0].tv_sec);
                                    }
                                    ...
                                    TUI_printfw("  busy = %6.2f %%",
                                                100.0 * dtexec_array[(long)(0.5 * PROCESSINFO_NBtimer)] /
                                                    (dtiter_array[(long)(0.5 * PROCESSINFO_NBtimer)] + 1));
            ...
            TUI_newline();
            TUI_printfw("Loop time = %9.8f s  ( max rate = %7.2f Hz)", tdiffvloop, 1.0 / tdiffvloop);
            TUI_newline();

            refresh();
        }

        if ((data.signal_TERM == 1) || (data.signal_INT == 1) || (data.signal_ABRT == 1) || (data.signal_BUS == 1) ||
