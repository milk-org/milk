    ...
    if (keywnode[nodeSelected].leaf > 0) // If this is not a directory
        {
            ...
            while (mask)
                {
                    int digit =
                        fps[fpsindexSelected].parray[pindexSelected].fpflag &
                                mask
                            ? 1
                            : 0;
                    if (digit == 1)
            ...
            while (refresh_screen == 0) // wait for input
                {
                    ...
                    int fcnt = functionparameter_read_fpsCMD_fifo(
                        fpsCTRLvar.fpsCTRLfifofd,
                        fpsctrltasklist,
                        fpsctrlqueuelist);
                    ...
                    if (taskflag > 0) // task has been performed
                    ...
                    else
                        {
                            getchardt_us =
                                (int) (1.01 *
                                       getchardt_us); // gradually slow down
            ...
            if (fpsCTRLvar.run_display == 1)
                {
                    ...
                    if (fpsCTRLvar.fpsCTRL_DisplayMode == 2) // FPS content
                        {
                            ...
                            DEBUG_TRACEPOINT(
                                "Check that selected node is OK : %d",
                    ...
                    TUI_newline();
                     ...
                     TUI_newline();*/
                                    if (strlen(keywnode[fpsCTRLvar.nodeSelected]
                                                   .keywordfull) <
                                    ...
                                    int GUIlineMax =
                                        keywnode[fpsCTRLvar
                                            ...
                                            .NBchild;
                                    for (level = 0;
                                         level < fpsCTRLvar.currentlevel;
                                         level++)
                                        {
                                            DEBUG_TRACEPOINT(
                                                "update GUIlineMax, the "
                                                "maximum number of lines");
                                            if (keywnode[nodechain[level]]
                                                    .NBchild > GUIlineMax)
                                                {
                                                    GUIlineMax =
                                                        keywnode
                    	...
                    	*/
                                    ...
                                    while (fpsCTRLvar.GUIlineSelected
                                               [fpsCTRLvar.currentlevel] >
                                           keywnode[fpsCTRLvar
                                                        .directorynodeSelected]
                                                   .NBchild -
                                    ...
                                    for (
                                        int GUIline = 0; GUIline < GUIlineMax;
                    ...
                    if (fpsCTRLvar.fpsCTRL_DisplayMode ==
                        3) // Task scheduler status
                        {
                            struct timespec tnow;
                            ...
                            sort_evalarray = (double *) malloc(
                                sizeof(double) * NB_FPSCTRL_TASK_MAX);
                            ...
                            sort_indexarray = (long *) malloc(
                                sizeof(long) * NB_FPSCTRL_TASK_MAX);
                            ...
                            long sortcnt = 0;
                            for (int fpscmdindex = 0;
                                 fpscmdindex < NB_FPSCTRL_TASK_MAX;
                                 fpscmdindex++)
                                {
                                    if (fpsctrltasklist[fpscmdindex].status &
                                        FPSTASK_STATUS_SHOW)
                                        {
                                            sort_evalarray[sortcnt] =
                                                -1.0 *
                                                fpsctrltasklist[fpscmdindex]
                                                    .inputindex;
                                            sort_indexarray[sortcnt] =
                                                fpscmdindex;
                                            sortcnt++;
                                        }
                                }
                            ...
                            for (int sortindex = 0; sortindex < sortcnt;
                                 sortindex++)
                                {

                                    DEBUG_TRACEPOINT("iteration %d / %ld",
                                                     sortindex,
                                                     sortcnt);

                                    int fpscmdindex =
                                        sort_indexarray[sortindex];

                                    DEBUG_TRACEPOINT("fpscmdindex = %d",
                                                     fpscmdindex);
                                    ...
                                    if (sortindex < wrow - 8) // display
                                        {
                                            ...
                                            if (fpsctrltasklist[fpscmdindex]
                                                    .status &
