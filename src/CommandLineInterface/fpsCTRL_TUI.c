...
inline static void fpsCTRLscreen_print_DisplayMode_status(int fpsCTRL_DisplayMode, int NBfps)
{
    ...
    if (snprintf(monstring, stringmaxlen,
                 "[%d x %d] [PID %d] FUNCTION PARAMETER MONITOR: PRESS (x) TO STOP, (h) FOR HELP [%d FPS]", wrow, wcol,
                 (int)getpid(), NBfps) < 0)
    {
...
inline static void fpsCTRLscreen_print_nodeinfo(FUNCTION_PARAMETER_STRUCT *fps, KEYWORD_TREE_NODE *keywnode,
                                                int nodeSelected, int fpsindexSelected, int pindexSelected)
{

    DEBUG_TRACEPOINT("Selected node %d in fps %d", nodeSelected, keywnode[nodeSelected].fpsindex);
    ...
    TUI_printfw("    FPS call              : %s -> %s [", fps[keywnode[nodeSelected].fpsindex].md->callprogname,
                fps[keywnode[nodeSelected].fpsindex].md->callfuncname);

    for (int i = 0; i < fps[keywnode[nodeSelected].fpsindex].md->NBnameindex; i++)
    {
        TUI_printfw(" %s", fps[keywnode[nodeSelected].fpsindex].md->nameindexW[i]);
    }
    TUI_printfw(" ]");
    TUI_newline();

    TUI_printfw("    FPS source            : %s %d", fps[keywnode[nodeSelected].fpsindex].md->sourcefname,
                fps[keywnode[nodeSelected].fpsindex].md->sourceline);
    TUI_newline();

    TUI_printfw("   %d libs : ", fps[keywnode[nodeSelected].fpsindex].md->NBmodule);
    for (int m = 0; m < fps[keywnode[nodeSelected].fpsindex].md->NBmodule; m++)
    {
        TUI_printfw(" [%s]", fps[keywnode[nodeSelected].fpsindex].md->modulename[m]);
    }
    TUI_newline();
    ...
    TUI_printfw("    ( FPS output data directory : %s )  ( FPS input conf directory : %s)",
                fps[keywnode[nodeSelected].fpsindex].md->datadir, fps[keywnode[nodeSelected].fpsindex].md->confdir);
    TUI_newline();
...
inline static void fpsCTRLscreen_level0node_summary(FUNCTION_PARAMETER_STRUCT *fps, int fpsindex)
{
    pid_t pid;
...
errno_t functionparameter_CTRLscreen(uint32_t mode, char *fpsnamemask, char *fpsCTRLfifoname)
{
    ...
    fpsctrlqueuelist = (FPSCTRL_TASK_QUEUE *)malloc(sizeof(FPSCTRL_TASK_QUEUE) * NB_FPSCTRL_TASKQUEUE_MAX);
    ...
    functionparameter_scan_fps(fpsCTRLvar.mode, fpsCTRLvar.fpsnamemask, data.fpsarray, keywnode, &fpsCTRLvar.NBkwn,
    ...
    while (loopOK == 1)
    {
        ...
        while (refresh_screen == 0) // wait for input
        {
            ...
            int fcnt = functionparameter_read_fpsCMD_fifo(fpsCTRLvar.fpsCTRLfifofd, fpsctrltasklist, fpsctrlqueuelist);

            DEBUG_TRACEPOINT(" ");

            // execute next command in the queue
            int taskflag = function_parameter_process_fpsCMDarray(fpsctrltasklist, fpsctrlqueuelist, keywnode,
                                                                  &fpsCTRLvar, data.fpsarray);
        ...
        loopOK =
            fpsCTRL_TUI_process_user_key(ch, data.fpsarray, keywnode, fpsctrltasklist, fpsctrlqueuelist, &fpsCTRLvar);
        ...
        if (fpsCTRLvar.run_display == 1)
        {
            ...
            fpsCTRLscreen_print_DisplayMode_status(fpsCTRLvar.fpsCTRL_DisplayMode, fpsCTRLvar.NBfps);

            DEBUG_TRACEPOINT(" ");

            TUI_printfw("======== FPSCTRL info  ( screen refresh cnt %7ld  scan interval %7ld us)", loopcnt,
                        getchardt_us);
            TUI_newline();
            TUI_printfw(
                "    INPUT FIFO       :  %s (fd=%d)    fifocmdcnt = %ld   NBtaskLaunched = %d -> %d   [NB FPS = %d]",
            ...
            if (fpsCTRLvar.fpsCTRL_DisplayMode == 2) // FPS content
            {
                ...
                if (fpsCTRLvar.NBfps > 0)
                {
                    ...
                    TUI_newline();
                     ...
                     TUI_newline();*/
                    if (strlen(keywnode[fpsCTRLvar.nodeSelected].keywordfull) < 1) // if not OK, set to last valid entry
                    {
                        fpsCTRLvar.nodeSelected = 1;
                    ...
                    fpsCTRLscreen_print_nodeinfo(data.fpsarray, keywnode, fpsCTRLvar.nodeSelected,
                    ...
                    TUI_printfw("[level %d %d] ", fpsCTRLvar.currentlevel + 1, nodechain[fpsCTRLvar.currentlevel + 1]);

                    if (fpsCTRLvar.currentlevel > 0)
                    {
                        TUI_printfw("[level %d %d] ", fpsCTRLvar.currentlevel, nodechain[fpsCTRLvar.currentlevel]);
                    }
                    ...
                    TUI_printfw("[node %d] level = %d   [%d] NB child = %d", fpsCTRLvar.nodeSelected,
                                fpsCTRLvar.currentlevel, fpsCTRLvar.directorynodeSelected,
                                keywnode[fpsCTRLvar.directorynodeSelected].NBchild);

                    TUI_printfw("   fps %d", fpsCTRLvar.fpsindexSelected);

                    TUI_printfw("   pindex %d ", keywnode[fpsCTRLvar.nodeSelected].pindex);
                    ...
                    if (!(data.fpsarray[fpsCTRLvar.fpsindexSelected].parray[0].fpflag & FPFLAG_VISIBLE)) // if invisible
                    {
                            ...
                            if (child_index[level] < keywnode[fpsCTRLvar.directorynodeSelected].NBchild)
                            {
                                ...
                                if (keywnode[knodeindex].leaf == 0) // If this is a directory
                                {
                                    ...
                                    if (fpsCTRLvar.currentlevel == 0) // provide a status summary if at root
                                    {
                                        ...
                                        {
                                            screenprint_setcolor(4);
                                            TUI_printfw("[%02d]", data.fpsarray[fpsindex].md->conferrcnt);
                                            screenprint_unsetcolor(4);
                                        }
                                        if (data.fpsarray[fpsindex].md->conferrcnt == 0)
                                        {
                                    ...
                                    if (child_index[level + 1] < keywnode[fpsCTRLvar.directorynodeSelected].NBchild)
                                    {
                                        ...
                                        TUI_printfw("%-16s", keywnode[knodeindex].keyword[level - 1]);
                                        screenprint_unsetcolor(5);
                                ...
                                else // If this is a parameter
                                {
                                    ...
                                    if (data.fpsarray[fpsindex].parray[pindex].type == FPTYPE_FILENAME)
                                    {
                                        ...
                                        TUI_printfw("  %10s", data.fpsarray[fpsindex].parray[pindex].val.string[0]);
                                    ...
                                    TUI_printfw("    %s", data.fpsarray[fpsindex].parray[pindex].description);
            ...
            if (fpsCTRLvar.fpsCTRL_DisplayMode == 3) // Task scheduler status
            {
                ...
                for (int sortindex = 0; sortindex < sortcnt; sortindex++)
                {
                    ...
                    if (sortindex < wrow - 8) // display
                    {
                        ...
                        TUI_printfw("[Q:%02d P:%02d] %4d", fpsctrltasklist[fpscmdindex].queue,
                                    fpsctrlqueuelist[fpsctrltasklist[fpscmdindex].queue].priority, fpscmdindex);
