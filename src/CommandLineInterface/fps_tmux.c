...
errno_t functionparameter_FPS_tmux_init(FUNCTION_PARAMETER_STRUCT *fps)
{
    ...
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl \"bash\" C-m", fps->md->name);

    // confstart
    //
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf \"bash\" C-m", fps->md->name);
    ...
    // runstart
    //
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \"bash\" C-m", fps->md->name);
