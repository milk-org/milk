    ...
    if (argpreprocess == 1)
        {
            ...
            if (strcmp(data.cmdargtoken[1].val.string, "..triggersname") == 0)
                {
                    printf(
                        "Command %ld: updating triggerstreamname to value %s\n",
                        data.cmdindex,
                        data.cmdargtoken[2].val.string);
                    strcpy(
                        data.cmd[data.cmdindex].cmdsettings.triggerstreamname,
                        data.cmdargtoken[2].val.string);
                    data.FPS_CMDCODE = FPSCMDCODE_IGNORE;
                    return RETURN_SUCCESS;
                }
            ...
            if (strcmp(data.cmdargtoken[1].val.string, "..triggerdelay") == 0)
                {
                    ...
                    data.cmd[data.cmdindex].cmdsettings.triggerdelay.tv_sec =
                        x_sec;
                    data.cmd[data.cmdindex].cmdsettings.triggerdelay.tv_nsec =
                        x_nsec;
                    data.FPS_CMDCODE = FPSCMDCODE_IGNORE;
                    return RETURN_SUCCESS;
                }
            ...
            if (strcmp(data.cmdargtoken[1].val.string, "..triggertimeout") == 0)
                {
                    ...
                    data.cmd[data.cmdindex].cmdsettings.triggertimeout.tv_sec =
                        x_sec;
                    data.cmd[data.cmdindex].cmdsettings.triggertimeout.tv_nsec =
                        x_nsec;
                    data.FPS_CMDCODE = FPSCMDCODE_IGNORE;
                    return RETURN_SUCCESS;
                }
