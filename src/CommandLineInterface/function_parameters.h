...
#define FPFLAG_DEFAULT_INPUT                                                         \
    FPFLAG_ACTIVE | FPFLAG_USED | FPFLAG_VISIBLE | FPFLAG_WRITE | FPFLAG_WRITECONF | \
        FPFLAG_SAVEONCHANGE | FPFLAG_FEEDBACK | FPFLAG_WRITESTATUS
...
#define FPS_SETUP_INIT_SIZED(VARfpsname, VARCMDmode, VARNBparamMAX)                              \
    ...
    {                                                                                            \
        ...
        {                                                                                        \
            ...
            SNPRINTF_CHECK(msgstring, STRINGMAXLEN_FPS_LOGMSG, "LOGSTART %s %d %s %d",           \
                           (VARfpsname), (VARCMDmode), fps.md->sourcefname, fps.md->sourceline); \
...
#define FPS_CONNECT(VARfpsname, VARCMDmode)                                            \
    ...
    {                                                                                  \
        ...
        {                                                                              \
            printf("ERROR: fps \"%s\" does not exist -> running without "              \
                   "FPS interface\n",                                                  \
                   VARfpsname);                                                        \
...
#define FPS_EXECFUNCTION_STD                                                       \
    ...
    {                                                                              \
        FUNCTION_PARAMETER_STRUCT fps;                                             \
        snprintf(data.FPS_name, STRINGMAXLEN_FPS_NAME, "%s-%06ld", CLIcmddata.key, \
                 (long) getpid());                                                 \
        data.FPS_CMDCODE = FPSCMDCODE_FPSINIT;                                     \
        FPSCONFfunction();                                                         \
        function_parameter_struct_connect(data.FPS_name, &fps, FPSCONNECT_SIMPLE); \
