    ...
    if (loopINIT == 0)
        {
            ...
            if (fps->CMDmode &
                FPSCMDCODE_CONFSTART) // parameter configuration loop
                {
    ...
    if (fps->md->signal & FUNCTION_PARAMETER_STRUCT_SIGNAL_CONFRUN)
        {
            ...
            if ((getpgid(fps->md->confpid) >= 0) && (fps->md->confpid > 0))
                {
                    fps->md->status |=
                        FUNCTION_PARAMETER_STRUCT_STATUS_CONF; // running
                }
            else
                {
                    fps->md->status &=
                        ~FUNCTION_PARAMETER_STRUCT_STATUS_CONF; // not running
                }
            ...
            if ((getpgid(fps->md->runpid) >= 0) && (fps->md->runpid > 0))
                {
                    fps->md->status |=
                        FUNCTION_PARAMETER_STRUCT_STATUS_RUN; // running
                }
            else
                {
                    fps->md->status &=
                        ~FUNCTION_PARAMETER_STRUCT_STATUS_RUN; // not running
                }
