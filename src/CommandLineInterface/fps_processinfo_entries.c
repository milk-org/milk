...
errno_t fps_add_processinfo_entries(FUNCTION_PARAMETER_STRUCT *fps)
{
    ...
    // run time string
    function_parameter_add_entry(fps, ".conf.timestring", "runstart time string", FPTYPE_STRING, FPFLAG, "undef", NULL);

    // custom label
    function_parameter_add_entry(fps, ".conf.label", "custom label", FPTYPE_STRING, FPFLAG, "", NULL);

    // output directory where results are saved
    //
    //	char outdir[FPS_DIR_STRLENMAX];
    //	snprintf(outdir, FPS_DIR_STRLENMAX, "fps.%s", fps->md->name);
    function_parameter_add_entry(fps, ".conf.datadir", "data directory", FPTYPE_DIRNAME, FPFLAG, fps->md->datadir,
                                 NULL);
    ...
    function_parameter_add_entry(fps, ".conf.confdir", "conf directory", FPTYPE_DIRNAME, FPFLAG, fps->md->confdir,
                                 NULL);

    // Where results are archived
    //
    function_parameter_add_entry(fps, ".conf.archivedir", "archive directory", FPTYPE_DIRNAME, FPFLAG, NULL, NULL);

    // value = -1 indicates no RT priority
    long RTprio_default[4] = {fps->cmdset.RT_priority, -1, 49, 20};
    function_parameter_add_entry(fps, ".procinfo.RTprio", "RTprio", FPTYPE_INT64, FPFLAG, &RTprio_default, NULL);

    // cset
    function_parameter_add_entry(fps, ".procinfo.cset", "CPUs set", FPTYPE_STRING, FPFLAG, "system", NULL);

    // taskset
    function_parameter_add_entry(fps, ".procinfo.taskset", "CPUs mask", FPTYPE_STRING, FPFLAG, "1-127", NULL);
    ...
    function_parameter_add_entry(fps, ".procinfo.enabled", "procinfo mode", FPTYPE_ONOFF, FPFLAG, NULL,
                                 &fp_pinfoenabled);
    ...
    function_parameter_add_entry(fps, ".procinfo.loopcntMax", "max loop cnt", FPTYPE_INT64, FPFLAG, &loopcntMax_default,
                                 NULL);

    long triggermode_default[4] = {fps->cmdset.triggermode, -1, 10, 0};
    function_parameter_add_entry(fps, ".procinfo.triggermode", "trigger mode", FPTYPE_INT64, FPFLAG,
                                 &triggermode_default, NULL);

    function_parameter_add_entry(fps, ".procinfo.triggersname", "trigger stream name", FPTYPE_STREAMNAME, FPFLAG,
                                 fps->cmdset.triggerstreamname, NULL);

    // -1 : auto (recommended)
    long semindexrequested_default[4] = {fps->cmdset.semindexrequested, -1, 10, 0};
    function_parameter_add_entry(fps, ".procinfo.semindexrequested", "trigger requested semaphore index", FPTYPE_INT64,
                                 FPFLAG, &semindexrequested_default, NULL);

    struct timespec triggerdelay_default[2] = {fps->cmdset.triggerdelay, {0, 0}};
    function_parameter_add_entry(fps, ".procinfo.triggerdelay", "trigger delay", FPTYPE_TIMESPEC, FPFLAG,
                                 &triggerdelay_default, NULL);

    struct timespec triggertimeout_default[2] = {fps->cmdset.triggertimeout, {0, 0}};
    function_parameter_add_entry(fps, ".procinfo.triggertimeout", "trigger timeout", FPTYPE_TIMESPEC, FPFLAG,
                                 &triggertimeout_default, NULL);
