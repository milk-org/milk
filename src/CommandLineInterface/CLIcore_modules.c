...
errno_t load_module_shared(const char *__restrict modulename)
{
    ...
    {
        int slen = snprintf(libname, STRINGMAXLEN_MODULE_SOFILENAME, "%s/lib/lib%s.so", data.installdir, modulenameLC);
        if (slen < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
            abort(); // can't handle this error any other way
...
errno_t RegisterModule(const char *__restrict FileName, const char *__restrict PackageName,
                       const char *__restrict InfoString, int versionmajor, int versionminor, int versionpatch)
{
    DEBUG_TRACE_FSTART();
    ...
    strncpy(data.module[data.NBmodule].loadname, data.moduleloadname, STRINGMAXLEN_MODULE_LOADNAME - 1);
    strncpy(data.module[data.NBmodule].sofilename, data.modulesofilename, STRINGMAXLEN_MODULE_SOFILENAME - 1);

    //printf("--- libnameloaded : %s\n", libnameloaded);
    strncpy(data.module[data.NBmodule].sofilename, libnameloaded, STRINGMAXLEN_MODULE_SOFILENAME - 1);
    ...
    {
        OKmsg = 1;
        DEBUG_TRACEPOINT("  %02ld  Found unloaded shared object in ./libs/ -> LOADING %10s  module %40s", data.NBmodule,
...
// Legacy function
//
uint32_t RegisterCLIcommand(const char *__restrict CLIkey, const char *__restrict CLImodulesrc, errno_t (*CLIfptr)(),
                            const char *__restrict CLIinfo, const char *__restrict CLIsyntax,
                            const char *__restrict CLIexample, const char *__restrict CLICcall)
{
    DEBUG_TRACE_FSTART();
...
uint32_t RegisterCLIcmd(CLICMDDATA CLIcmddata, errno_t (*CLIfptr)())
{
    ...
    CLIhelp_make_cmdexamplestring(CLIcmddata.funcfpscliarg, CLIcmddata.nbarg, CLIcmddata.key, cmdexamplestring);
    ...
    if (CLIcmddata.nbarg > 0)
    {
        ...
        for (int argi = 0; argi < CLIcmddata.nbarg; argi++)
        {
            ...
            case CLIARG_FLOAT32:
                data.cmd[data.NBcmd].argdata[argi].val.f32 = atof(CLIcmddata.funcfpscliarg[argi].example);
                break;

            case CLIARG_FLOAT64:
                data.cmd[data.NBcmd].argdata[argi].val.f64 = atof(CLIcmddata.funcfpscliarg[argi].example);
                break;
            ...
            case CLIARG_INT32:
                data.cmd[data.NBcmd].argdata[argi].val.i32 = (int32_t)atol(CLIcmddata.funcfpscliarg[argi].example);
                break;

            case CLIARG_UINT32:
                data.cmd[data.NBcmd].argdata[argi].val.ui32 = (uint32_t)atol(CLIcmddata.funcfpscliarg[argi].example);
                break;

            case CLIARG_INT64:
                data.cmd[data.NBcmd].argdata[argi].val.i64 = (int64_t)atol(CLIcmddata.funcfpscliarg[argi].example);
                break;

            case CLIARG_UINT64:
                data.cmd[data.NBcmd].argdata[argi].val.ui64 = (uint64_t)atol(CLIcmddata.funcfpscliarg[argi].example);
                break;
