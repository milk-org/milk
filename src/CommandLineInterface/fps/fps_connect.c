                                       ...
                                       int                        fpsconnectmode)
{
    ...
    if (data.FPS_TIMESTAMP == 0)
    {
        ...
        case FPSCONNECT_CONF:
            snprintf(data.FPS_PROCESS_TYPE, STRINGMAXLEN_FPSPROCESSTYPE, "conf-%s", name);
            break;

        case FPSCONNECT_RUN:
            snprintf(data.FPS_PROCESS_TYPE, STRINGMAXLEN_FPSPROCESSTYPE, "run-%s", name);
            break;

        case FPSCONNECT_SIMPLE:
            snprintf(data.FPS_PROCESS_TYPE, STRINGMAXLEN_FPSPROCESSTYPE, "init-%s", name);
            break;
        }
    ...
    fps->md = (FUNCTION_PARAMETER_STRUCT_MD *) mmap(0, file_stat.st_size, PROT_READ | PROT_WRITE,
                                                    MAP_SHARED, SM_fd, 0);
    ...
    if (fpsconnectmode == FPSCONNECT_RUN)
    {
        ...
        //
        {
            ...
            if (pindex > -1)
            {
                ...
                if (snprintf(fps->parray[pindex].val.string[0], FUNCTION_PARAMETER_STRMAXLEN, "%s",
                             timestring) < 0)
                {
                    PRINT_ERROR("snprintf error");
                }
