...
errno_t functionparameter_FPS_tmux_kill(FUNCTION_PARAMETER_STRUCT *fps)
{
    ...
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl C-c 2> /dev/null", fps->md->name);
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl \"exit\" C-m 2> /dev/null", fps->md->name);

    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf C-c 2> /dev/null", fps->md->name);
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf \"exit\" C-m 2> /dev/null", fps->md->name);

    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run C-c 2> /dev/null", fps->md->name);
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \"exit\" C-m 2> /dev/null", fps->md->name);
...
errno_t functionparameter_FPS_tmux_init(FUNCTION_PARAMETER_STRUCT *fps)
{
    ...
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl \" cd %s\" C-m", fps->md->name,
                           fps->md->workdir);
    ...
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf \" bash\" C-m",
                           fps->md->name); // This spins a bash-in-bash.
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf \" cd %s\" C-m", fps->md->name,
                           fps->md->workdir);
    ...
    snprintf(functionstring, funcstring_maxlen,
             ...
             "}\n",
             fps->md->callprogname, fps->md->name, mloadstring, fps->md->callfuncname, argstring);

    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:conf \" %s\" C-m", fps->md->name, functionstring);

    // runstart
    //
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \" bash\" C-m",
                           fps->md->name); // This spins a bash-in-bash.
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \" cd %s\" C-m", fps->md->name,
                           fps->md->workdir);
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \" source ../fpstmuxenv\" C-m", fps->md->name);
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \" source fpstmuxenv\" C-m", fps->md->name);
    ...
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \"%s\" C-m", fps->md->name, functionstring);
