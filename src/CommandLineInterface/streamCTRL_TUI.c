        ...
        if (streaminfoproc->fuserUpdate == 1)
        {
            ...
            else
            {
                ...
                if (PReadMode == 0)
                {
                    // popen option
                    {
                        int slen = snprintf(command, STRINGMAXLEN_COMMAND, "/bin/fuser %s/%s.im.shm 2>/dev/null",
                ...
                else
                {
                    ...
                    {
                        int slen = snprintf(command, STRINGMAXLEN_COMMAND, "/bin/fuser %s/%s.im.shm 2>/dev/null > %s",
 ...
 *
 */
static int streamCTRL_print_procpid(int DispPID_NBchar, pid_t procpid, pid_t *upstreamproc, int NBupstreamproc,
                                    uint32_t mode)
{
...
static errno_t streamCTRL_print_SPTRACE_details(IMAGE *streamCTRLimages, imageID ID, pid_t *upstreamproc,
                                                int NBupstreamproc, uint32_t print_pid_mode)
{
    /*
       	int             triggermode;
    	pid_t           procwrite_PID;
    ...
    */
    ...
    TUI_newline();
    TUI_printfw("   %*s %*s %*s", Disp_inode_NBchar, "inode", Disp_sname_NBchar, "stream", Disp_cnt0_NBchar, "cnt0",
                Disp_PID_NBchar, "PID", Disp_type_NBchar, "type");
    TUI_newline();
    ...
    for (int spti = 0; spti < streamCTRLimages[ID].md[0].NBproctrace; spti++)
    {
        ...
        if (print_timing == 1)
        {
            TUI_printfw(" at %ld.%09ld s", streamCTRLimages[ID].streamproctrace[spti].ts_procstart.tv_sec,
...
errno_t streamCTRL_CTRLscreen()
{
    ...
    while (loopOK == 1)
    {
        ...
        //attron(A_BOLD);
        screenprint_setbold();
        sprintf(monstring, "[%d x %d] [PID %d] STREAM MONITOR: PRESS (x) TO STOP, (h) FOR HELP", wrow, wcol, getpid());
        ...
        else
        {
            ...
            TUI_printfw("PIDmax = %d    Update frequ = %2d Hz  fscan=%5.2f Hz ( %5.2f Hz %5.2f %% busy ) ", PIDmax,
                        (int)(frequ + 0.5), 1.0 / streaminfoproc.dtscan, 1000000.0 / streaminfoproc.twaitus,
            ...
            if (streaminfoproc.fuserUpdate == 1)
            {
                ...
                TUI_printfw("fuser scan ongoing  %4d  / %4d   ", streaminfoproc.sindexscan, NBsindex);
            ...
            TUI_printfw("%*s  %-*s  %-*s  %*s   %*s %*s %*s %8s", 9, "inode", DispName_NBchar, "name", DispSize_NBchar,
                        "type", Dispcnt0_NBchar, "cnt0", DispPID_NBchar, "creaPID", DispPID_NBchar, "ownPID",
                        Dispfreq_NBchar, "   frequ ", "#sem");
            ...
            for (dindex = 0; dindex < NBsindex; dindex++)
            {
                ...
                {
                    // print file inode
                    streamCTRL_print_inode(streamCTRLimages[ID].md[0].inode, upstreaminode, NBupstreaminode,
                                           downstreammin);
                    TUI_printfw(" ");
                }
                ...
                if (streamCTRLimages[streaminfo[sindex].ID].md != NULL)
                {
                    if ((DisplayMode == DISPLAY_MODE_SPTRACE) && (DisplayFlag == 1)) // sem read PIDs
                    {
                        ...
                        {
                            TUI_newline();
                            streamCTRL_print_SPTRACE_details(streamCTRLimages, ID, upstreamproc, NBupstreamproc,
                                                             PRINT_PID_DEFAULT);
                        }
                    }
                }
                    ...
                    case 1:
                        ...
                        for (pidIndex = 0; pidIndex < streaminfo[sindex].streamOpenPID_cnt; pidIndex++)
                        {
                            ...
                            if ((getpgid(pid) >= 0) && (pid != getpid()))
                            {
                                ...
                                sprintf(string, ":%-*.*s", PIDnameStringLen, PIDnameStringLen, PIDname_array[pid]);
                                TUI_printfw(string);
