...
errno_t functionparameter_RUNstop(FUNCTION_PARAMETER_STRUCT *fps)
{
    ...
    //
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl \"cd %s\" C-m", fps->md->name, fps->md->workdir);

    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:ctrl \"fpsrunstop\" C-m", fps->md->name);

    // Send C-c in case runstop command is not implemented
    EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run C-c &> /dev/null", fps->md->name);
