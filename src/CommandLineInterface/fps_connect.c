...
long function_parameter_struct_connect(const char *name, FUNCTION_PARAMETER_STRUCT *fps, int fpsconnectmode)
{
    ...
    if ((fpsconnectmode == FPSCONNECT_CONF) || (fpsconnectmode == FPSCONNECT_RUN))
    {
        ...
        {
            if ((fps->parray[pindex].fpflag & FPFLAG_ACTIVE) && (fps->parray[pindex].fpflag & FPFLAG_USED) &&
    ...
    if (fpsconnectmode == FPSCONNECT_RUN)
    {
        ...
        //
        {
            ...
            if (pindex > -1)
            {
                ...
                if (snprintf(fps->parray[pindex].val.string[0], FUNCTION_PARAMETER_STRMAXLEN, "%s", timestring) < 0)
                {
                    PRINT_ERROR("snprintf error");
                }
            }
