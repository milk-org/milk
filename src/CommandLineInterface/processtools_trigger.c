                                           ...
                                           int          semindexrequested)
{
    ...
    {
        DEBUG_TRACEPOINT("trigger mode %d = cnt2 of ID %ld", PROCESSINFO_TRIGGERMODE_CNT2, trigID);
    ...
    if (processinfo->triggermode == PROCESSINFO_TRIGGERMODE_SEMAPHORE)
    {
        ...
        processinfo->triggersem =
            ImageStreamIO_getsemwaitindex(&data.image[trigID], semindexrequested);
