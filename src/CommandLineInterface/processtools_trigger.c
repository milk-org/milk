 ...
 *
 */
errno_t processinfo_waitoninputstream_init(PROCESSINFO *processinfo, imageID trigID, int triggermode,
                                           int semindexrequested)
{
    ...
    {
        DEBUG_TRACEPOINT("trigger mode %d = semaphore %d on ID %ld", PROCESSINFO_TRIGGERMODE_SEMAPHORE,
                         semindexrequested, trigID);
...
errno_t processinfo_waitoninputstream(PROCESSINFO *processinfo)
{
    ...
    if (processinfo->triggermode == PROCESSINFO_TRIGGERMODE_CNT0)
    {
        ...
        processinfo->triggermissedframe =
            data.image[processinfo->triggerstreamID].md[0].cnt0 - processinfo->triggerstreamcnt - 1;
    ...
    if (processinfo->triggermode == PROCESSINFO_TRIGGERMODE_CNT1)
    {
        ...
        processinfo->triggermissedframe =
            data.image[processinfo->triggerstreamID].md[0].cnt1 - processinfo->triggerstreamcnt - 1;
        // update trigger counter
        processinfo->triggerstreamcnt = data.image[processinfo->triggerstreamID].md[0].cnt1;
