...
#define FUNC_CHECK_RETURN(errval)                                                                                      \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            char errmsg_funcretfailure[STRINGMAXLEN_FUNCERRORMSG];                                                     \
            int errormsg_slen =                                                                                        \
                snprintf(errmsg_funcretfailure, STRINGMAXLEN_FUNCERRORMSG, "[%d] %s", retcheckvalue, xstr(errval));    \
            if (errormsg_slen < 1)                                                                                     \
            {                                                                                                          \
                printf("snprintf in FUNC_RETURN_FAILURE: wrote <1 char");                                              \
                abort();                                                                                               \
            ...
            printf("\n");                                                                                              \
            printf("%c[%d;%dm > > > FCALLERR %c[%dm [ %s %s %d ]\n", (char)27, 1, 31, (char)27, 0, __FILE__, __func__, \
                   __LINE__);                                                                                          \
            printf("%c[%d;%dm ***** %c[%d;m [rval = %d] %s\n", (char)27, 1, 31, (char)27, 0, retcheckvalue,            \
                   xstr(errval));                                                                                      \
            printf("%c[%d;%dm ***** %c[%d;m -> Function %s returns RETURN_FAILURE\n", (char)27, 1, 31, (char)27, 0,    \
                   __func__);                                                                                          \
            DEBUG_TRACE_FEXIT();                                                                                       \
...
#define DEBUG_TRACEPOINT_PRINT(...)                                                                                    \
    ...
    {                                                                                                                  \
        ...
        printf("DEBUG MSG [%s %s  %d]: %s\n", data.testpoint.file, data.testpoint.func, data.testpoint.line,           \
               data.testpoint.msg);                                                                                    \
    } while (0)
#endif
