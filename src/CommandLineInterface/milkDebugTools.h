...
#define FUNC_RETURN_FAILURE(...)                                                                  \
    do                                                                                            \
    {                                                                                             \
        char errmsg_funcretfailure[STRINGMAXLEN_FUNCERRORMSG];                                    \
        int  errormsg_slen =                                                                      \
            snprintf(errmsg_funcretfailure, STRINGMAXLEN_FUNCERRORMSG, __VA_ARGS__);              \
        if (errormsg_slen < 1)                                                                    \
        {                                                                                         \
        ...
        DEBUG_TRACEPOINT("%c[%d;%dm FERR %c[%dm %s", (char) 27, 1, 31, (char) 27, 0,              \
                         errmsg_funcretfailure);                                                  \
        printf("\n");                                                                             \
        printf("%c[%d;%dm ERROR %c[%dm [ %s %s %d ]\n", (char) 27, 1, 31, (char) 27, 0, __FILE__, \
               __func__, __LINE__);                                                               \
        printf("%c[%d;%dm ***** %c[%d;m %s\n", (char) 27, 1, 31, (char) 27, 0,                    \
               data.testpoint.msg);                                                               \
        printf("%c[%d;%dm ***** %c[%d;m -> Function %s returns "                                  \
               "RETURN_FAILURE\n",                                                                \
...
//
#define FUNC_CHECK_RETURN(errval)                                                                 \
    do                                                                                            \
    {                                                                                             \
        ...
        {                                                                                         \
            char errmsg_funcretfailure[STRINGMAXLEN_FUNCERRORMSG];                                \
            int  errormsg_slen = snprintf(errmsg_funcretfailure, STRINGMAXLEN_FUNCERRORMSG,       \
                                          "[%d] %s", retcheckvalue, xstr(errval));                \
            if (errormsg_slen < 1)                                                                \
            {                                                                                     \
                printf("snprintf in FUNC_RETURN_FAILURE: wrote <1 "                               \
                       "char");                                                                   \
                abort();                                                                          \
            }                                                                                     \
            if (errormsg_slen >= STRINGMAXLEN_FUNCERRORMSG)                                       \
            {                                                                                     \
                printf("snprintf in FUNC_RETURN_FAILURE: string "                                 \
                       "truncation");                                                             \
                abort();                                                                          \
            }                                                                                     \
            DEBUG_TRACEPOINT("%c[%d;%dm FCALLERR %c[%dm %s", (char) 27, 1, 31, (char) 27, 0,      \
                             errmsg_funcretfailure);                                              \
            printf("\n");                                                                         \
            printf("%c[%d;%dm > > > FCALLERR %c[%dm [ %s %s %d ]\n", (char) 27, 1, 31, (char) 27, \
                   0, __FILE__, __func__, __LINE__);                                              \
            printf("%c[%d;%dm ***** %c[%d;m [rval = %d] %s\n", (char) 27, 1, 31, (char) 27, 0,    \
                   retcheckvalue, xstr(errval));                                                  \
            printf("%c[%d;%dm ***** %c[%d;m -> Function %s returns "                              \
                   "RETURN_FAILURE\n",                                                            \
                   (char) 27, 1, 31, (char) 27, 0, __func__);                                     \
            DEBUG_TRACE_FEXIT();                                                                  \
...
#define PRINT_ERROR(...)                                                                         \
    do                                                                                           \
    {                                                                                            \
        int print_error_slen =                                                                   \
            snprintf(data.testpoint.msg, STRINGMAXLEN_FUNCTIONARGS, __VA_ARGS__);                \
        ...
        printf("ERROR: %c[%d;%dm %s %c[%d;m\n", (char) 27, 1, 31, data.testpoint.msg, (char) 27, \
               0);                                                                               \
        ...
        print_error_slen =                                                                       \
            snprintf(data.testpoint.func, STRINGMAXLEN_FUNCTIONNAME, "%s", __func__);            \
        if (print_error_slen < 1)                                                                \
        {                                                                                        \
            printf("snprintf in PRINT_ERROR: wrote <1 char");                                    \
            abort();                                                                             \
...
#define PRINT_WARNING(...)                                                       \
    ...
    {                                                                            \
        ...
        fprintf(stderr,                                                          \
                "%c[%d;%dm WARNING [ FILE: %s   FUNCTION: %s  LINE: %d ] "       \
                " %c[%d;m\n",                                                    \
...
#else
#    define PRINT_DEBUGWARNING(...) PRINT_WARNING(__VA_ARGS__)
#endif
...
#else
#    define PRINT_DEBUGERROR(...) PRINT_ERROR(__VA_ARGS__)
#endif
...
#    define DEBUG_TRACE_FSTART(...)                                                   \
        ...
        {                                                                             \
            ...
            {                                                                         \
                strncpy(data.testpoint.funcstack[data.testpoint.funclevel], __func__, \
                        STRINGMAXLEN_FUNCSTAK_FUNCNAME - 1);                          \
...
#    define DEBUG_TRACE_FEXIT(...)                                                           \
        ...
        {                                                                                    \
            ...
            {                                                                                \
                ...
                {                                                                            \
                    PRINT_ERROR("function level mismatch - check source "                    \
                                "code");                                                     \
                    abort();                                                                 \
                }                                                                            \
            }                                                                                \
...
#define DEBUG_TRACEPOINTRAW(...)                                                             \
    ...
    {                                                                                        \
        ...
        {                                                                                    \
            memcpy(&data.testpointarray[data.testpointcnt], &data.testpoint,                 \
                   sizeof(CODETESTPOINT));                                                   \
            data.testpointcnt++;                                                             \
...
#define EXECUTE_SYSTEM_COMMAND_ERRCHECK(...)                                             \
    do                                                                                   \
    {                                                                                    \
        char syscommandstring[STRINGMAXLEN_COMMAND];                                     \
        int  slen = snprintf(syscommandstring, STRINGMAXLEN_COMMAND, __VA_ARGS__);       \
