...
static errno_t fpselem_statusprint_FPSNAME(int fpsindex, int pindex, int isVISIBLE)
{
    ...
    TUI_printfw(" %10s [%ld %ld %ld]", data.fpsarray[fpsindex].parray[pindex].val.string[0],
...
static errno_t fpsCTRLdisplay_FPSerrormsgs(FPSCTRL_PROCESS_VARS *fpsCTRLvar)
{
    ...
    else
    {
        ...
        {
            int pindex = data.fpsarray[fpsCTRLvar->fpsindexSelected].md->msgpindex[msgi];
            TUI_printfw("%-40s %s",
                    ...
                    keywnode[fpsCTRLvar->directorynodeSelected].NBchild)
                {
                    ...
                    if (keywnode[knodeindex].leaf == 0)
                    {
                        ...
                        if (fpsCTRLvar->currentlevel == 0)
                        {
                            ...
                            DEBUG_TRACEPOINT("fpsindex = %d", fpsindex);
                    ...
                    else // If this is a parameter
                    {
                        ...
                        TUI_printfw(" %-20s", data.fpsarray[fpsindex]
                                                  .parray[pindex]
                                                  .keyword[keywnode[knodeindex].keywordlevel - 1]);
                        ...
                        if (data.fpsarray[fpsindex].parray[pindex].type == FPTYPE_PID)
                        {
                            ...
                            TUI_printfw("  %10d",
                                        (int) data.fpsarray[fpsindex].parray[pindex].val.pid[0]);
