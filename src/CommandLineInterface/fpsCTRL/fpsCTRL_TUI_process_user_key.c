                                 ...
                                 FPSCTRL_PROCESS_VARS      *fpsCTRLvar)
{
        ...
        if (fps[fpsindex].parray[pindex].fpflag & FPFLAG_WRITESTATUS)
        {
            if (fps[fpsindex].parray[pindex].type == FPTYPE_ONOFF)
            {
                if (fps[fpsindex].parray[pindex].fpflag & FPFLAG_ONOFF) // ON -> OFF
                {
                    ...
                    functionparameter_outlog("SETVAL", "%s ONOFF OFF",
                ...
                else // OFF -> ON
                {
                    ...
                    functionparameter_outlog("SETVAL", "%s ONOFF ON",
                                             fps[fpsindex].parray[pindex].keywordfull);
                }
        ...
        for (int kwnindex = 0; kwnindex < fpsCTRLvar->NBkwn; kwnindex++)
        {
            ...
            if (keywnode[kwnindex].leaf == 0)
            {
                if (keywnode[kwnindex].keywordlevel == 1)
                {
                    ...
                    {
                        functionparameter_outlog("STATUS", "CONFPID ALIVE %s %ld",
                                                 keywnode[kwnindex].keywordfull, pid);
                    }
                    else // PID not active
                    {
                        ...
                        {
                            functionparameter_outlog("STATUS", "CONFPID CRASHED %s %ld",
                                                     keywnode[kwnindex].keywordfull, pid);
                        }
                        else
                        {
                            functionparameter_outlog("STATUS", "CONFPID STOPPED %s %ld",
                                                     keywnode[kwnindex].keywordfull, pid);
                        }
                    }
                    ...
                    if ((getpgid(pid) >= 0) && (pid > 0))
                    {
                        functionparameter_outlog("STATUS", "RUNPID ALIVE %s %ld",
                                                 keywnode[kwnindex].keywordfull, pid);
                    }
                    else // PID not active
                    {
                        ...
                        {
                            functionparameter_outlog("STATUS", "RUNPID CRASHED %s %ld",
                                                     keywnode[kwnindex].keywordfull, pid);
                        }
                        else
                        {
                            functionparameter_outlog("STATUS", "RUNPID STOPPED %s %ld",
                                                     keywnode[kwnindex].keywordfull, pid);
                        }
                    }
                }
    ...
    case 'L': // log all parameters
        ...
        for (int kwnindex = 0; kwnindex < fpsCTRLvar->NBkwn; kwnindex++)
        {
            ...
            if (keywnode[kwnindex].leaf == 1)
            {
                ...
                functionparameter_PrintParameter_ValueString(&fps[fpsindex].parray[pindex],
                                                             msgstring, STRINGMAXLEN_FPS_LOGMSG);
                functionparameter_outlog("STATUS", "%s", msgstring);
            }
        }
        break;
    ...
    case 'P': // process input command file
        ...
        functionparameter_FPSprocess_cmdfile("confscript", fps, keywnode, fpsctrlqueuelist,
                                             fpsCTRLvar);
