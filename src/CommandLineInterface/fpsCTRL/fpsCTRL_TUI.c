...
inline static void fpsCTRLscreen_print_help()
{
    ...
    print_help_entry("T / CRTL+t", "initialize (T)mux session / kill (t)mux session");
    print_help_entry("CRTL+a", "(a)ttach tmux session (then, CTRL+b, D to detach.)");
                                               ...
                                               FPSCTRL_PROCESS_VARS *fpsCTRLvar)
{
    ...
    for (int m = 0; m < data.fpsarray[keywnode[fpsCTRLvar->nodeSelected].fpsindex].md->NBmodule;
         m++)
    {
    ...
    snprintf(helpfunctionstring, 2000,
             ...
             data.fpsarray[keywnode[fpsCTRLvar->nodeSelected].fpsindex].md->name, mloadstring,
             data.fpsarray[keywnode[fpsCTRLvar->nodeSelected].fpsindex].md->callfuncname);
...
errno_t functionparameter_CTRLscreen(uint32_t mode, char *fpsnamemask, char *fpsCTRLfifoname)
{
    ...
    //
    FPSCTRL_TASK_ENTRY *fpsctrltasklist = calloc(NB_FPSCTRL_TASK_MAX, sizeof(*fpsctrltasklist));
    ...
    FPSCTRL_TASK_QUEUE *fpsctrlqueuelist =
        calloc(NB_FPSCTRL_TASKQUEUE_MAX, sizeof(*fpsctrlqueuelist));
    ...
    {
        long NBpindex = 0;
        functionparameter_scan_fps(fpsCTRLvar.mode, fpsCTRLvar.fpsnamemask, data.fpsarray, keywnode,
    ...
    while (loopOK == 1)
    {
        ...
        while (refresh_screen == 0) // wait for input
        {
            ...
            int fcnt = functionparameter_read_fpsCMD_fifo(fpsCTRLvar.fpsCTRLfifofd, fpsctrltasklist,
                                                          fpsctrlqueuelist);

            DEBUG_TRACEPOINT(" ");

            // execute next command in the queue
            int taskflag = function_parameter_process_fpsCMDarray(
                fpsctrltasklist, fpsctrlqueuelist, keywnode, &fpsCTRLvar, data.fpsarray);
        ...
        loopOK = fpsCTRL_TUI_process_user_key(ch, data.fpsarray, keywnode, fpsctrltasklist,
                                              fpsctrlqueuelist, &fpsCTRLvar);
        ...
        if (fpsCTRLvar.run_display == 1)
        {
            ...
            {
                TUI_printfw("======== FPSCTRL info  ( screen refresh cnt %7ld  "
            ...
            {
                fpsCTRL_scheduler_display(fpsctrltasklist, fpsctrlqueuelist, wrow,
