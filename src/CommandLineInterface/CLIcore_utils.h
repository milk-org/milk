...
#define CLICMD_FIELDS_FPSPROC                                                                                          \
    __FILE__, sizeof(farg) / sizeof(CLICMDARGDEF), farg, CLICMDFLAG_FPS | CLICMDFLAG_PROCINFO, NULL, NULL, NULL
...
#define FARG_INPUTIM(imkey)                                                                                            \
    {                                                                                                                  \
        CLIARG_STR, "." #imkey ".name", "input image", #imkey, CLIARG_VISIBLE_DEFAULT, (void **)&imkey.name            \
    }
...
#define FARG_OUTIM2D(imkey)                                                                                            \
    {CLIARG_STR, "." #imkey ".name", "output image", #imkey, CLIARG_VISIBLE_DEFAULT, (void **)&imkey.name, NULL},      \
        {CLIARG_LONG, "." #imkey ".xsize", "x size", "256", CLIARG_VISIBLE_DEFAULT, (void **)&imkey.xsize, NULL},      \
        {CLIARG_LONG, "." #imkey ".ysize", "y size", "256", CLIARG_VISIBLE_DEFAULT, (void **)&imkey.ysize, NULL},      \
        {CLIARG_LONG, "." #imkey ".shared", "shared flag", "0", CLIARG_HIDDEN_DEFAULT, (void **)&imkey.shared, NULL},  \
        {CLIARG_LONG, "." #imkey ".NBkw", "number keywords", "10", CLIARG_HIDDEN_DEFAULT, (void **)&imkey.NBkw, NULL}, \
    {                                                                                                                  \
        CLIARG_LONG, "." #imkey ".CBsize", "circ buffer size", "0", CLIARG_HIDDEN_DEFAULT, (void **)&imkey.CBsize,     \
            NULL                                                                                                       \
    }
...
#define INSERT_STD_FPSCONFfunction                                                                                     \
    ...
    {                                                                                                                  \
        ...
        STD_FARG_LINKfunction if (CLIcmddata.FPS_customCONFsetup != NULL) { CLIcmddata.FPS_customCONFsetup(); }        \
        FPS_CONFLOOP_START                                                                                             \
...
#define INSERT_STD_PROCINFO_COMPUTEFUNC_INIT                                                                           \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            processinfo = processinfo_setup(CLIcmddata.key, pinfodescr, "startup", __FUNCTION__, __FILE__, __LINE__);  \
        }                                                                                                              \
...
#define INSERT_STD_FPSCLIfunctions INSERT_STD_FPSCONFfunction INSERT_STD_FPSRUNfunction INSERT_STD_FPSCLIfunction
...
static inline imageID imcreatelikewiseIMGID(IMGID *target_img, IMGID *source_img)
{
    if (target_img->ID == -1)
    {
        ...
        create_image_ID(target_img->name, source_img->naxis, source_img->size, source_img->datatype, source_img->shared,
                        source_img->NBkw, source_img->CBsize, &target_img->ID);
        DEBUG_TRACEPOINT(" ");
 ...
 */
static inline imageID imcreateIMGID(IMGID *img) { return imcreatelikewiseIMGID(img, img); }
...
static inline imageID resolveIMGID(IMGID *img, int ERRMODE)
{
    ...
    if (img->ID == -1 || (img->createcnt != data.image[img->ID].createcnt) || (data.image[img->ID].used != 1))
    {
