...
errno_t memory_re_alloc()
{
   ...
   */
    ...
    if ((current_NBimage + NB_IMAGES_BUFFER) > data.NB_MAX_IMAGE)
    {
        ...
        if (data.image == NULL)
        {
            PRINT_ERROR("Reallocation of data.image has failed - exiting "
                        "program");
            return -1; //  exit(0);
        }
        if (data.Debug > 0)
   ...
   */
    ...
    if ((compute_nb_variable() + NB_VARIABLES_BUFFER) > data.NB_MAX_VARIABLE)
    {
        ...
        data.variable =
            (VARIABLE *) realloc(data.variable, sizeof(VARIABLE) * data.NB_MAX_VARIABLE);
        if (data.variable == NULL)
        {
            PRINT_ERROR("Reallocation of data.variable has failed - exiting "
                        "program");
            return -1; // exit(0);
        }
