...
static int CLI_checkarg0(int CLIargnum, uint32_t funcargtype, int errmsg)
{
    ...
    case CLIARG_FLOAT:   // to be deprecated
        ...
        case CLIARG_STR_NOT_IMG:
            ...
            if (IDv == -1) // if not a variable -> not OK
            {
                if (errmsg == 1)
                {
                    printf("arg %d is string (=\"%s\"), but "
                           "should be integer\n",
                           CLIargnum - 1, data.cmdargtoken[CLIargnum].val.string);
                }
                rval = 1;
            }
            else // if variable, read it -> OK
            {
                ...
                {
                case CLIARG_FLOAT64:
                    data.cmdargtoken[CLIargnum].val.numf = data.variable[IDv].value.f;
                    data.cmdargtoken[CLIargnum].type     = CLIARG_FLOAT64;
                    rval                                 = 0;
                    break;
                case CLIARG_INT64:
                    data.cmdargtoken[CLIargnum].val.numf = 1.0 * data.variable[IDv].value.l;
                    data.cmdargtoken[CLIargnum].type     = CLIARG_FLOAT64;
                    rval                                 = 0;
                    break;
                default:
        ...
        case CLIARG_IMG:
            if (errmsg == 1)
            {
                printf("  arg %d (image \"%s\") not a floating point "
                       "number\n",
                       CLIargnum - 1, data.cmdargtoken[CLIargnum].val.string);
            }
            rval = 1;
        ...
        case CLIARG_STR:
            if (errmsg == 1)
            {
                printf("  arg %d (command \"%s\") not a floating "
                       "point number\n",
                       CLIargnum - 1, data.cmdargtoken[CLIargnum].val.string);
            }
            rval = 1;
            ...
            else
            {
                ...
                else
                {
                    ...
                    case CLIARG_FLOAT64: // double
                        data.cmdargtoken[CLIargnum].val.numl = (long) (data.variable[IDv].value.f);
                        data.cmdargtoken[CLIargnum].type     = CLIARG_INT64;
                        rval                                 = 0;
                        break;
                    ...
                    case CLIARG_UINT64: // uint64
                        data.cmdargtoken[CLIargnum].val.numl = data.variable[IDv].value.l;
                        data.cmdargtoken[CLIargnum].type     = CLIARG_INT64;
                        rval                                 = 0;
                        break;
    ...
    if (rval == 2)
    {
        if (errmsg == 1)
        {
            printf("arg %d: wrong arg type 0x%X ->  0x%X  vs 0x%X\n", CLIargnum, funcargtype,
                   funcargtype & 0x0000FFFF, data.cmdargtoken[CLIargnum].type);
        }
        rval = 1;
    }
        ...
        if (!(data.cmd[data.cmdindex].argdata[argi].flag & CLICMDARG_FLAG_NOFPS))
        {
            ...
            case CLIARG_FLOAT32:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             ...
                                             FPTYPE_FLOAT32, FPFLAG_DEFAULT_INPUT, &tmpf, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_FLOAT64:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &tmplf, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_ONOFF: // default to INT64
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, &tmpvall, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_INT32:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_INT32, FPFLAG_DEFAULT_INPUT, &tmpi32, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_UINT32:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_UINT32, FPFLAG_DEFAULT_INPUT, &tmpui32, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_INT64:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &tmpi64, NULL);
                NBarg_processed++;
            }
            break;
            ...
            case CLIARG_UINT64:
            {
                ...
                function_parameter_add_entry(fps, data.cmd[data.cmdindex].argdata[argi].fpstag,
                                             data.cmd[data.cmdindex].argdata[argi].descr,
                                             FPTYPE_UINT64, FPFLAG_DEFAULT_INPUT, &tmpui64, NULL);
                NBarg_processed++;
            }
            break;
