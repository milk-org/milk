...
errno_t load_module_shared(const char *__restrict modulename)
{
    ...
    DEBUG_TRACEPOINT("Searching for shared object in directory [data.installdir]/lib : "
                     "%s/lib",
                     data.installdir);

    {
        int slen = snprintf(libname, STRINGMAXLEN_MODULE_SOFILENAME, "%s/lib/lib%s.so",
                            data.installdir, modulenameLC);
        if (slen < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
            abort(); // can't handle this error any other way
    ...
    strncpy(data.module[data.moduleindex].sofilename, data.modulesofilename,
            STRINGMAXLEN_MODULE_SOFILENAME - 1);

    strncpy(data.module[data.moduleindex].loadname, data.moduleloadname,
                            ...
                            const char *__restrict CLICcall)
{
    ...
    else
    {
        ...
        else
        {
            ...
            snprintf(data.cmd[data.NBcmd].key, STRINGMAXLEN_CMD_KEY, "%s.%s",
