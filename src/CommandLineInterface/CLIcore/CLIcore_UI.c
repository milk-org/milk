...
errno_t runCLI_prompt(char *promptstring, char *prompt)
{
    ...
    if (strlen(promptstring) > 0)
    {
        ...
        {
            snprintf(prompt, FPS_DIR_STRLENMAX, COLORHBOLDCYAN "%s > " COLORRESET, promptstring);
        }
        else
        {
            snprintf(prompt, FPS_DIR_STRLENMAX, COLORHBOLDCYAN "%s-%s > " COLORRESET, promptstring,
                     data.processname);
        }
    }
    else
    {
        snprintf(prompt, FPS_DIR_STRLENMAX, COLORHBOLDCYAN "%s > " COLORRESET, data.processname);
    }
    ...
    if (fp != NULL)
    {
        for (uint64_t i = 0; i < CODETESTPOINTARRAY_NBCNT; i++)
        {
            ...
            if (data.testpointarray[j].line != 0)
            {
                ...
                fprintf(fp, "T %6ld %s %-20s %6d %-20s  %s\n", index, timestring, lastword,
                        data.testpointarray[j].line, data.testpointarray[j].func,
