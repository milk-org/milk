                                 ...
                                 FPSCTRL_PROCESS_VARS *fpsCTRLvar)
{
    ...
    case 'e': // erase FPS
        ...
        fpsCTRLvar->fpsindexSelected = 0; // safeguard in case current selection disappears
        break;
    ...
    case KEY_NPAGE:
        fpsCTRLvar->direction = 1;
        fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel] += 10;
        while (fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel] > fpsCTRLvar->NBindex - 1)
        {
            fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel] = fpsCTRLvar->NBindex - 1;
        }
    ...
    case KEY_LEFT:
        ...
        {
            fpsCTRLvar->directorynodeSelected = keywnode[fpsCTRLvar->directorynodeSelected].parent_index;
    ...
    case KEY_RIGHT:
        if (keywnode[fpsCTRLvar->nodeSelected].leaf == 0) // this is a directory
        {
            if (keywnode[keywnode[fpsCTRLvar->directorynodeSelected]
                             .child[fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel]]]
                    .leaf == 0)
            {
                fpsCTRLvar->directorynodeSelected = keywnode[fpsCTRLvar->directorynodeSelected]
                                                        .child[fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel]];
                fpsCTRLvar->nodeSelected = fpsCTRLvar->directorynodeSelected;
            }
        }
    ...
    case 10: // enter key
        ...
        if (keywnode[fpsCTRLvar->nodeSelected].leaf == 1) // this is a leaf
        {
            ...
            functionparameter_UserInputSetParamValue(&fps[fpsCTRLvar->fpsindexSelected], fpsCTRLvar->pindexSelected);
    ...
    case ' ':
        ...
        if (fps[fpsindex].parray[pindex].type == FPTYPE_EXECFILENAME)
        {
            ...
            EXECUTE_SYSTEM_COMMAND("tmux send-keys -t %s:run \"%s %s/%s.fps\" C-m", fps[fpsindex].md->name,
                                   fps[fpsindex].parray[pindex].val.string[0], fps[fpsindex].md->datadir,
    ...
    case '<': // Load from confdir
        ...
        if (fpin != NULL)
        {
            ...
            while ((read = getline(&FPSline, &len, fpin)) != -1)
            {
                ...
                free(inputCopy);
                functionparameter_FPSprocess_cmdline(FPScmdline, fpsctrlqueuelist, keywnode, fpsCTRLvar, fps,
                                                     &taskstatus);
            }
            fclose(fpin);
        }
    ...
    case 'F': // process FIFO
        ...
        printf("Reading FIFO file \"%s\"  fd=%d\n", fpsCTRLvar->fpsCTRLfifoname, fpsCTRLvar->fpsCTRLfifofd);

        if (fpsCTRLvar->fpsCTRLfifofd > 0)
        {
            // int verbose = 1;
            functionparameter_read_fpsCMD_fifo(fpsCTRLvar->fpsCTRLfifofd, fpsctrltasklist, fpsctrlqueuelist);
        }
    ...
    case 'P': // process input command file
        ...
        functionparameter_FPSprocess_cmdfile("confscript", fps, keywnode, fpsctrlqueuelist, fpsCTRLvar);
