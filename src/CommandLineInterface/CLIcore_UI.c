...
static void *xmalloc(int size)
{
    ...
    if (!buf)
        {
            fprintf(stderr,
                    COLORRED "Error: Out of memory. Exiting.'n" COLORRESET);
            exit(1);
        }
            ...
            for (uint64_t i = 0; i < CODETESTPOINTARRAY_NBCNT; i++)
                {
                    ...
                    uint64_t index = data.testpointarray[j].loopcnt *
                    ...
                    if (data.testpointarray[j].line != 0)
                        {
                            char timestring[20];
                            mkUTtimestring_nanosec(timestring,
                                                   data.testpointarray[j].time);
                            ...
                            for (int level = 0;
                                 level < data.testpointarray[j].funclevel;
                                 level++)
                                {
                                    fprintf(
                                        fp,
            ...
            while ((tokengroup = strtok_r(rest, "\"", &rest)))
                {
                    ...
                    if (rawstringmode ==
                        0) // not in a raw string, process tokengroup
                        {
                        ...
                        {
                            strcpy(data.cmdargtoken[data.cmdNBarg].val.string,
                                   tokengroup);
