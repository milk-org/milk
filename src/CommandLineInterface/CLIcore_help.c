...
errno_t printInfo()
{
    ...
    printf("   version                     offset = %4zu bit  = %4zu byte     [%4zu byte]\n", 8 * offsetval0,
           offsetval0, offsetval - offsetval0);
...
int CLIhelp_make_argstring(CLICMDARGDEF fpscliarg[], int nbarg, char *outargstring)
{
    ...
    for (int arg = 0; arg < nbarg; arg++)
    {
        if (!(fpscliarg[arg].flag & CLICMDARG_FLAG_NOCLI))
        {
            ...
            if (arg == 0)
            {
                sprintf(tmpstr, "<%s [%s] ->(%s)>", fpscliarg[arg].descr, typestring, fpscliarg[arg].fpstag);
            }
            else
            {
                char tmpstr1[STRINGMAXLEN_CMD_SYNTAX];
                snprintf(tmpstr1, STRINGMAXLEN_CMD_SYNTAX - 1, " <%s [%s] ->(%s)>", fpscliarg[arg].descr, typestring,
...
errno_t help_command(const char *__restrict cmdkey)
{
    ...
    for (unsigned int cmdi = 0; cmdi < data.NBcmd; cmdi++)
    {
        ...
        {
            printf("\n");
            printf(COLORCMD "%s" COLORRESET " in %s [%s]\n\t" COLORINFO "%s\n" COLORRESET, data.cmd[cmdi].key,
                   data.cmd[cmdi].module, data.module[data.cmd[cmdi].moduleindex].shortname, data.cmd[cmdi].info);

            //printf("syntax     :    %s\n", data.cmd[cmdi].syntax);
            printf("\texample> %s\n", data.cmd[cmdi].example);
            printf("\tsrc: %s\n", data.cmd[cmdi].srcfile);

            int FPSsupport = checkFlag64(data.cmd[cmdi].cmdsettings.flags, CLICMDFLAG_FPS, "FPS support");
            if (FPSsupport == 1)
            {
                                ...
                                "processinfo support (..procinfo 0/1)") == 1)
                {
                    ...
                    printf("        triggermode        : %d ", data.cmd[cmdi].cmdsettings.triggermode);
...
errno_t command_info_search(const char *restrict searchstring)
{
    ...
    for (unsigned int cmdi = 0; cmdi < data.NBcmd; cmdi++)
    {
        ...
        if (strstr(data.cmd[cmdi].info, searchstring) != NULL)
        {
            foundsubstring = 1;
            matchsubstring = 1;
            printf("%c[%d;%dm%s%c[%dm in %s [%s]\n\t%c[%d;%dm%s%c[%dm\n", (char)27, 1, colorcodecmd, data.cmd[cmdi].key,
                   (char)27, 0, data.cmd[cmdi].module, data.module[data.cmd[cmdi].moduleindex].shortname, (char)27, 1,
        ...
        if (matchsubstring == 0)
        {
            ...
            if (!reti)
            {
                ...
                printf("%c[%d;%dm%s%c[%dm in %s [%s]\n\t%c[%d;%dm%s%c[%dm\n", (char)27, 1, colorcodecmd,
                       data.cmd[cmdi].key, (char)27, 0, data.cmd[cmdi].module,
    ...
    else
    {
        ...
        for (i = 0; i < data.NBmodule; i++)
        {
            printf("%2ld %10s \033[1m%32s\033[0m %10s %2d.%02d.%02d    %11s %8s  %s\n", i, data.module[i].shortname,
                   data.module[i].name, data.module[i].package, data.module[i].versionmajor,
                   data.module[i].versionminor, data.module[i].versionpatch, data.module[i].datestring,
