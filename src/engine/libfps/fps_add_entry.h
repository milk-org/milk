...
#define FPS_ADDPARAM_FLT32_IN(key, pname, pdescr, dflt)                        \
    ...
    {                                                                          \
        /*fp_##key =*/                                                         \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_FLOAT32,  \
                                     FPFLAG_DEFAULT_INPUT, (dflt), &fp_##key); \
        (void) fp_##key;                                                       \
    } while (0)
...
#define FPS_ADDPARAM_FLT64_IN(key, pname, pdescr, dflt)                        \
    ...
    do                                                                         \
    {                                                                          \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_FLOAT64,  \
                                     FPFLAG_DEFAULT_INPUT, (dflt), &fp_##key); \
        (void) fp_##key;                                                       \
    } while (0)
...
#define FPS_ADDPARAM_INT32_IN(key, pname, pdescr, dflt)                                           \
    ...
    do                                                                                            \
    {                                                                                             \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_INT32, FPFLAG_DEFAULT_INPUT, \
                                     (dflt), &fp_##key);                                          \
        (void) fp_##key;                                                                          \
    } while (0)
...
#define FPS_ADDPARAM_INT64_IN(key, pname, pdescr, dflt)                                           \
    ...
    do                                                                                            \
    {                                                                                             \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, \
                                     (dflt), &fp_##key);                                          \
        (void) fp_##key;                                                                          \
    } while (0)
...
#define FPS_ADDPARAM_FILENAME_IN(key, pname, pdescr, dflt)                            \
    ...
    do                                                                                \
    {                                                                                 \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_FILENAME,        \
                                     FPFLAG_DEFAULT_INPUT_STREAM, (dflt), &fp_##key); \
        (void) fp_##key;                                                              \
    } while (0)
...
#define FPS_ADDPARAM_STREAM_IN(key, pname, pdescr, dflt)                              \
    ...
    do                                                                                \
    {                                                                                 \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_STREAMNAME,      \
                                     FPFLAG_DEFAULT_INPUT_STREAM, (dflt), &fp_##key); \
        (void) fp_##key;                                                              \
    } while (0)
...
#define FPS_ADDPARAM_ONOFF(key, pname, pdescr, dflt)                                              \
    ...
    do                                                                                            \
    {                                                                                             \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, \
                                     (dflt), &fp_##key);                                          \
        (void) fp_##key;                                                                          \
    } while (0)
...
#define FPS_ADDPARAM_FLT32_OUT(key, pname, pdescr)                            \
    ...
    {                                                                         \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_FLOAT32, \
                                     FPFLAG_DEFAULT_OUTPUT, NULL, &fp_##key); \
        (void) fp_##key;                                                      \
    } while (0)
...
#define FPS_ADDPARAM_INT32_OUT(key, pname, pdescr)                                                 \
    ...
    do                                                                                             \
    {                                                                                              \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_INT64, FPFLAG_DEFAULT_OUTPUT, \
                                     NULL, &fp_##key);                                             \
        (void) fp_##key;                                                                           \
    } while (0)
...
#define FPS_ADDPARAM_STREAM_OUT(key, pname, pdescr)                                  \
    ...
    {                                                                                \
        function_parameter_add_entry(&fps, (pname), (pdescr), FPTYPE_STREAMNAME,     \
                                     FPFLAG_DEFAULT_OUTPUT_STREAM, NULL, &fp_##key); \
        (void) fp_##key;                                                             \
    } while (0)
