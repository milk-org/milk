...
imageID functionparameter_LoadStream(FPS *fps, int pindex, int fpsconnectmode)
{
    ...
    if (sp.error)
    {
        PRINT_ERROR("invalid stream modifier "
                    "prefix in \"%s\"",
                    rawname);
        return -1;
    }
    ...
    {
    case 'L':
        fps->parray[pindex].fpflag |= FPFLAG_STREAM_LOAD_FORCE_LOCALMEM;
        break;

    case 'S':
        fps->parray[pindex].fpflag |= FPFLAG_STREAM_LOAD_FORCE_SHAREMEM;
        break;
    ...
    if (sp.loc == 'L' && ID >= 0 && imLOC == STREAM_LOAD_SOURCE_SHAREMEM)
    {
        PRINT_ERROR("@L modifier -- "
                    "stream \"%s\" is in shared"
                    " memory, not local",
                    sp.name);
        return -1;
    }
    if (sp.loc == 'S' && ID >= 0 && imLOC == STREAM_LOAD_SOURCE_LOCALMEM)
    {
        PRINT_ERROR("@S modifier -- "
    ...
    if (conf_req && ID == -1)
    {
        if (name_empty)
        {
            fprintf(stderr,
                    ...
                    fps->parray[pindex].keywordfull, fps->md->name,
