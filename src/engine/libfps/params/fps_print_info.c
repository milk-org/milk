...
int function_parameter_print_info(FPS *fps, int verbose, int show_info)
{
    ...
    printf("%4s %-*s %12s %*s %8s %s\n", "CLI", kw_width, "Keyword", "Type", val_width, "Value",
           "Count", "Description");
    for (int ii = 0; ii < 4 + 1 + kw_width + 1 + 12 + 1 + val_width + 1 + 8 + 1 + 30; ii++)
    {
        printf("-");
    }
    printf("\n");
    ...
    for (int pindex = 0; pindex < fps->md->NBparamMAX; pindex++)
    {
        if (fps->parray[pindex].fpflag & FPFLAG_USED)
        {
            ...
            printf("%4s %s%-*s%s %12s %*s %8lu %s\n", cli_idx_str, color_start, kw_width,
                   display_keyword, color_end, type_str, val_width, valstring,
                   fps->parray[pindex].value_cnt, fps->parray[pindex].description);
            ...
            if (show_info && fps->parray[pindex].type == FPTYPE_STREAMNAME)
            {
                ...
                if (!sp.error)
                {
                    shm_ok = (ImageStreamIO_filename(shmpath_pi, sizeof(shmpath_pi), sp.name) ==
                              IMAGESTREAMIO_SUCCESS) &&
                             (access(shmpath_pi, F_OK) == 0);
                }

                if (shm_ok && ImageStreamIO_openIm(&tmpimg, sp.name) == IMAGESTREAMIO_SUCCESS)
                {
