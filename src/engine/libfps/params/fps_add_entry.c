                                     ...
                                     long       *pindexptr)
{
    ...
    if (scanOK == 0) // not found
    {
        ...
        if (pindex == NBparamMAX)
        {
            ...
            {
                FUNC_RETURN_FAILURE("NBparamMAX %ld limit reached and "
                                    "realloc failed",
                                    NBparamMAX);
            }
        }
        ...
        case FPTYPE_FILENAME:
            SNPRINTF_CHECK(funcparamarray[pindex].val.string[0], FUNCTION_PARAMETER_STRMAXLEN,
                           "NULLFNAME");
            SNPRINTF_CHECK(funcparamarray[pindex].val.string[1], FUNCTION_PARAMETER_STRMAXLEN,
                           "NULLFNAME");
            break;
