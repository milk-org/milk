 ...
 */
int functionparameter_FPS_tmux_kill(FPS *fps)
{
 ...
 * @return RETURN_SUCCESS
 */
int functionparameter_FPS_tmux_attach(FPS *fps)
{
 ...
 */
int functionparameter_FPS_tmux_init(FPS *fps)
{
    ...
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux new-session -s %s -d -n ctrl \\;"
                                   " new-window -n conf \\;"
                                   " new-window -n run \\;"
    ...
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:ctrl \" cd %s\" C-m", fps->md->name,
                                   fps->md->workdir);
    ...
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:conf \" bash\" C-m",
                                   fps->md->name); // This spins a bash-in-bash.
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:conf \" cd %s\" C-m", fps->md->name,
                                   fps->md->workdir);
    ...
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:conf \" %s\" C-m", fps->md->name,
                                   functionstring);
    ...
    EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:run \"%s\" C-m", fps->md->name,
                                   functionstring);
 ...
 */
int functionparameter_FPS_tmux_standalone_setup(const char *fps_name)
{
    char cmd[2048];
 ...
 */
int functionparameter_FPS_tmux_send(const char *fps_name, const char *window, const char *cmd_str)
{
 ...
 */
int functionparameter_FPS_tmux_send_dispatch(const char *fps_name,
                                             const char *command,
