...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-i, --interval SEC", mh_color ? MH_RST : "",
    ...
    const char *see_also[] = { "milk-fps-info:inspect FPS directory contents",
...
void print_ut_timestamp()
{
    ...
    printf("%04d%02d%02dT%02d:%02d:%02d.%03ld", ut_tm->tm_year + 1900, ut_tm->tm_mon + 1,
           ut_tm->tm_mday, ut_tm->tm_hour, ut_tm->tm_min, ut_tm->tm_sec, ts.tv_nsec / 1000000);
}
    ...
    while (keep_running)
    {
        ...
        for (int ii = 0; ii < NBfps; ii++)
        {
            ...
            if (track_idx == -1)
            {
                ...
                strncpy(track_list[track_idx].name, fpsarray[ii].md->name,
                        STRINGMAXLEN_FPS_NAME - 1);
                track_list[track_idx].NBparam = fpsarray[ii].md->NBparamMAX;
                track_list[track_idx].params =
                    (PARAM_TRACK *) calloc(fpsarray[ii].md->NBparamMAX, sizeof(PARAM_TRACK));
                ...
                for (int pp = 0; pp < fpsarray[ii].md->NBparamMAX; pp++)
                {
                    if (fpsarray[ii].parray[pp].fpflag & FPFLAG_ACTIVE)
                    {
                        ...
                        functionparameter_GetParamValueString(
                            &fpsarray[ii].parray[pp], track_list[track_idx].params[pp].value,
                            VALSTR_LEN);
                    }
                    else
                    {
            ...
            {
                PARAM_TRACK *tmp =
                    (PARAM_TRACK *) realloc(track_list[track_idx].params,
            ...
            for (int pp = 0; pp < fpsarray[ii].md->NBparamMAX; pp++)
            {
                ...
                if (fpsarray[ii].parray[pp].cnt0 != track_list[track_idx].params[pp].cnt0)
                {
                    ...
                    functionparameter_GetParamValueString(&fpsarray[ii].parray[pp], current_val,
                                                          VALSTR_LEN);
                    ...
                    if (track_list[track_idx].params[pp].cnt0 != -1)
                    {
                        ...
                        printf(" %s %s : %s -> %s  (cnt: %ld)\n", track_list[track_idx].name,
                               fpsarray[ii].parray[pp].keywordfull,
                               track_list[track_idx].params[pp].value, current_val,
        ...
        for (int jj = 0; jj < track_list_cnt; jj++)
        {
            if (!track_list[jj].active && track_list[jj].name[0] != '\0')
            {
                print_ut_timestamp();
                printf(" DEL_FPS %s\n", track_list[jj].name);
                fflush(stdout);
