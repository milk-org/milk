...
#define FEL_DESC "list installed milk-fpsexec-* standalone compute units"
...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-f, --fuzzy", mh_color ? MH_RST : "",
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_ARG : "", "SEARCH_TERM", mh_color ? MH_RST : "",
    ...
    const char *see_also[] = { "milk-fpsexec-help:print detailed FPS framework info",
 ...
 * -------------------------------------------------------------- */
static int fetch_description(const char *cmd, char *desc, size_t descsz)
{
    char cmdline[256];
 ...
 * -------------------------------------------------------------- */
static const char *g_skip[] = { "milk-fpsexec-help", "milk-fpsexec-list", NULL };
...
static void discover_commands(void)
{
    ...
    while (dir != NULL && g_n_entries < FEL_MAX_CMDS)
    {
        ...
        {
            for (size_t path_idx = 0; path_idx < gl.gl_pathc && g_n_entries < FEL_MAX_CMDS;
                 path_idx++)
            {
                /* Extract basename */
                ...
                struct stat st;
                if (stat(gl.gl_pathv[path_idx], &st) != 0 || !(st.st_mode & S_IXUSR))
                {
                    continue;
                }
                ...
                if (!fetch_description(gl.gl_pathv[path_idx], e->desc, sizeof(e->desc)))
                {
 ...
 * -------------------------------------------------------------- */
static int fuzzy_score(const char *query,
                       const char *text,
                       int        *positions,
                       int         posmax,
                       int        *n_pos_out)
{
    ...
    for (size_t ti = 0; ti < tlen && (size_t) qi < qlen; ti++)
    {
 ...
 * -------------------------------------------------------------- */
static void print_highlighted(const char *s, const int *hl_pos, int hl_n)
{
 ...
 * -------------------------------------------------------------- */
static void output_json(const struct felentry *entries, int n)
{
    printf("[\n");
...
int main(int argc, char *argv[])
{
    ...
    {
        if (strcmp(argv[arg_idx], "-f") == 0 || strcmp(argv[arg_idx], "--fuzzy") == 0)
        {
            fuzzy = 1;
        }
        else if (strcmp(argv[arg_idx], "-j") == 0 || strcmp(argv[arg_idx], "--json") == 0)
        {
            json_out = 1;
        }
    ...
    for (int ent_idx = 0; ent_idx < g_n_entries; ent_idx++)
    {
        ...
        if (fuzzy)
        {
            ...
            for (int term_idx = 0; term_idx < n_terms; term_idx++)
            {
                ...
                int        n_pos = 0;
                int        sc    = fuzzy_score(terms[term_idx], line, pos,
            ...
            int copy_n = n_all_pos < (int) (sizeof(fe.hl_pos) / sizeof(fe.hl_pos[0]))
                             ? n_all_pos
                             : (int) (sizeof(fe.hl_pos) / sizeof(fe.hl_pos[0]));
        ...
        else
        {
            ...
            regex_t re;
            if (regcomp(&re, terms[0], REG_EXTENDED | REG_ICASE | REG_NOSUB) != 0)
            {
                fprintf(stderr,
                        "\033[1;31mERROR\033[0m:"
                        " invalid regex '%s'\n",
