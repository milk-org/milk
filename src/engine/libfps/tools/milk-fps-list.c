...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-14s%s %s\n\n", mh_color ? MH_ARG : "", "[regex]", mh_color ? MH_RST : "",
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-e, --exec", mh_color ? MH_RST : "",
           "Show full path to executable");
...
int main(int argc, char *argv[])
{
    ...
    printf(C_TITLE "%-25s %-30s %-20s %s %s %s   %s" C_RST "\n", "FPS Name", "Executable",
           "Cmd Key", "   CONF", "    RUN", "P", "Description");
    ...
    if (NBfps > 0)
    {
        for (int ii = 0; ii < NBfps; ii++)
        {
            ...
            {
                exec_basename = fpsarray[ii].md->execfullpath;
            }
            ...
            // Check tmux session
            char tmux_cmd[256];
            snprintf(tmux_cmd, sizeof(tmux_cmd), "tmux has-session -t %s 2> /dev/null",
                     fpsarray[ii].md->name);
            ...
            snprintf(status_str, 256, "%s %s %s  %s ", conf_pid_str, run_pid_str, tmux_str,
                     proc_str);


            if (show_exec)
            {
                printf(C_NAME "%-25s" C_RST " " C_TYPE "%-30s" C_RST " " C_HDR "%-20s" C_RST " "
                              "%s   %s\n",
                       fpsarray[ii].md->name, fpsarray[ii].md->execfullpath,
            ...
            else
            {
                printf(C_NAME "%-25s" C_RST " " C_TYPE "%-30s" C_RST " " C_HDR "%-20s" C_RST " "
                              "%s   %s\n",
                       fpsarray[ii].md->name, exec_basename, fpsarray[ii].md->callprogname,
                       status_str, fpsarray[ii].md->description);
            }
