...
errno_t fps_add_processinfo_entries(FPS *fps)
{
    ...
    // run time string
    function_parameter_add_entry(fps, ".conf.timestring", "runstart time string", FPTYPE_STRING,
                                 FPFLAG, "undef", NULL);

    // custom label
    function_parameter_add_entry(fps, ".conf.label", "custom label", FPTYPE_STRING, FPFLAG, "",
                                 NULL);
    ...
    function_parameter_add_entry(fps, ".conf.datadir", "data directory", FPTYPE_DIRNAME, FPFLAG,
                                 fps->md->datadir, NULL);
    ...
    function_parameter_add_entry(fps, ".conf.confdir", "conf directory", FPTYPE_DIRNAME, FPFLAG,
                                 fps->md->confdir, NULL);

    // Where results are archived
    //
    function_parameter_add_entry(fps, ".conf.archivedir", "archive directory", FPTYPE_DIRNAME,
                                 FPFLAG, NULL, NULL);
    ...
    // taskset
    function_parameter_add_entry(fps, ".procinfo.taskset", "CPUs mask", FPTYPE_STRING, FPFLAG,
                                 "1-127", NULL);

    // value = 0 indicates process will adjust to available nb cores
    long maxNBthread_default[4] = { 1, 0, 128, 1 };
    ...
    function_parameter_add_entry(fps, ".procinfo.enabled", "procinfo mode", FPTYPE_ONOFF, FPFLAG,
                                 NULL, &fp_pinfoenabled);
