...
)
{
    ...
    /* Print prompt with param context */
    {
        char prompt[512];
        int n;
        if(!(fps[fps_idx].parray[p_idx].fpflag
                & FPFLAG_WRITESTATUS))
        {
...
)
{
    ...
    if(ch != -1)
    {
        ...
        if(fpsCTRLvar->search_mode > 0)
        {
            ...
            if(ch != ANSI_KEY_UP && ch != ANSI_KEY_DOWN && ch != ANSI_KEY_PGUP && ch != ANSI_KEY_PGDN
                    && ch != ANSI_KEY_LEFT && ch != ANSI_KEY_RIGHT)
            {
        ...
        case 'y':     // Yank to tmux buffer
        {
            ...
            if(keywnode[fpsCTRLvar->nodeSelected].leaf == 1)
            {
                ...
                snprintf(yankstr, sizeof(yankstr), "%s = %s", fps[fps_idx].parray[p_idx].keywordfull, valstr);
            }
            else
            {
        ...
        case ' ' :
            ...
            {
                char cmd[512];
                snprintf(cmd, sizeof(cmd), "tmux send-keys -t %s:run \"cd %s\" C-m", fps[fps_idx].md->name,
                         fps[fps_idx].md->workdir);
                if(system(cmd) != 0) { }  // Corrected escaping for "cd %s"
                snprintf(cmd, sizeof(cmd), "tmux send-keys -t %s:run \"%s %s/%s.fps\" C-m", fps[fps_idx].md->name,
                         fps[fps_idx].parray[p_idx].val.string[0], fps[fps_idx].md->datadir, fps[fps_idx].md->name);
        ...
        case 'R' : // start run process
        {
            ...
            EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:run \" cd %s\" C-m", selected_fps->md->name,
                                           selected_fps->md->workdir);
            EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:run \" %s %s:runstart\" C-m",
                                           selected_fps->md->name, progexec, selected_fps->md->name);
        }
        break;
        ...
        case 'O': // start conf process
        {
            ...
            EXECUTE_SYSTEM_COMMAND_NOCHECK("tmux send-keys -t %s:conf \" cd %s\" C-m", selected_fps->md->name,
                                           selected_fps->md->workdir);
