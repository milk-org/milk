...
)
{
    ...
    for(int sortindex = 0; sortindex < sortcnt; sortindex++)
    {
        ...
        if(sortindex < wrow - 8)   // display
        {
            ...
            if(state->tasklist[fpscmdindex].status & FPSTASK_STATUS_ACTIVE) TUI_printfw(">> ");
            else TUI_printfw("  ");

            if(state->tasklist[fpscmdindex].flag & FPSTASK_FLAG_WAITONRUN) TUI_printfw("WR ");
            else TUI_printfw("   ");

            if(state->tasklist[fpscmdindex].flag & FPSTASK_FLAG_WAITONCONF) TUI_printfw("WC ");
            else TUI_printfw("   ");
            ...
            if(state->tasklist[fpscmdindex].status & FPSTASK_STATUS_RECEIVED) TUI_printfw(" R");
            else TUI_printfw(" -");
