 ...
 */
errno_t fpsCTRL_scheduler_display(FPSCTRL_PROCESS_VARS *fpsCTRLvar,
                                  int                   wrow,
                                  int                  *wrowstart __attribute__((unused)))
{
    ...
    if (sort_evalarray == NULL || sort_indexarray == NULL)
    {
    ...
    for (int sort_idx = 0; sort_idx < sortcnt; sort_idx++)
    {
        ...
        if (sort_idx < wrow - 8) // display
        {
            ...
            TUI_printfw("[Q:%02d P:%02d] %4d", state->tasklist[fpscmd_idx].queue,
