 ...
 * level.
 */
static void fpsCTRL__filter_and_sort_children(KEYWORD_TREE_NODE    *keywnode,
                                              ...
                                              int                  *num_filtered)
{
    ...
    if (fpsCTRLvar->sort_mode == 1 || fpsCTRLvar->sort_mode == 2)
    {
        for (int curr_idx = 0; curr_idx < (*num_filtered) - 1; curr_idx++)
        {
            for (int next_idx = curr_idx + 1; next_idx < (*num_filtered); next_idx++)
            {
                int swap = 0;
                ...
                {
                    /* A-Z by keyword */
                    swap = (strcasecmp(keywnode[ki].keyword[fpsCTRLvar->currentlevel],
                ...
                else
                {
                    ...
                    uint32_t si = 0;
                    uint32_t sj = 0;
                    if (fi >= 0 && fpsarray[fi].md)
                    {
                        si = fpsarray[fi].md->status;
                    }
                    if (fj >= 0 && fpsarray[fj].md)
                    {
 ...
 */
static void fpsCTRL__calculate_widths_and_layout(KEYWORD_TREE_NODE    *keywnode,
                                                 FPSCTRL_PROCESS_VARS *fpsCTRLvar,
                                                 int                  *nodechain,
                                                 int                   num_filtered,
                                                 int                  *filtered_children,
                                                 ...
                                                 int                  *max_val_width)
{
    ...
    if (!(fpsarray[fpsCTRLvar->fpsindexSelected].parray[0].fpflag & FPFLAG_VISIBLE)) // if invisible
    {
    ...
    while (fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel] > num_filtered - 1)
    {
        fpsCTRLvar->GUIlineSelected[fpsCTRLvar->currentlevel]--;
    }
    ...
    for (int filt_idx = 0; filt_idx < num_filtered; filt_idx++)
    {
        ...
        if (keywnode[knode_idx].leaf)
        {
            ...
            kw_len       = strlen(
                fpsarray[fpsindex].parray[pindex].keyword[keywnode[knode_idx].keywordlevel - 1]);
            ...
            {
                functionparameter_GetParamValueString(&fpsarray[fpsindex].parray[pindex], valstring,
                                                      200);
            }
    ...
    DEBUG_TRACEPOINT("max_kw_width[cl]: %d, (*max_val_width): %d", max_kw_width[cl],
                     (*max_val_width));
 ...
 */
static void fpsCTRL__render_summary_and_breadcrumbs(KEYWORD_TREE_NODE    *keywnode,
                                                    FPSCTRL_PROCESS_VARS *fpsCTRLvar,
                                                    int                  *nodechain,
                                                    int                   GUIlineMax,
                                                    int                   num_filtered,
                                                    ...
                                                    int                  *summary_printed)
{
    ...
    if (keywnode[fpsCTRLvar->nodeSelected].leaf)
    {
        ...
        {
            IMAGE                 tmpimg;
            FPS_STREAMNAME_PARSED sp_sum =
                fps_streamname_parse(fpsarray[fpsidx].parray[pidx].val.string[0]);
        ...
        if (!(*summary_printed))
        {
            ...
            if (klen >= nlen && strcmp(kfull + klen - nlen, needle) == 0)
            {
 ...
 */
static void fpsCTRL__render_parameter_rows(KEYWORD_TREE_NODE    *keywnode,
                                           FPSCTRL_PROCESS_VARS *fpsCTRLvar,
                                           int                  *nodechain,
                                           int                  *filtered_children,
                                           int                   num_filtered,
         ...
         GUIline++) // GUIline is the line number on GUI display
    {
        ...
        if (knodeindex >= 0 && knodeindex < fpsCTRLvar->NBkwn)
        {
            ...
            if (keywnode[knodeindex].leaf == 0) // If this is a directory
            {
                ...
                if (child_index[cl] < num_filtered)
                {
                    ...
                    TUI_printfw("%-*.*s", max_kw_width[cl], max_kw_width[cl],
                                keywnode[knodeindex].keyword[l - 1]);
                    screenprint_unsetcolor(5);
            ...
            else // If this is a parameter
            {
                ...
                int isVISIBLE = 1;
                int isTRIGGER = 0;
                if (!(fpsarray[fpsindex].parray[pindex].fpflag & FPFLAG_VISIBLE)) // if invisible
                {
                    isVISIBLE = 0;
                    screenprint_setdim();
                ...
                if (isVISIBLE && (fpsarray[fpsindex].parray[pindex].fpflag & FPFLAG_TRIGGER_STREAM))
                {
                    isTRIGGER = 1;
                ...
                TUI_printfw("%-*.*s", max_kw_width[cl], max_kw_width[cl],
                            fpsarray[fpsindex]
                                .parray[pindex]
                                .keyword[keywnode[knodeindex].keywordlevel - 1]);
                if (fpsarray[fpsindex].parray[pindex].fpflag & FPFLAG_PRIMARY_CLI_INPUT)
                {
                    if (GUIline != fpsCTRLvar->GUIlineSelected[cl])
                    {
                ...
                if (fpsarray[fpsindex].parray[pindex].fpflag & FPFLAG_FEEDBACK)
                {
                    ...
                    {
                        if (fpsarray[fpsindex].parray[pindex].val.i64[0] !=
                            fpsarray[fpsindex].parray[pindex].val.i64[3])
                        {
                            paramsync = 0;
                        }
                    }
                    ...
                    {
                        if (fabs(fpsarray[fpsindex].parray[pindex].val.f64[0] -
                                 fpsarray[fpsindex].parray[pindex].val.f64[3]) > 1e-6)
                        {
                            paramsync = 0;
                        }
                    }
                ...
                {
                    functionparameter_GetParamValueString(&fpsarray[fpsindex].parray[pindex],
                                                          valstring, 200);
                }
                ...
                if (isVISIBLE == 1 && valstring[0] != '\0')
                {
                    ...
                    if (ptype == FPTYPE_STREAMNAME)
                    {
                        ...
                        else if (sp_v.must_new && exists)
                        {
                            path_val_color = 4;
                        }
                        else if (sp_v.must_exist && !exists)
                        {
                            path_val_color = 4;
                        }
                        else if (exists)
                    ...
                    {
                        if (stat(valstring, &st) == 0 && S_ISDIR(st.st_mode))
                        {
                            path_val_color = 2;
                        }
                        else
                    ...
                    else if (ptype == FPTYPE_EXECFILENAME)
                    {
                        if (stat(valstring, &st) == 0 && S_ISREG(st.st_mode) &&
                            (st.st_mode & S_IXUSR))
                        {
                            path_val_color = 2;
                        }
                    ...
                    else if (ptype == FPTYPE_FILENAME || ptype == FPTYPE_FITSFILENAME)
                    {
                        if (stat(valstring, &st) == 0 && S_ISREG(st.st_mode))
                        {
                            path_val_color = 2;
                ...
                int onoff_on = 0;
                if (isVISIBLE == 1 && fpsarray[fpsindex].parray[pindex].type == FPTYPE_ONOFF &&
                    fpsarray[fpsindex].parray[pindex].val.i32[0])
                {
                    onoff_on = 1;
                    screenprint_setbgcolor_highlight();
                    screenprint_setbold();
 ...
 */
static void fpsCTRL__render_footer_status(FPSCTRL_PROCESS_VARS *fpsCTRLvar,
                                          int                   GUIlineMax,
                                          int                   dispindexMax)
{
    ...
    else
    {
        ...
        for (int msgi = 0; msgi < fpsarray[fpsCTRLvar->fpsindexSelected].md->msgcnt; msgi++)
        {
            ...
            TUI_printfw("%-40s %s", fpsarray[fpsCTRLvar->fpsindexSelected].parray[pidx].keywordfull,
...
errno_t fpsCTRL_FPSdisplay(KEYWORD_TREE_NODE *keywnode, FPSCTRL_PROCESS_VARS *fpsCTRLvar)
{
    ...
    if (fpsCTRLvar->NBfps > 0)
    {
        ...
        {
            fpsCTRLscreen_print_nodeinfo(fpsarray, keywnode, fpsCTRLvar->nodeSelected,
        ...
        fpsCTRL__calculate_widths_and_layout(keywnode, fpsCTRLvar, nodechain, num_filtered,
                                             filtered_children, &GUIlineMax, child_index,
                                             max_kw_width, &max_val_width);

        int dispindexMax    = 5;
        int doffsetindex    = 0;
        int lastindex       = 0;
        int summary_printed = 0;
        fpsCTRL__render_summary_and_breadcrumbs(
            keywnode, fpsCTRLvar, nodechain, GUIlineMax, num_filtered, filtered_children,
            child_index, &dispindexMax, &doffsetindex, &lastindex, &summary_printed);

        fpsCTRL__render_parameter_rows(keywnode, fpsCTRLvar, nodechain, filtered_children,
                                       num_filtered, doffsetindex, lastindex, child_index,
                                       max_kw_width, max_val_width, GUIlineMax);
 ...
 */
errno_t fpsCTRL_FPSlog(KEYWORD_TREE_NODE *keywnode, FPSCTRL_PROCESS_VARS *fpsCTRLvar)
{
    (void) keywnode;
    if (fpsCTRLvar->NBfps > 0)
    {
