    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        else if (strncmp(line, "set ", 4) == 0)
        {
            ...
            if (sscanf(line, "set %s %[^\n]", vname, vval) == 2)
            {
                ...
                {
                    snprintf(ctx->vars[ctx->num_vars].name, sizeof(ctx->vars[0].name), "%s", vname);
                    snprintf(ctx->vars[ctx->num_vars].value, sizeof(ctx->vars[0].value), "%s",
                             vval);
                    ctx->num_vars++;
                }
            }
                            ...
                            KEYWORD_TREE_NODE *keywnode)
{
    ...
    for (int i = 0; i < ctx.num_lines; i++)
    {
            ...
            if (end_idx != -1)
            {
                if (condition_met)
                {
                    ...
                    for (int j = i + 1; j < end_idx; j++)
                    {
                        ...
                        state->tasklist[t_idx].status =
                            FPSTASK_STATUS_ACTIVE | FPSTASK_STATUS_WAITING;
        ...
        uint32_t t_idx = state->task_input_counter % NB_FPSCTRL_TASK_MAX;
        snprintf(state->tasklist[t_idx].cmdstring, sizeof(state->tasklist[t_idx].cmdstring), "%s",
                 cmd);
        state->tasklist[t_idx].queue      = queue;
        state->tasklist[t_idx].flag       = current_error_policy;
