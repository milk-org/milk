                           ...
                           FPSCTRL_PROCESS_VARS *vars)
{
    ...
    for (uint32_t qi = 0; qi < NB_FPSCTRL_TASKQUEUE_MAX; qi++)
    {
        ...
        while (queue_nexttask[qi] == QUEUE_SCANREADY)
        {
            ...
            for (uint32_t cmdindex = 0;
                 cmdindex < state->NBtasks_max && cmdindex < NB_FPSCTRL_TASK_MAX; cmdindex++)
            {
            ...
            if (cmdOK == 1)
            {
                ...
                else
                {
                    ...
                    {
                        if (fps[state->tasklist[cmdindexExec].fpsindex].md->status &
                    ...
                    if (state->tasklist[cmdindexExec].flag & MILKSEQ_TASKFLAG_WAITSEQ_IDLE)
                    {
                        ...
                        if (sscanf(state->tasklist[cmdindexExec].cmdstring, "wait_seq %63s idle",
                                   target_seq) == 1)
                        {
                            MILKSEQ_STATE *tstate = milkseq_connect(target_seq);
                            if (tstate != NULL)
                            {
                                if (tstate->NBtasks_active > 0 ||
                                    (tstate->status & MILKSEQ_STATUS_RUNNING))
                                {
                                    task_completed     = 0;
                                    queue_nexttask[qi] = QUEUE_WAIT;
                                }
    ...
    while (taskcnt > state->NBtasks_max - NB_FPSCTRL_TASK_PURGESIZE)
    {
        ...
        for (uint32_t cmdindex = 0; cmdindex < state->NBtasks_max && cmdindex < NB_FPSCTRL_TASK_MAX;
             cmdindex++)
        {
    ...
    if (nexttask_cmdindex != -1 && nexttask_priority > 0)
    {
        ...
        state->tasklist[cmdindexExec].fpsindex =
            milkseq_exec_cmd(cmdindexExec, state, fps, keywnode, vars, &taskstatus);
