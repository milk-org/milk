...
void procctrl_handle_keyboard_event(procctrl_context_t *ctx, int ch, int NBactive)
{
    ...
    else if ((ch == 'T' || ch == 'K' || ch == 'I') && NBactive > 0)
    {
        ...
        for (int i = 0; i < PROCESSINFOLISTSIZE; i++)
        {
            if (ctx->procinfoproc->selectedarray[i])
            {
    ...
    else if ((ch == 'p' || ch == 19 || ch == 'e') && NBactive > 0)
    {
        ...
        for (int i = 0; i < PROCESSINFOLISTSIZE; i++)
        {
            if (ctx->procinfoproc->selectedarray[i])
            {
                ...
                {
                    ctx->procinfoproc->pinfoarray[i]->CTRLval =
                        (ctx->procinfoproc->pinfoarray[i]->CTRLval == 0) ? 1 : 0;
                }
    ...
    else if ((ch == 'z' || ch == 'Z') && NBactive > 0)
    {
        ...
        for (int i = 0; i < PROCESSINFOLISTSIZE; i++)
        {
            if ((all || ctx->procinfoproc->selectedarray[i]) && ctx->procinfoproc->pinfoarray[i])
            {
                ctx->procinfoproc->pinfoarray[i]->loopcnt = 0;
            }
        }
        if (!all && !ctx->procinfoproc->selectedarray[ctx->pindexSelected] &&
            ctx->pindexSelected >= 0 && ctx->procinfoproc->pinfoarray[ctx->pindexSelected])
        {
