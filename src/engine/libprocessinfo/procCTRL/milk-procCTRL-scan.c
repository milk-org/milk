...
int main(int argc, char *argv[]) {
                ...
                if (scan_shm->request_scan[i] == 1) {
                    ...
                    if (pinfo_mappings[i] != NULL) {
                        // Update live stats in scan_shm for readers
                                            ...
                                            strncpy(scan_shm->pinfodisp[i].statusmsg, pinfo_mappings[i]->statusmsg, 199);
                                            strncpy(scan_shm->pinfodisp[i].name, pinfo_mappings[i]->name, 39);
                                            
                                            // Triggering info
                                            scan_shm->pinfodisp[i].triggermode = pinfo_mappings[i]->triggermode;
                                            strncpy(scan_shm->pinfodisp[i].triggerstreamname, pinfo_mappings[i]->triggerstreamname, 79);
                        ...
                        if (pinfo_mappings[i]->MeasureTiming != 0) {
                            ...
                            for(int tindex = 0; tindex < PROCESSINFO_NBtimer - 1; tindex++) {
                                ...
                                dtexec_array[tindex] = (pinfo_mappings[i]->texecend[ti0].tv_nsec - pinfo_mappings[i]->texecstart[ti0].tv_nsec) + 
    ...
    for(long i=0; i<PROCESSINFOLISTSIZE; i++) if(pinfo_mappings[i]) processinfo_shm_close(pinfo_mappings[i], pinfo_fds[i]);
    free(timearray);
    free(indexarray);
