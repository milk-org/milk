 ...
 */
static int __attribute__((unused)) processinfo_SelectFromList(STRINGLISTENTRY *StringList,
                                                              int              NBelem)
{
    int   selected = 0;
    int   inputOK  = 0;
...
static void procctrl_update_stats(procctrl_context_t *ctx)
{
    ...
    double elapsed = (ctx->t_now.tv_sec - ctx->t_last_scan.tv_sec) +
                     (ctx->t_now.tv_nsec - ctx->t_last_scan.tv_nsec) * 1e-9;
...
static errno_t procctrl_init(procctrl_context_t *ctx)
{
    ...
    snprintf(scan_shm_name, sizeof(scan_shm_name), "%s/%s", ctx->procdname,
    ...
    ctx->procinfoproc->pinfodisp =
        (PROCESSINFODISP *) calloc(PROCESSINFOLISTSIZE, sizeof(PROCESSINFODISP));
...
static void procctrl_cleanup(procctrl_context_t *ctx)
{
    ...
    if (ctx->procinfoproc)
    {
        ...
        {
            if (ctx->procinfoproc->pinfommapped[i])
            {
                if (ctx->procinfoproc->pinfoarray[i] != NULL &&
                    ctx->procinfoproc->pinfoarray[i] != (PROCESSINFO *) MAP_FAILED)
                {
                    processinfo_shm_close(ctx->procinfoproc->pinfoarray[i],
                                          ctx->procinfoproc->fdarray[i]);
                }
            }
        }
...
errno_t processinfo_CTRLscreen()
{
    ...
    while (ctx.loopOK)
    {
        ...
        if (NBactive > 0 && ctx.scan_shm != NULL)
        {
            ...
            {
                ctx.pindexSelected =
                    ctx.procinfoproc->local_sorted_pindex[ctx.pindexActiveSelected];
            }
            else
            {
