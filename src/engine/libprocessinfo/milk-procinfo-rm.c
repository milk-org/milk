...
#define PI_RM_DESC "remove processinfo shared-memory entries matching a regex"
...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-c, --clean-dead", mh_color ? MH_RST : "",
           "Remove all CRASHED or STOPPED entries");
...
int main(int argc, char *argv[])
{
    ...
    static struct option long_options[] = { { "clean-dead", no_argument, 0, 'c' },
                                            { "verbose", no_argument, 0, 'v' },
                                            { "help", no_argument, 0, 'h' },
                                            { 0, 0, 0, 0 } };

    while ((opt = getopt_long(argc, argv, "cvh", long_options, NULL)) != -1)
    {
        switch (opt)
        {
        case 'c':
