...
static CLICMDARGDEF farg[] = { { CLIARG_STR, ".strfilter", "string filter", "im",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &stringfilter, NULL } };

static CLICMDDATA CLIcmddata = { "shmimpurge", "purge orphan streams", CLICMD_FIELDS_DEFAULTS };
...
errno_t shmim_purge(const char *strfilter)
{
    ...
    for (int sindex = 0; sindex < NBstream; sindex++)
    {
        ...
        DEBUG_TRACEPOINT("stream %s loaded ID %ld", streaminfo[sindex].sname, (long) ID);
