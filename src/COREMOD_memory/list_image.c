...
{
    RegisterCLIcommand("mmon", __FILE__, memory_monitor__cli, "Monitor memory content", "terminal tty name",
                       "mmon /dev/pts/4", "int memory_monitor(const char *ttyname)");

    RegisterCLIcommand("listim", __FILE__, list_image_ID, "list images in memory", "no argument", "listim",
                       "errno_t list_image_ID()");
...
errno_t list_image_ID_ofp_simple(FILE *fo)
{
    ...
    for (i = 0; i < data.NB_MAX_IMAGE; i++)
        if (data.image[i].used == 1)
        {
            ...
            fprintf(fo, "%20s %d %ld %d %4ld", data.image[i].name, datatype, (long)data.image[i].md[0].naxis,
                    data.image[i].md[0].shared, (long)data.image[i].md[0].size[0]);
