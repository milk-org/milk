...
static errno_t COREMOD_MEMORY_streamDelay__cli()
{
    ...
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, 5) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) ==
        0)
    {
        COREMOD_MEMORY_streamDelay(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t stream_delay_addCLIcmd()
{
    ...
    RegisterCLIcommand(
        "streamdelay", __FILE__, COREMOD_MEMORY_streamDelay__cli, "delay 2D image stream",
        "<image2d in> <image2d out> <delay [us]> <resolution [us]>", "streamdelay instream outstream 1000 10",
...
errno_t COREMOD_MEMORY_streamDelay_FPCONF()
{
    ...
    long fp_delayus = 0;
    function_parameter_add_entry(&fps, ".delayus", "Delay [us]", FPTYPE_INT64, FPFLAG, &delayus_default, &fp_delayus);
    (void)fp_delayus; // suppresses unused parameter compiler warning

    long dtus_default[4] = {50, 1, 200, 50};
    long fp_dtus = 0;
    function_parameter_add_entry(&fps, ".dtus", "Loop period [us]", FPTYPE_INT64, FPFLAG, &dtus_default, &fp_dtus);
    ...
    FPS_ADDPARAM_INT64_IN(option_timeavemode, ".option.timeavemode", "Enable time window averaging (>0)",
                          &timeavemode_default);

    double avedt_default[4] = {0.001, 0.0001, 1.0, 0.001};
    FPS_ADDPARAM_FLT64_IN(option_avedt, ".option.avedt", "Averaging time window width", &avedt_default);
...
errno_t COREMOD_MEMORY_streamDelay_RUN()
{
    ...
    strncpy(IDin_name, functionparameter_GetParamPtr_STRING(&fps, ".in_name"), FUNCTION_PARAMETER_STRMAXLEN - 1);

    char IDout_name[FUNCTION_PARAMETER_STRMAXLEN];
    strncpy(IDout_name, functionparameter_GetParamPtr_STRING(&fps, ".out_name"), FUNCTION_PARAMETER_STRMAXLEN - 1);
    ...
    if (IDin == -1)
    {
        ...
        fprintf(stderr, "%02d:%02d:%02d.%09ld  ERROR [%s %s %d] Input stream %s does not exist, cannot proceed\n",
                errtm->tm_hour, errtm->tm_min, errtm->tm_sec, errtime.tv_nsec, __FILE__, __FUNCTION__, __LINE__,
                IDin_name);
    ...
    *kkout = 0;
    ...
    processinfo_waitoninputstream_init(processinfo, IDin, PROCESSINFO_TRIGGERMODE_DELAY, -1);
