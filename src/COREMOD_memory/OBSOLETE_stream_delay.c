...
errno_t COREMOD_MEMORY_streamDelay_RUN()
{
    ...
    strncpy(IDin_name,
            functionparameter_GetParamPtr_STRING(&fps, ".in_name"),
            FUNCTION_PARAMETER_STRMAXLEN - 1);

    char IDout_name[FUNCTION_PARAMETER_STRMAXLEN];
    strncpy(IDout_name,
            functionparameter_GetParamPtr_STRING(&fps, ".out_name"),
            FUNCTION_PARAMETER_STRMAXLEN - 1);
    ...
    int timeavemode =
        functionparameter_GetParamValue_INT64(&fps, ".option.timeavemode");
    double *avedt =
        functionparameter_GetParamPtr_FLOAT64(&fps, ".option.avedt");

    long *zsize = functionparameter_GetParamPtr_INT64(&fps, ".status.zsize");
    long *framelag =
        functionparameter_GetParamPtr_INT64(&fps, ".status.framelag");
                    ...
                    switch (timeavemode)
                            ...
                            if (cntskip > 0)
                                {
                                    ...
                                    ptr +=
                                        SIZEOF_DATATYPE_FLOAT * xysize * *kkout;
                                    ...
                                    COREMOD_MEMORY_image_set_sempost_byID(IDout,
                                                                          -1);
                        ...
                        default: // strict time window (note: other modes will be coded in the future)
                            ...
                            for (kkinscan = 0; kkinscan < *zsize; kkinscan++)
                                {
                                    tdiff =
                                        timespec_diff(t0array[kkinscan], tnow);
                                    tdiffv = 1.0 * tdiff.tv_sec +
                                             1.0e-9 * tdiff.tv_nsec;

                                    if ((tdiffv > 0) &&
                                        (fabs(tdiffv - 1.0e-6 * delayus) <
                                         *avedt))
                                        {
                                            float coeff = 1.0;
                                            for (ii = 0; ii < xysize; ii++)
