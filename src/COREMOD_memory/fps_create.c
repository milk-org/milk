...
{
    if (0 + CLI_checkarg(1, CLIARG_LONG) + CLI_checkarg_noerrmsg(2, CLIARG_STR) == 0)
    {
        function_parameter_struct_create(data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.string);
...
errno_t fps_create_addCLIcmd()
{
    ...
    RegisterCLIcommand("fpscreate", __FILE__, fps_create__cli, "create function parameter structure (FPS)",
...
errno_t function_parameter_struct_create(int NBparamMAX, const char *name)
{
    ...
    for (int m = 0; m < data.NBmodule; m++)
    {
        ...
        {
            strncpy(fps.md->modulename[fps.md->NBmodule], data.module[m].loadname, FPS_MODULE_STRMAXLEN - 1);
            fps.md->NBmodule++;
        }
    }
