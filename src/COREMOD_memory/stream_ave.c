...
static errno_t COREMOD_MEMORY_streamAve__cli()
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_LONG) +
        ...
        0)
    {
        COREMOD_MEMORY_streamAve(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
...
{
    RegisterCLIcommand(
        "streamave", __FILE__, COREMOD_MEMORY_streamAve__cli, "averages stream",
...
imageID COREMOD_MEMORY_streamAve(const char *IDstream_name, int NBave, int mode, const char *IDout_name)
{
    ...
    FUNC_CHECK_RETURN(create_2Dimage_ID("_streamAve_tmp", xsize, ysize, &IDout0));
