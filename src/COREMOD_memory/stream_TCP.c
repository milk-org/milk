...
static uint8_t MAGIC_UDP_MULTIGRAMS = 0x3E;         // Random magic to start datagrams with.
static int     UDP_DGRAM_CHUNK_SIZE =
    62 * 1024; // Max payload per datagram, just shy of the maximum 65507 bytes
...
imageID COREMOD_MEMORY_image_NETWORKreceive(int port, int do_counter_sync, int RT_priority);
...
{
    RegisterCLIcommand("imnetwtransmit", __FILE__, COREMOD_MEMORY_image_NETWORKtransmit__cli,
    ...
    RegisterCLIcommand("imnetwreceive", __FILE__, COREMOD_MEMORY_image_NETWORKreceive__cli,
    ...
    RegisterCLIcommand("imudptransmit", __FILE__, COREMOD_MEMORY_image_NETUDPtransmit__cli,
                       "transmit image over network",
    ...
    RegisterCLIcommand("imudpreceive", __FILE__, COREMOD_MEMORY_image_NETUDPreceive__cli,
...
int initialize_endpoints_tcp(const char         *IPaddr,
                             const int           port,
                             int                *p_fds_client,
                             ...
                             PROCESSINFO        *pinfo)
{
    ...
    if (connect(*p_fds_client, (struct sockaddr *) sock_server, sizeof(*sock_server)) < 0)
    {
...
int initialize_endpoints_udp(const char         *IPaddr,
                             const int           port,
                             int                *p_fds_client,
                             struct sockaddr_in *sock_server,
                             PROCESSINFO        *pinfo)
{
    ...
    int r = setsockopt(*p_fds_client, SOL_SOCKET, SO_REUSEADDR, (char *) &flag, sizeof(flag));
    r -= setsockopt(*p_fds_client, SOL_SOCKET, SO_REUSEPORT, (char *) &flag, sizeof(flag));

#ifdef SO_ATTACH_REUSEPORT_CBPF
    r -= setsockopt(*p_fds_client, SOL_SOCKET, SO_ATTACH_REUSEPORT_CBPF, (char *) &flag,
                    sizeof(flag));
#endif
    if (r < 0)
    {
...
long send_TCP(int fds_client, int num_chunks, char **chunks_ptrs, uint64_t *chunk_sizes)
{
    // SEND
    long _sent_bytes = 0;
              ...
              uint64_t          *chunk_sizes)
{
    ...
    for (int chunk = 0; chunk < num_chunks; ++chunk)
    {
        ...
        while (chunk_remaining_bytes > 0)
        {
            ...
            sendto(fds_client, src, (size_t) to_send, closes_dgram ? 0 : MSG_MORE, pss, psss);
...
imageID common_NETWORKtransmit(const char *IDname,
                               const char *IPaddr,
                               int         port,
                               int         do_counter_sync,
                               int         RT_priority,
                               int         use_TCP)
{
    ...
    {
        loopOK =
            (0 == initialize_endpoints_tcp(IPaddr, port, &fds_client, &sock_server, processinfo));
    }
    else
    {
        loopOK =
            (0 == initialize_endpoints_udp(IPaddr, port, &fds_client, &sock_server, processinfo));
    }
    if (!loopOK)
    {
        goto cleanup;
    ...
    uint64_t size_img_data =
        ImageStreamIO_typesize(img_p->md->datatype) * img_p->md->size[0] * img_p->md->size[1];
    ...
    uint64_t size_img_keywords = SEND_KEYWORDS ? img_p->md->NBkw * sizeof(IMAGE_KEYWORD) : 0L;
    ...
    uint64_t total_bytes_to_transfer =
        size_header + size_metadata + size_img_data + size_img_keywords;
    ...
    while (loopOK == 1)
    {
        ...
        {
            _sent_bytes = send_UDP(fds_client, sock_server, 4, pointers_to_send, sizes_to_send);
        }
        ...
        if (_sent_bytes != total_bytes_to_transfer)
        {
            ...
            snprintf(errmsg, 200,
                     ...
                     _sent_bytes, (long) total_bytes_to_transfer);
            printf("%s\n", errmsg);
            fflush(stdout);
                                             ...
                                             int         RT_priority)
{
    ...
    fflush(stdout);
    return common_NETWORKtransmit(IDname, IPaddr, port, do_counter_sync, RT_priority, 1);
}
                                            ...
                                            int         RT_priority)
{
    ...
    fflush(stdout);
    return common_NETWORKtransmit(IDname, IPaddr, port, do_counter_sync, RT_priority, 0);
}
 ...
 */
static imageID ensure_recv_image(IMAGE_METADATA *md, long nbkw_wire)
{
    int     axis;
    ...
    if (!ok)
    {
        ...
        fflush(stdout);
        create_image_ID(md->name, md->naxis, md->size, md->datatype, md->shared, (int) nbkw_wire, 0,
                        &ID);
...
static int initialize_receiving_endpoints_tcp(int port, PROCESSINFO *processinfo, int *p_fds_client)
{
    int                flag = 1;
    int                result;
    int                MAXPENDING = 5;
    ...
    memset((char *) &sock_server, 0, sizeof(sock_server));
    result = setsockopt(fds_server, IPPROTO_TCP, TCP_NODELAY, (char *) &flag, sizeof(flag));
...
static int initialize_receiving_endpoints_udp(int port, PROCESSINFO *processinfo, int *p_fds_server)
{
    int                flag = 1;
    struct sockaddr_in sock_server;
    int                fds_server;
...
#ifdef SO_ATTACH_REUSEPORT_CBPF
    setsockopt(fds_server, SOL_SOCKET, SO_ATTACH_REUSEPORT_CBPF, &flag, sizeof(flag));
#endif
...
static imageID initial_receive_and_initialize_image_TCP(int fds_endpoint, int *nbkw_wire)
{
    ...
    while (1)
    {
        int r = recv(fds_endpoint, buffer, sizeof(NETWORK_HEADER) + sizeof(IMAGE_METADATA),
                     MSG_WAITALL);
        if (r < 0)
        {
 ...
 */
static long recv_buffer_udp(int fds, char *buff, char *dgram_buff, long framesizefull)
{
    int n_dgrams =
        (int) ((framesizefull + (long) UDP_DGRAM_CHUNK_SIZE - 1) / (long) UDP_DGRAM_CHUNK_SIZE);
    ...
    while (1) // retry until a valid seq-0 datagram with good magic is received
    {
        ...
        struct iovec  iov0[2]  = { { .iov_base = hdr0, .iov_len = 2 },
                                   { .iov_base = buff, .iov_len = (size_t) payload0 } };
        struct msghdr msg0     = {
            .msg_name = &src, .msg_namelen = sizeof(src), .msg_iov = iov0, .msg_iovlen = 2
        };
        ...
        {
            uint8_t hdrk[2];
            long    expect = (k == n_dgrams - 1) ? (framesizefull - (long) k * UDP_DGRAM_CHUNK_SIZE)
                                                 : (long) UDP_DGRAM_CHUNK_SIZE;
            struct iovec  iovk[2] = { { .iov_base = hdrk, .iov_len = 2 },
                                      { .iov_base = buff + (long) k * UDP_DGRAM_CHUNK_SIZE,
                                        .iov_len  = (size_t) expect } };
            struct msghdr msgk    = {
                .msg_name = &src, .msg_namelen = sizeof(src), .msg_iov = iovk, .msg_iovlen = 2
            };
                                    ...
                                    int                         use_TCP)
{
    ...
    if (sigaction(SIGTERM, &data.sigact, NULL) == -1 ||
        ...
        sigaction(SIGHUP, &data.sigact, NULL) == -1 || sigaction(SIGPIPE, &data.sigact, NULL) == -1)
    {
    ...
    if (use_TCP)
    {
        if (initialize_receiving_endpoints_tcp(port, processinfo, &fds_receive_point) != 0)
        {
            goto cleanup;
        }
    }
    ...
    {
        if (initialize_receiving_endpoints_udp(port, processinfo, &fds_receive_point) != 0)
        {
            goto cleanup;
        }
    }
    ...
    ssize_t framesize =
        img_p->md->size[0] * img_p->md->size[1] * ImageStreamIO_typesize(img_p->md->datatype);
    ssize_t recv_size_full = sizeof(NETWORK_HEADER) + sizeof(IMAGE_METADATA) + framesize +
    ...
    // Log stream info
    {
        ...
        char m[200];
        snprintf(m, 200, "<- %s [%d x %d x %ld] %s", img_p->md->name, img_p->md->size[0],
                 img_p->md->size[1], n_slices, typestring);
        snprintf(processinfo->description, 200, "%s %dx%dx%ld %s", img_p->md->name,
    ...
    IMAGE_METADATA *recv_md   = (IMAGE_METADATA *) ((char *) hdr + sizeof(NETWORK_HEADER));
    ...
    while (loopOK == 1)
    {
        ...
        {
            recvsize = recv_buffer_udp(fds_receive_point, buffer_full_data, buffer_datagrams,
                                       recv_size_full);
        }
        if (recvsize <= 0)
        {
        ...
        if (_monitor_index == _monitor_interval)
        {
            printf("[%5ld]  input %20ld (+ %8ld)  output %20ld (+ %8ld)\n", _monitor_loop_index,
                   hdr->cnt0, hdr->cnt0 - _m_input_cnt0, img_p->md->cnt0,
                   img_p->md->cnt0 - _m_output_cnt0);
