...
imageID COREMOD_MEMORY_image_NETWORKtransmit(const char *IDname, const char *IPaddr, int port, int mode,
                                             int RT_priority);
...
static errno_t COREMOD_MEMORY_image_NETWORKtransmit__cli()
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) + CLI_checkarg(5, CLIARG_LONG) ==
        0)
    {
        COREMOD_MEMORY_image_NETWORKtransmit(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand("testfuncsem", __FILE__, COREMOD_MEMORY_testfunction_semaphore__cli, "test semaphore loop",
                       "<image> <semindex> <testmode>", "testfuncsem im1 1 0",
                       "int COREMOD_MEMORY_testfunction_semaphore(const char *IDname, int semtrig, int testmode)");

    RegisterCLIcommand(
        "imnetwtransmit", __FILE__, COREMOD_MEMORY_image_NETWORKtransmit__cli, "transmit image over network",
        "<image> <IP addr> <port [long]> <sync mode [int]>", "imnetwtransmit im1 127.0.0.1 0 8888 0",
...
errno_t COREMOD_MEMORY_testfunction_semaphore(const char *IDname, int semtrig, int testmode)
{
    ...
    while (loopOK == 1)
    {
        ...
        if (rv == -1)
        {
            ...
            case EAGAIN:
                printf("    The operation could not be performed without blocking (i.e., the semaphore currently has "
                       "the value zero)\n");
                break;
                                             ...
                                             int RT_priority)
{
    ...
    if (loopOK == 1)
    {
        ...
        {
            framesizeall = framesize1 + data.image[ID].md[0].NBkw * sizeof(IMAGE_KEYWORD);
        }
    ...
    while (loopOK == 1)
    {
        ...
        if (processinfo_compute_status(processinfo) == 1)
        {
            ...
            if (semr == 0)
            {
                ...
                if (TCPTRANSFERKW == 1)
                {
                    memcpy(buff + framesize1, (char *)data.image[ID].kw,
                ...
                if (rs != framesizeall)
                {
                    ...
                    sprintf(errmsg, "ERROR: send() sent a different number of bytes (%d) than expected %ld  %ld  %ld",
                            rs, (long)framesize, (long)framesizeall, (long)sizeof(TCP_BUFFER_METADATA));
                    printf("%s\n", errmsg);
                    fflush(stdout);
...
imageID COREMOD_MEMORY_image_NETWORKreceive(int port, __attribute__((unused)) int mode, int RT_priority)
{
    ...
    if (OKim == 0)
    {
        ...
        create_image_ID(imgmd[0].name, imgmd[0].naxis, imgmd[0].size, imgmd[0].datatype, imgmd[0].shared, nbkw, 0, &ID);
    ...
    if (data.processinfo == 1)
    {
        ...
        sprintf(processinfo->description, "%s %dx%dx%ld %s", imgmd[0].name, (int)xsize, (int)ysize, NBslices,
                typestring);
        processinfo_WriteMessage(processinfo, msgstring);
    }
    ...
    while (loopOK == 1)
    {
        ...
        if (socketOpen == 1)
        {
            ...
            {
                // copy kw
                memcpy(data.image[ID].kw, (IMAGE_KEYWORD *)(buff + framesize1), nbkw * sizeof(IMAGE_KEYWORD));
            }
            ...
            {
                printf("[%5ld]  input %20ld (+ %8ld) output %20ld (+ %8ld)\n", monitorloopindex, frame_md[0].cnt0,
                       frame_md[0].cnt0 - minputcnt, data.image[ID].md[0].cnt0, data.image[ID].md[0].cnt0 - moutputcnt);
