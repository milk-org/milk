...
static errno_t COREMOD_MEMORY_sharedMem_2Dim_log__cli()
{
    ...
    if (0 + CLI_checkarg(1, 3) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, 3) == 0)
    {
...
errno_t logshmim_addCLIcmd()
{
    ...
    RegisterCLIcommand("shmimstreamlog", __FILE__, COREMOD_MEMORY_sharedMem_2Dim_log__cli,
                       "logs shared memory stream (run in current directory)", "<shm image> <cubesize [long]> <logdir>",
                       "shmimstreamlog wfscamim 10000 /media/data \"\"",
                       "long COREMOD_MEMORY_sharedMem_2Dim_log(const char *IDname, uint32_t zsize, const char *logdir, "
                       "const char *IDlogdata_name");

    RegisterCLIcommand("shmimslogstat", __FILE__, COREMOD_MEMORY_logshim_printstatus__cli,
                       "print log shared memory stream status", "<shm image>", "shmimslogstat wfscamim",
                       "int COREMOD_MEMORY_logshim_printstatus(const char *IDname)");

    RegisterCLIcommand("shmimslogonset", __FILE__, COREMOD_MEMORY_logshim_set_on__cli,
                       "set on variable in log shared memory stream", "<shm image> <setv [long]>",
                       "shmimslogonset imwfs 1", "int COREMOD_MEMORY_logshim_set_on(const char *IDname, int setv)");

    RegisterCLIcommand("shmimslogexitset", __FILE__, COREMOD_MEMORY_logshim_set_logexit__cli,
                       "set exit variable in log shared memory stream", "<shm image> <setv [long]>",
...
void *save_fits_function(void *ptr)
{
    ...
    strcpy(imkwarray[0].value.valstr,
           timedouble_to_UTC_timeofdaystring(0.5 * tmsg->arraytime[0] + 0.5 * tmsg->arraytime[tmsg->cubesize - 1]));
    ...
    imkwarray[2].type = 'S';
    strcpy(imkwarray[2].value.valstr, timedouble_to_UTC_timeofdaystring(tmsg->arraytime[tmsg->cubesize - 1]));
    ...
    imkwarray[7].type = 'S';
    strcpy(imkwarray[7].value.valstr, timedouble_to_UTC_timeofdaystring(tmsg->arraytime[0] + lt.tm_gmtoff));
    ...
    strcpy(imkwarray[8].value.valstr,
           timedouble_to_UTC_timeofdaystring(tmsg->arraytime[tmsg->cubesize - 1] + lt.tm_gmtoff));
    ...
    if (tmsg->partial == 0) // full image
    {
        ...
        saveFITS(tmsg->iname, tmsg->fname, 0, tmsg->fname_auxFITSheader, imkwarray, NBcustomKW);
    }
    else
    {
        ...
        printf("auxFITSheader = \"%s\"\n", tmsg->fname_auxFITSheader);
        saveFITS("tmpsavecube", tmsg->fname, 0, tmsg->fname_auxFITSheader, imkwarray, NBcustomKW);
    ...
    if (tmsg->saveascii == 1)
    {
        ...
        for (k = 0; k < tmsg->cubesize; k++)
        {
            ...
            fprintf(fp, "%10ld  %10lu  %15.9lf   %20.9lf  %10ld   %10ld\n", k, tmsg->arrayindex[k],
                    tmsg->arraytime[k] - t0, tmsg->arraytime[k], tmsg->arraycnt0[k], tmsg->arraycnt1[k]);
        }
        fclose(fp);
    }
...
COREMOD_MEMORY_sharedMem_2Dim_log(const char *IDname, uint32_t zsize, const char *logdir, const char *IDlogdata_name)
{
    ...
    while ((logshimconf[0].filecnt != NBfiles) && (logshimconf[0].logexit == 0))
    {
        ...
        if (likely(use_semlog == 1))
        {
            ...
            if (ret == -1)
            {
                ...
                if (errno == EINVAL)
                {
                    ...
                    printf("               The value of abs_timeout.tv_nsecs is less than 0, or greater than or equal "
                           "to 1000 million\n");
                }

                if (errno == EAGAIN)
                {
                    printf("%5d  sem_timedwait [index %ld]: The operation could not be performed without blocking "
        ...
        if ((index > zsize - 1) || ((timeout == 1) && (index > 0)))
        {
            ...
            sprintf(fnameascii, "%s/%s_%02d:%02d:%02ld.%09ld.txt", logdir, IDname, uttimeStart->tm_hour,
                    uttimeStart->tm_min, timenowStart.tv_sec % 60, timenowStart.tv_nsec);
            ...
            sprintf(fname, "%s/%s_%02d:%02d:%02ld.%09ld.fits", logdir, IDname, uttimeStart->tm_hour,
                    uttimeStart->tm_min, timenowStart.tv_sec % 60, timenowStart.tv_nsec);
            ...
            tmsg->arraycnt1 = array_cnt1_cp;
            tmsg->arraytime = array_time_cp;
            WRITE_FILENAME(tmsg->fname_auxFITSheader, "%s/%s.auxFITSheader.shm", data.shmdir, IDname);
