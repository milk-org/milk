...
errno_t create_image_ID(const char *name, long naxis, uint32_t *size, uint8_t datatype, int shared, int nbkw,
                        int CBsize, imageID *outID);
...
static errno_t create_image__cli()
{
    ...
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg_noerrmsg(2, CLIARG_LONG) == 0)
    {
        ...
        {
        case 0:
            create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_FLOAT, data.SHARED_DFT,
                            data.NBKEYWORD_DFT, 0, NULL);
            break;
        case 1:
            create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_DOUBLE, data.SHARED_DFT,
                            data.NBKEYWORD_DFT, 0, NULL);
            break;
        }
        free(imsize);
    ...
    else if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) +
             ...
             0) // type option exists
    {
        ...
        create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, datatype, data.SHARED_DFT, data.NBKEYWORD_DFT, 0,
                        NULL);
...
static errno_t create_image_shared__cli() // default precision
{
    ...
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_LONG) == 0)
    {
        ...
        {
        case 0:
            create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_FLOAT, 1, data.NBKEYWORD_DFT, 0,
                            NULL);
            break;
        case 1:
            create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_DOUBLE, 1, data.NBKEYWORD_DFT, 0,
                            NULL);
            break;
        }
        free(imsize);
...
static errno_t create_ushort_image_shared__cli() // default precision
{
    ...
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_LONG) == 0)
    {
        ...
        create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_UINT16, 1, data.NBKEYWORD_DFT, 0,
                        NULL);
...
static errno_t create_sshort_image_shared__cli() // default precision
{
    ...
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_LONG) == 0)
    {
        ...
        create_image_ID(data.cmdargtoken[1].val.string, naxis, imsize, _DATATYPE_INT16, 1, data.NBKEYWORD_DFT, 0, NULL);
...
static errno_t create_3Dimage_float()
{
    ...
    create_image_ID(data.cmdargtoken[1].val.string, 3, imsize, _DATATYPE_FLOAT, data.SHARED_DFT, data.NBKEYWORD_DFT, 0,
                    NULL);
...
errno_t create_image_addCLIcmd()
{
    ...
    RegisterCLIcommand(
        "creaim", __FILE__, create_image__cli, "create image, default precision", "<name> <xsize> <ysize> <opt: zsize>",
        "creaim imname 512 512",
        "long create_image_ID(const char *name, long naxis, uint32_t *size, uint8_t datatype, 0, 10, NULL)");

    RegisterCLIcommand(
        "creaimshm", __FILE__, create_image_shared__cli, "create image in shared mem, default precision",
        "<name> <xsize> <ysize> <opt: zsize>", "creaimshm imname 512 512",
        "long create_image_ID(const char *name, long naxis, uint32_t *size, uint8_t datatype, 0, 10, NULL)");

    RegisterCLIcommand("creaushortimshm", __FILE__, create_ushort_image_shared__cli,
                       "create unsigned short image in shared mem", "<name> <xsize> <ysize> <opt: zsize>",
                       "creaushortimshm imname 512 512",
                       "long create_image_ID(const char *name, long naxis, long *size, _DATATYPE_UINT16, 0, 10, NULL)");

    RegisterCLIcommand("creasshortimshm", __FILE__, create_sshort_image_shared__cli,
                       "create signed short image in shared mem", "<name> <xsize> <ysize> <opt: zsize>",
                       "creasshortimshm imname 512 512",
                       "long create_image_ID(const char *name, long naxis, long *size, _DATATYPE_INT16, 0, 10, NULL)");

    RegisterCLIcommand("crea3dim", __FILE__, create_3Dimage_float, "creates 3D image, single precision",
                       "<name> <xsize> <ysize> <zsize>", "crea3dim imname 512 512 100",
...
{
    DEBUG_TRACE_FSTART();
    DEBUG_TRACEPOINT("FARG %s %ld %d %d %d %d", name, naxis, (int)datatype, shared, NBkw, CBsize);
    ...
    if (image_ID(name) == -1)
    {
        ...
        ImageStreamIO_createIm(&data.image[ID], name, naxis, size, datatype, shared, NBkw, CBsize);
    }
    else
    {
        ...
        for (int i = 0; i < naxis; i++)
            ...
            {
                FUNC_RETURN_FAILURE("Pre-existing image \"%s\" has wrong size: axis %d :  %ld  %ld", name, i,
...
errno_t create_1Dimage_ID(const char *ID_name, uint32_t xsize, imageID *outID)
{
    ...
    { // single precision
        FUNC_CHECK_RETURN(
            create_image_ID(ID_name, naxis, naxes, _DATATYPE_FLOAT, data.SHARED_DFT, data.NBKEYWORD_DFT, 0, &ID));
    }
    if (data.precision == 1)
    { // double precision
        FUNC_CHECK_RETURN(
...
errno_t create_1DCimage_ID(const char *ID_name, uint32_t xsize, imageID *outID)
{
    ...
    if (data.precision == 0)
    { // single precision
        FUNC_CHECK_RETURN(create_image_ID(ID_name, naxis, naxes, _DATATYPE_COMPLEX_FLOAT, data.SHARED_DFT,
                                          data.NBKEYWORD_DFT, 0, &ID));
    }
    if (data.precision == 1)
    { // double precision
        FUNC_CHECK_RETURN(create_image_ID(ID_name, naxis, naxes, _DATATYPE_COMPLEX_DOUBLE, data.SHARED_DFT,
                                          data.NBKEYWORD_DFT, 0, &ID));
    }
...
errno_t create_2Dimage_ID_double(const char *ID_name, uint32_t xsize, uint32_t ysize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
...
/* 2D complex image */
errno_t create_2DCimage_ID_double(const char *ID_name, uint32_t xsize, uint32_t ysize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
...
errno_t create_3Dimage_ID_float(const char *ID_name, uint32_t xsize, uint32_t ysize, uint32_t zsize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
...
errno_t create_3Dimage_ID_double(const char *ID_name, uint32_t xsize, uint32_t ysize, uint32_t zsize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
...
errno_t create_3Dimage_ID(const char *ID_name, uint32_t xsize, uint32_t ysize, uint32_t zsize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
...
/* 3D complex image */
errno_t create_3DCimage_ID(const char *ID_name, uint32_t xsize, uint32_t ysize, uint32_t zsize, imageID *outID)
{
    DEBUG_TRACE_FSTART();
