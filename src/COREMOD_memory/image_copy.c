...
{
    RegisterCLIcommand("cp", __FILE__, copy_image_ID__cli, "copy image", "source, dest",
                       "cp im1 im4",
...
*/
    ...
    RegisterCLIcommand("mv", __FILE__, chname_image_ID__cli, "change image name", "source, dest",
                       "mv im1 im4", "long chname_image_ID(const char *name, const char *newname)");

    RegisterCLIcommand("imcp2shm", __FILE__, COREMOD_MEMORY_cp2shm__cli,
                       "copy image ot shared memory", "<image> <shared mem image>",
                       "imcp2shm im1 ims1",
...
imageID copy_image_ID_IMGID(IMGID *imgin, IMGID *imgout, int shared)
{
    ...
    if (imgout->ID == -1)
    {
        create_image_ID(imgout->name, naxis, size, datatype, shared, NB_KEYWNODE_MAX, 0,
                        &imgout->ID);
