...
{
    RegisterCLIcommand("fpslist", __FILE__, fps_list, "list function parameter structures (FPSs)",
                       "no argument", "fpslist", "errno_t fps_list()");
    ...
    for (fpsID = 0; fpsID < data.NB_MAX_FPS; fpsID++)
    {
        if (data.fpsarray[fpsID].SMfd > -1)
        {
            ...
            printf("%*ld  %*s  %*ld/%*ld entries\n", NBchar_fpsID, fpsID, NBchar_fpsname,
                   data.fpsarray[fpsID].md[0].name, NBchar_NBparam,
                   data.fpsarray[fpsID].NBparamActive, NBchar_NBparam,
