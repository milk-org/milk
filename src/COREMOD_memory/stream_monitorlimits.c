...
errno_t stream_monitorlimits_addCLIcmd()
{
    ...
    RegisterCLIcommand("streammlim", __FILE__, COREMOD_MEMORY_stream_monitorlimits__cli,
                       "monitor stream values for safety", "FPS function", "streammlim",
...
errno_t stream_monitorlimits_FPCONF()
{
    ...
    long fpi_minON = 0;
    function_parameter_add_entry(&fps, ".minON", "min toggle", FPTYPE_ONOFF, FPFLAG, NULL, &fpi_minON);
    (void)fpi_minON;

    long fpi_minVal = 0;
    function_parameter_add_entry(&fps, ".minVal", "min value", FPTYPE_FLOAT32, FPFLAG, NULL, &fpi_minVal);
    (void)fpi_minVal;

    long fpi_maxON = 0;
    function_parameter_add_entry(&fps, ".maxON", "max toggle", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, NULL, &fpi_maxON);
    (void)fpi_maxON;

    long fpi_maxVal = 0;
    function_parameter_add_entry(&fps, ".minVal", "min value", FPTYPE_FLOAT32, FPFLAG, NULL, &fpi_maxVal);
    (void)fpi_maxVal;
...
errno_t stream_monitorlimits_RUN()
{
    ...
    strncpy(IDin_name, functionparameter_GetParamPtr_STRING(&fps, ".in_sname"), FUNCTION_PARAMETER_STRMAXLEN - 1);
