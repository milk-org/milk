...
imageID COREMOD_MEMORY_PixMapDecode_U(const char *inputstream_name, uint32_t xsizeim, uint32_t ysizeim,
...
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_STR_NOT_IMG) + CLI_checkarg(5, CLIARG_IMG) + CLI_checkarg(6, CLIARG_STR_NOT_IMG) +
...
{
    RegisterCLIcommand("impixdecodeU", __FILE__, COREMOD_MEMORY_PixMapDecode_U__cli, "decode image stream",
                       "<in stream> <xsize [long]> <ysize [long]> <nbpix per slice [ASCII file]> <decode map> <out "
                       ...
                       "COREMOD_MEMORY_PixMapDecode_U(const char *inputstream_name, uint32_t xsizeim, uint32_t "
                       "ysizeim, const char* NBpix_fname, const char* IDmap_name, const char *IDout_name, const char "
                                      ...
                                      const char *IDout_pixslice_fname, uint32_t reverse)
{
    ...
    char pinfodescr[200];
    sprintf(pinfodescr, "%ldx%ldx%ld->%ldx%ld", (long)xsizein, (long)ysizein, NBslice, (long)xsizeim, (long)ysizeim);
    char msgstring[200];
    ...
    sizearray[0] = xsizeim;
    sizearray[1] = ysizeim;
    create_image_ID(IDout_name, 2, sizearray, data.image[IDin].md[0].datatype, 1, 25, 0, &IDout);
