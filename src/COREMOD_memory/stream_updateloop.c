...
imageID COREMOD_MEMORY_image_streamupdateloop(const char *IDinname, const char *IDoutname, long usperiod, long NBcubes,
                                              long period, long offsetus, const char *IDsync_name, int semtrig,
                                              int timingmode);

imageID COREMOD_MEMORY_image_streamupdateloop_semtrig(const char *IDinname, const char *IDoutname, long period,
                                                      long offsetus, const char *IDsync_name, int semtrig,
                                                      int timingmode);
...
static errno_t COREMOD_MEMORY_image_streamburst__cli()
{
    ...
    {
        COREMOD_MEMORY_image_streamburst(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) + CLI_checkarg(5, CLIARG_LONG) + CLI_checkarg(6, CLIARG_LONG) +
            CLI_checkarg(7, CLIARG_STR) + CLI_checkarg(8, CLIARG_LONG) + CLI_checkarg(9, CLIARG_LONG) ==
        0)
    {
        COREMOD_MEMORY_image_streamupdateloop(
            ...
            data.cmdargtoken[7].val.string, data.cmdargtoken[8].val.numl, data.cmdargtoken[9].val.numl);
        return CLICMD_SUCCESS;
    }
    else
    {
...
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) + CLI_checkarg(5, CLIARG_STR) + CLI_checkarg(6, CLIARG_LONG) +
            CLI_checkarg(7, CLIARG_LONG) ==
        0)
    {
        COREMOD_MEMORY_image_streamupdateloop_semtrig(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    RegisterCLIcommand(
        "streamburst", __FILE__, COREMOD_MEMORY_image_streamburst__cli, "send burst of frames to stream",
        "<input cube> <output stream> <period[us]>", "streamburst inC outstream 1000",
        "errno_t COREMOD_MEMORY_image_streamburst(const char *IDin_name, const char *IDout_name, long periodus)");

    RegisterCLIcommand(
        "creaimstream", __FILE__, COREMOD_MEMORY_image_streamupdateloop__cli, "create 2D image stream from 3D cube",
        "<image3d in> <image2d out> <interval [us]> <NBcubes> <period> <offsetus> <sync stream name> <semtrig> <timing "
        "mode>",
        "creaimstream imcube imstream 1000 3 3 154 ircam1 3 0",
        "long COREMOD_MEMORY_image_streamupdateloop(const char *IDinname, const char *IDoutname, long usperiod, long "
        "NBcubes, long period, long offsetus, const char *IDsync_name, int semtrig, int timingmode)");

    RegisterCLIcommand(
        "creaimstreamstrig", __FILE__, COREMOD_MEMORY_image_streamupdateloop_semtrig__cli,
        ...
        "long COREMOD_MEMORY_image_streamupdateloop_semtrig(const char *IDinname, const char *IDoutname, long period, "
                                              ...
                                              __attribute__((unused)) int timingmode)
{
    ...
    while (loopOK == 1)
    {
        ...
        case _DATATYPE_INT8:
            ...
            ptr1 = (char *)data.image[IDout].array.SI8;
            framesize = data.image[IDin[cubeindex]].md[0].size[0] * data.image[IDin[cubeindex]].md[0].size[1] *
        ...
        case _DATATYPE_FLOAT:
            ...
            ptr1 = (char *)data.image[IDout].array.F;
            framesize =
                data.image[IDin[cubeindex]].md[0].size[0] * data.image[IDin[cubeindex]].md[0].size[1] * sizeof(float);
            break;
        ...
        case _DATATYPE_DOUBLE:
            ...
            ptr1 = (char *)data.image[IDout].array.D;
            framesize =
                data.image[IDin[cubeindex]].md[0].size[0] * data.image[IDin[cubeindex]].md[0].size[1] * sizeof(double);
            break;
        }
        ...
        if (loopCTRLexit == 1)
        {
            ...
            if (data.processinfo == 1)
            {
                ...
                sprintf(msgstring, "CTRLexit at %02d:%02d:%02d.%03d", tstoptm->tm_hour, tstoptm->tm_min,
