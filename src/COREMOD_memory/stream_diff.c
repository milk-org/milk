...
imageID COREMOD_MEMORY_streamDiff(const char *IDstream0_name, const char *IDstream1_name, const char *IDstreammask_name,
...
static errno_t COREMOD_MEMORY_streamDiff__cli()
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_IMG) + CLI_checkarg(3, 5) +
            CLI_checkarg(4, CLIARG_STR_NOT_IMG) + CLI_checkarg(5, CLIARG_LONG) ==
        0)
    {
...
errno_t stream_diff_addCLIcmd()
{
    RegisterCLIcommand("streamdiff", __FILE__, COREMOD_MEMORY_streamDiff__cli,
                       ...
                       "long COREMOD_MEMORY_streamDiff(const char *IDstream0_name, const char *IDstream1_name, const "
        ...
        else
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
                data.image[IDout].array.F[ii] =
                    (data.image[ID0].array.F[ii] - data.image[ID1].array.F[ii]) * data.image[IDmask].array.F[ii];
            }
        }
