...
static CLICMDARGDEF farg[] = {
    ...
    {CLIARG_STR, ".imim_name", "output real image", "outre", CLIARG_VISIBLE_DEFAULT, (void **)&outreimname, NULL},
    {CLIARG_STR, ".out_name", "output imaginary image", "outim", CLIARG_VISIBLE_DEFAULT, (void **)&outimimname, NULL}};
...
errno_t mk_reim_from_complex(const char *in_name, const char *re_name, const char *im_name, int sharedmem)
{
    ...
    {
        FUNC_CHECK_RETURN(
            create_image_ID(re_name, naxis, naxes, _DATATYPE_FLOAT, sharedmem, data.NBKEYWORD_DFT, 0, &IDre));

        FUNC_CHECK_RETURN(
            create_image_ID(im_name, naxis, naxes, _DATATYPE_FLOAT, sharedmem, data.NBKEYWORD_DFT, 0, &IDim));
...
#pragma omp for
    ...
    {
        FUNC_CHECK_RETURN(
            create_image_ID(re_name, naxis, naxes, _DATATYPE_DOUBLE, sharedmem, data.NBKEYWORD_DFT, 0, &IDre));

        FUNC_CHECK_RETURN(
            create_image_ID(im_name, naxis, naxes, _DATATYPE_DOUBLE, sharedmem, data.NBKEYWORD_DFT, 0, &IDim));
        data.image[IDre].md[0].write = 1;
        data.image[IDim].md[0].write = 1;
#ifdef _OPENMP
#pragma omp parallel if (nelement > OMP_NELEMENT_LIMIT)
