...
static CLICMDARGDEF farg[] = {
    {CLIARG_IMG, ".imamp_name", "amplitude image", "imamp", CLIARG_VISIBLE_DEFAULT, (void **)&inampimname, NULL},
    {CLIARG_IMG, ".impha_name", "phase image", "impha", CLIARG_VISIBLE_DEFAULT, (void **)&inphaimname, NULL},
    {CLIARG_STR, ".out_name", "output complex image", "outim", CLIARG_VISIBLE_DEFAULT, (void **)&outimname, NULL}};

static CLICMDDATA CLIcmddata = {"ap2c", "amplitude, phase -> complex", CLICMD_FIELDS_DEFAULTS};
...
errno_t mk_complex_from_amph(const char *am_name, const char *ph_name, const char *out_name, int sharedmem)
{
    ...
    if ((datatype_am == _DATATYPE_FLOAT) && (datatype_ph == _DATATYPE_FLOAT))
    {
        ...
        FUNC_CHECK_RETURN(
            create_image_ID(out_name, naxis, naxes, datatype_out, sharedmem, data.NBKEYWORD_DFT, 0, &IDout));
...
#pragma omp for
            ...
            for (uint64_t ii = 0; ii < nelement; ii++)
            {
                data.image[IDout].array.CF[ii].re =
                    data.image[IDam].array.F[ii] * ((float)cos(data.image[IDph].array.F[ii]));
                data.image[IDout].array.CF[ii].im =
                    data.image[IDam].array.F[ii] * ((float)sin(data.image[IDph].array.F[ii]));
            }
#ifdef _OPENMP
        }
#endif
