...
static CLICMDARGDEF farg[] = {
    {CLIARG_IMG, ".in_sname", "input stream name", "im1", CLIARG_VISIBLE_DEFAULT, (void **)&instreamname, NULL},
    {CLIARG_LONG, ".cubesize", "cube size", "10000", CLIARG_VISIBLE_DEFAULT, (void **)&logcubesize, NULL},
    {CLIARG_STR, ".logdir", "log directory", "/media/data", CLIARG_VISIBLE_DEFAULT, (void **)&logdir, NULL}};

// flag CLICMDFLAG_FPS enabled FPS capability
static CLICMDDATA CLIcmddata = {"shmimlog", "log shared memory stream", CLICMD_FIELDS_DEFAULTS};

// Forward declarations

static errno_t __attribute__((hot))
shmimlog2D(const char *IDname, uint32_t zsize, const char *logdir, const char *IDlogdata_name);
    ...
    while ((logshimconf[0].filecnt != NBfiles) && (logshimconf[0].logexit == 0))
    {
            ...
            if (likely(use_semlog == 1))
            {
                ...
                if (ret == -1)
                {
                    ...
                    if (errno == EINVAL)
                    {
                        ...
                        printf("               The value of abs_timeout.tv_nsecs is less than 0, or greater than or "
        ...
        if (likely(logshimconf[0].on == 1))
        {
            if (likely(wOK == 1)) // normal step: a frame has arrived
            {
                ...
                if (data.image[ID].md->CBsize > 0)
                {
                    ...
                    long NBframesbehind = (long)(CBcyclediff * (long)data.image[ID].md->CBsize + CBindexdiff);

                    if (VERBOSE > 0)
                    {
                        printf("    DIFF                [%8ld %3ld]    %8ld frames\n", CBcyclediff, CBindexdiff,
                               NBframesbehind);
                    }
                    ...
                    if (VERBOSE > 0)
                    {
                        ...
                        printf("     GRAB RANGE %3ld %3ld  (%3ld) ->  %6ld %6ld\n", grabStart0, grabEnd0, grabSpan0,
                               index0start, index0end);
                        printf("     GRAB RANGE %3ld %3ld  (%3ld) ->  %6ld %6ld\n", grabStart1, grabEnd1, grabSpan1,
        ...
        if ((index > zsize - 1) || ((timeout == 1) && (index > 0)))
        {
            ...
            sprintf(fnameascii, "%s/%s.%04d%02d%02dT%02d%02d%02ld.%09ldZ.txt", logdir, IDname,
                    1900 + uttimeStart->tm_year, 1 + uttimeStart->tm_mon, uttimeStart->tm_mday, uttimeStart->tm_hour,
            ...
            sprintf(fname, "%s/%s.%04d%02d%02dT%02d%02d%02ld.%09ldZ.fits", logdir, IDname, 1900 + uttimeStart->tm_year,
                    1 + uttimeStart->tm_mon, uttimeStart->tm_mday, uttimeStart->tm_hour, uttimeStart->tm_min,
            ...
            printf("=== %6ld %6ld  %6ld   %6ld ====\n", array_cnt0[0], array_cnt0[index - 1], index,
            ...
            tmsg->arraycnt1 = array_cnt1_cp;
            tmsg->arraytime = array_time_cp;
            WRITE_FULLFILENAME(tmsg->fname_auxFITSheader, "%s/%s.auxFITSheader.shm", data.shmdir, IDname);
