...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Compute a running average of image stream frames. Accumulates N "
                        "consecutive frames and writes the mean to the output stream. Useful for "
                        "background estimation and noise reduction."
};
...
#define FPS_PARAMS(X)                                                                              \
    X(".in_name", streamave_inimname, FPTYPE_STREAMNAME, 1,                                        \
      FPFLAG_DEFAULT_INPUT | FPFLAG_TRIGGER_STREAM, "input image")                                 \
    X(".outave_name", streamave_outimave, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT,              \
      "output average image")                                                                      \
    X(".out_shared", &streamave_outimshared, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,               \
      "output shared flag")                                                                        \
    X(".outrms_name", streamave_outimrms, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,              \
      "output RMS image")                                                                          \
    X(".NBcoadd", &streamave_NBcoadd, FPTYPE_UINT64, 1, FPFLAG_DEFAULT_INPUT, "number of coadded") \
    X(".cntindex", &streamave_cntindex, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_INPUT, "counter index")   \
    X(".comp.ave", &streamave_compave, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "compute average")   \
    X(".comp.rms", &streamave_comprms, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "compute rms")
...
#define STREAM_AVE_DISPATCH(DTYPE, ACC, CTYPE) \
    else if (imgin->md[0].datatype == DTYPE) STREAM_AVE_LOOP(CTYPE, ACC)
...
static MILK_HOT errno_t __attribute__((unused)) compute_function()
{
    ...
    if (streamave_comprms && strlen(streamave_outimrms) > 0)
    {
        IMGID outrms =
