...
static const DTYPE_ENTRY dtype_table[] = {
    { "uint8", _DATATYPE_UINT8 },   { "u8", _DATATYPE_UINT8 },      { "int8", _DATATYPE_INT8 },
    { "i8", _DATATYPE_INT8 },       { "uint16", _DATATYPE_UINT16 }, { "u16", _DATATYPE_UINT16 },
    { "int16", _DATATYPE_INT16 },   { "i16", _DATATYPE_INT16 },     { "uint32", _DATATYPE_UINT32 },
    { "u32", _DATATYPE_UINT32 },    { "int32", _DATATYPE_INT32 },   { "i32", _DATATYPE_INT32 },
    { "uint64", _DATATYPE_UINT64 }, { "u64", _DATATYPE_UINT64 },    { "int64", _DATATYPE_INT64 },
    { "i64", _DATATYPE_INT64 },     { "float", _DATATYPE_FLOAT },   { "f32", _DATATYPE_FLOAT },
    { "double", _DATATYPE_DOUBLE }, { "f64", _DATATYPE_DOUBLE },    { NULL, 0 }
};
...
{
    printf("\n" C_HDR "Usage:" C_RST " %s " C_OPT "[options]" C_RST " " C_ARG "<name>" C_RST
    ...
    printf("  " C_ARG "ysize" C_RST "      Height (optional,"
           " makes 2D)\n");
    printf("  " C_ARG "zsize" C_RST "      Depth (optional,"
           " makes 3D / circ buffer)\n\n");
    printf(C_HDR "Options:" C_RST "\n");
    printf("  " C_OPT "-t, --type" C_RST " " C_ARG "TYPE" C_RST "   Data type"
           " (default: float)\n");
    printf("  " C_OPT "-k, --kw" C_RST " " C_ARG "N" C_RST "       Number of keywords"
           " (default: 10)\n");
    ...
    printf("  " C_CMD "float" C_RST "/f32"
           "     " C_CMD "double" C_RST "/f64\n\n");
    printf(C_HDR "Examples:" C_RST "\n");
    printf("  $ " C_CMD "%s" C_RST " " C_ARG "wfs" C_RST " 128 128\n", progname);
    printf("  $ " C_CMD "%s" C_RST " " C_OPT "-t uint16" C_RST " " C_ARG "cam" C_RST " 512 512\n",
           progname);
    printf("  $ " C_CMD "%s" C_RST " " C_OPT "-t double" C_RST " " C_ARG "signal" C_RST " 1024\n\n",
           progname);
}
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "t:k:h", long_options, NULL)) != -1)
    {
        switch (opt)
        {
        case 't':
