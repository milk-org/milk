...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-a, --all", mh_color ? MH_RST : "",
                ...
                if (lstat(fullname, &buf) == 0)
                {
                    if (S_ISLNK(buf.st_mode))
                    {
                        ...
                        if (len != -1)
                        {
                            ...
                            printf(C_LINK "%-30s" C_RST " " C_DIM "%-12s" C_RST " -> ", sname,
                                   "LINK");
                            if (!target_exists)
                            {
                        ...
                        {
                            printf(C_LINK "%-30s" C_RST " " C_ERR "%-12s" C_RST "\n", sname,
                                   "LINK (err)");
                        }
                    }
                    else
                    {
                        ...
                        if (ret == IMAGESTREAMIO_SUCCESS)
                        {
                            ...
                            {
                                snprintf(size_str, 32, "%u x %u", image.md->size[0],
                                         image.md->size[1]);
                            }
                            else
                            {
                                snprintf(size_str, 32, "%u x %u x %u", image.md->size[0],
                                         image.md->size[1], image.md->size[2]);
                            }

                            printf(C_NAME "%-30s" C_RST " " C_TYPE "%-12s" C_RST " " C_SIZE
                                          "%-20s" C_RST " " C_CNT "%-12lu" C_RST,
                                   sname, typestr ? typestr : "???", size_str,
                        ...
                        {
                            printf(C_NAME "%-30s" C_RST " " C_ERR "%-12s" C_RST "\n", sname,
                                   "ERROR_OPEN");
                        }
                    }
                }
