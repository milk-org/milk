...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Construct a complex image from separate amplitude and phase images. "
                        "Computes real = amp * cos(pha), imag = amp * sin(pha)."
};
...
#define FPS_PARAMS(X)                                                                            \
    X(".imamp_name", inampimname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "amplitude image") \
    X(".impha_name", inphaimname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "phase image")     \
    X(".out_name", outimname, FPTYPE_STRING, 1, FPFLAG_DEFAULT_INPUT, "output complex image")
...
#define MK_COMPLEX_LOOP(DTYPE_OUT, TYPE_AM, TYPE_PH, TYPE_OUT, UNION_AM, UNION_PH, UNION_OUT, \
                        COS_FUNC, SIN_FUNC)                                                   \
    {                                                                                         \
        ...
        {                                                                                     \
            _Pragma("omp for simd") for (uint32_t kk = 0; kk < zsize; kk++)                   \
            {                                                                                 \
                uint32_t kkamp = kk;                                                          \
                if (kkamp > zsizeamp - 1)                                                     \
                    kkamp = zsizeamp - 1;                                                     \
    ...
    {
        MK_COMPLEX_LOOP(_DATATYPE_COMPLEX_DOUBLE, double, float, complex_double, D, F, CD, cosf,
                        sinf)
    }
    else if ((datatype_am == _DATATYPE_DOUBLE) && (datatype_ph == _DATATYPE_DOUBLE))
    {
        MK_COMPLEX_LOOP(_DATATYPE_COMPLEX_DOUBLE, double, double, complex_double, D, D, CD, cos,
                        sin)
    }
