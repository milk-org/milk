...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description = "log stream to FITS file(s)",
    .description_long =
        "Log shared memory image stream frames to disk as FITS files. Records every new frame "
        "triggered by semaphore posting, with configurable logging duration and file naming."
};
...
#define FPS_PARAMS(X)                                                                           \
    X(".sname", streamname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "stream image")         \
    X(".saveON", &saveON, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "toggle save on/off")          \
    X(".lastcubeON", &lastcubeON, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                        \
      "toggle last cube on/off")                                                                \
    X(".nextcube", &nextcube, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "force jump to next cube") \
    X(".cubesize", &cubesize, FPTYPE_UINT32, 1, (FPFLAG_DEFAULT_INPUT),                         \
      "cube size, nb frame per cube")                                                           \
    X(".dirname", savedirname, FPTYPE_STRING, 1, (FPFLAG_DEFAULT_INPUT), "log directory")       \
    X(".frameindex", &frameindex, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_OUTPUT,                      \
      "frame index within cube")                                                                \
    X(".framecnt", &framecnt, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_OUTPUT,                          \
      "frame counter since started")                                                            \
    X(".maxframecnt", &maxframecnt, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_INPUT, "max frame count")  \
    X(".filecnt", &filecnt, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_OUTPUT, "file counter")            \
    X(".maxfilecnt", &maxfilecnt, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_INPUT, "max file counter")   \
    X(".outfname", outfname, FPTYPE_STRING, 0, FPFLAG_DEFAULT_OUTPUT, "output file name")       \
    X(".compress", &compressON, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                          \
      "toggle compression on/off")                                                              \
    X(".savetime", &savetime, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT, "time taken to save")   \
    X(".writerRTprio", &writerRTprio, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                   \
...
static MILK_COLD errno_t __attribute__((unused)) customCONFsetup()
{
    if (dcfpsptr != NULL)
    {
        ...
        if (fpi >= 0)
        {
            dcfpsptr->parray[fpi].fpflag |= FPFLAG_WRITERUN;
        }
 ...
 * logshmim_save.c */
extern void *save_telemetry_fits_function(void *ptr);
...
static MILK_HOT errno_t __attribute__((unused)) compute_function()
{
    ...
    if (inimg.md->naxis == 3)
    {
        PRINT_ERROR("streamFITSlog with 3D data"
                    " is NOT supported");
    }
     ...
     * need direct FPS manipulation */
        ...
        INSERT_STD_PROCINFO_COMPUTEFUNC_LOOPSTART
        {
            ...
            else
            {
                ...
                if (IsNewFrame == 1)
                {
                    ...
                    if (framecnt >= maxframecnt)
                    {
                        ...
                        if (fpi_saveON >= 0)
                        {
                            dcfpsptr->parray[fpi_saveON].fpflag &= ~FPFLAG_ONOFF;
                        }
                    }
                    ...
                    if (saveON == 1)
                    {
                        if (frameindex == 0)
                        {
                            ...
                            char hrminstring[6];
                            snprintf(hrminstring, sizeof(hrminstring), "%02d:%02d",
                                     uttimeStart->tm_hour, uttimeStart->tm_min);
                            if (VERBOSE > 0)
                            {
                                printf("hrmin: %s\n", hrminstring);
                            }
            ...
            if (nextcube == 1)
            {
                ...
                if (fpi_nextcube >= 0)
                {
                    dcfpsptr->parray[fpi_nextcube].fpflag &= ~FPFLAG_ONOFF;
                }
                SaveCube = 1;
            }
            ...
            if (SaveCube == 1)
            {
                if (frameindex > 0)
                {
                    ...
                    if (buffindex == 0)
                    {
                        __builtin_memcpy(imgbuff0.im->kw, inimg.im->kw,
                                         sizeof(IMAGE_KEYWORD) * inimg.md->NBkw);
                    }
                    else
                    {
                        __builtin_memcpy(imgbuff1.im->kw, inimg.im->kw,
                                         sizeof(IMAGE_KEYWORD) * inimg.md->NBkw);
                    }

                    {
                        ...
                        snprintf(tmsg->fnameascii, sizeof(tmsg->fnameascii), "%s",
                                 ASCIITIMEffilename);
                        tmsg->saveascii = 1;
                        ...
                        snprintf(tmsg->fname_auxFITSheader, sizeof(tmsg->fname_auxFITSheader),
                                 "%s/%s.aux.fits", dcshmdir, streamname);
                        ...
                        {
                            snprintf(tmsg->compress_string, sizeof(tmsg->compress_string),
                                     "[compress R"
                                     " 1,1,10000]");
                        }
                        ...
                        iret_savefits      = pthread_create(&thread_savefits, NULL,
                                                            save_telemetry_fits_function, tmsg);
                ...
                if ((lastcube == 1) || (lastcubeON == 1))
                {
                    ...
                    if (fpi_lastcubeON >= 0)
                    {
                        dcfpsptr->parray[fpi_lastcubeON].fpflag &= ~FPFLAG_ONOFF;
                    }
                }
            }
