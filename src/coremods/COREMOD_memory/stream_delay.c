...
#define FPS_PARAMS(X) \
    ...
    X(".delaysec", &delaysec, \
      ...
      FPFLAG_DEFAULT_INPUT, \
      "delay [s]") \
    X(".timebuffsize", &timebuffsize, \
      FPTYPE_UINT64, 0, \
      FPFLAG_DEFAULT_INPUT, \
      "time buffer size") \
    X(".option.timeavemode", &avemode, \
      FPTYPE_INT32, 0, \
      FPFLAG_DEFAULT_INPUT, \
      "Enable time window averaging (>0)") \
    X(".option.timeavedtns", &avedtns, \
      FPTYPE_UINT64, 0, \
      FPFLAG_DEFAULT_INPUT, \
      "Averaging time window width [ns]") \
    X(".status.framelag", &statusframelag, \
      FPTYPE_UINT64, 0, \
      FPFLAG_DEFAULT_OUTPUT, \
      "current time lag frame index") \
    X(".status.kkin", &statuskkin, \
      FPTYPE_UINT64, 0, \
      FPFLAG_DEFAULT_OUTPUT, \
      "input cube slice index") \
    X(".status.kkout", &statuskkout, \
      FPTYPE_UINT64, 0, \
      FPFLAG_DEFAULT_OUTPUT, \
