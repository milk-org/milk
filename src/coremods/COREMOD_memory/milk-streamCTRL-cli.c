...
static int scan_streams(const char   *shmdir,
                        ...
                        int           use_regex)
{
    ...
    while ((de = readdir(d)) && n < max_entries)
    {
        ...
        if (suffix_pos != (int) strlen(de->d_name) - 7)
        {
            continue;
        }
        ...
        if (!entries[n].is_link || !entries[n].link_broken)
        {
            IMAGE   im = { 0 };
            errno_t r  = ImageStreamIO_read_sharedmem_image_toIMAGE(sname, &im);
            if (r == IMAGESTREAMIO_SUCCESS && im.md)
            {
                entries[n].open_ok  = 1;
...
static void print_stream(const stream_entry *e)
{
    if (e->is_link)
    {
        if (e->link_broken)
        {
            printf(C_LINK "%-28s" C_RST " " C_ERR "LINK -> %s (broken)" C_RST "\n", e->sname,
                   e->link_target);
        }
        else
        {
    ...
    printf(C_NAME "%-28s" C_RST " " C_TYPE "%-10s" C_RST " " C_SIZE "%-18s" C_RST " " C_CNT
                  "%12lu" C_RST,
...
{
    milk_help_banner(progname, "command-line interface for monitoring shared memory streams",
                     mh_color);
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-n, --interval SEC", mh_color ? MH_RST : "",
           "Refresh interval (default 1.0)");
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-1, --once", mh_color ? MH_RST : "",
           "Print once and exit");
    ...
    const char *see_also[] = { "milk-streamCTRL:launch the stream control dashboard TUI",
...
int main(int argc, char *argv[])
{
    ...
    int action = milk_help_init(
        argc, argv, "command-line interface for monitoring shared memory streams",
    ...
    static struct option long_opts[] = { { "interval", required_argument, 0, 'n' },
                                         { "once", no_argument, 0, '1' },
                                         { "help", no_argument, 0, 'h' },
                                         { 0, 0, 0, 0 } };

    int opt;
    while ((opt = getopt_long(argc, argv, "n:1h", long_opts, NULL)) != -1)
    {
        switch (opt)
        {
        case 'n':
    ...
    while (keep_running)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                if (strcmp(entries[i].sname, prev[j].sname) == 0)
                {
        ...
        printf(C_TITLE "milk-streamCTRL-cli" C_RST " | " C_DIM "%s" C_RST " | " C_DIM
                       "%d streams" C_RST "\n\n",
               shmdir, n);
        print_header();
        for (int i = 0; i < n; i++)
