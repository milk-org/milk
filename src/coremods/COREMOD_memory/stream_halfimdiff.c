...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description = "half-image difference",
    .description_long =
        "Compute the difference between the left and right halves of an image stream. Produces a "
        "half-width output. Used for differential measurements in optical systems."
};
...
#define HALFDIFF_TYPES(X)                                               \
    X(_DATATYPE_UINT8, UI8, uint8_t, _DATATYPE_INT16, SI16, int16_t)    \
    X(_DATATYPE_INT8, SI8, int8_t, _DATATYPE_INT16, SI16, int16_t)      \
    X(_DATATYPE_UINT16, UI16, uint16_t, _DATATYPE_INT32, SI32, int32_t) \
    X(_DATATYPE_INT16, SI16, int16_t, _DATATYPE_INT32, SI32, int32_t)   \
    X(_DATATYPE_UINT32, UI32, uint32_t, _DATATYPE_INT64, SI64, int64_t) \
    X(_DATATYPE_INT32, SI32, int32_t, _DATATYPE_INT64, SI64, int64_t)   \
    X(_DATATYPE_UINT64, UI64, uint64_t, _DATATYPE_INT64, SI64, int64_t) \
    X(_DATATYPE_INT64, SI64, int64_t, _DATATYPE_INT64, SI64, int64_t)   \
    X(_DATATYPE_FLOAT, F, float, _DATATYPE_FLOAT, F, float)             \
    X(_DATATYPE_DOUBLE, D, double, _DATATYPE_DOUBLE, D, double)

/**
 * Compute difference between two halves of an
 * image stream. Triggers on instream.
 */
imageID MILK_HOT COREMOD_MEMORY_stream_halfimDiff(const char *IDstream_name,
...
#define HALFDIFF_OUTTYPE(DT_IN, IACC, ICT, DT_OUT, OACC, OCT) \
    case DT_IN:                                               \
        datatypeout = DT_OUT;                                 \
        break;
...
#undef HALFDIFF_OUTTYPE
    ...
    while (1)
    {
         ...
         */
#define HALFDIFF_CASE(DT_IN, IACC, ICT, DT_OUT, OACC, OCT)                    \
    case DT_IN:                                                               \
    {                                                                         \
        ICT *MILK_RESTRICT pin  = MILK_ASSUME_ALIGNED(img0.im->array.IACC);   \
        OCT *MILK_RESTRICT pout = MILK_ASSUME_ALIGNED(imgout.im->array.OACC); \
        for (uint64_t ii = 0; ii < xysize; ii++)                              \
            pout[ii] = (OCT) pin[ii] - (OCT) pin[xysize + ii];                \
        break;                                                                \
    }
