...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description = "transmit image over UDP network",
    .description_long =
        "Transmit or receive image stream data over UDP for low-latency network streaming. "
        "Suitable for real-time telemetry where occasional packet loss is acceptable."
};
...
static FPS_APP_INFO FPS_app_info_rx = {
    ...
    .cmdkey      = "imudpreceive",
    .description = "receive image(s) over UDP "
                   "network",
    .description_long =
        "Transmit or receive image stream data over UDP for low-latency network streaming. "
        "Suitable for real-time telemetry where occasional packet loss is acceptable."
                                            ...
                                            int         RT_priority)
{
    ...
    int use_sem =
        stream_net_decide_sync(dcimg[ID].md[0].sem, do_counter_sync, semtrig, processinfo);
    ...
    while (loopOK == 1)
    {
        ...
        if (processinfo_compute_status(processinfo) == 1)
        {
            if (semr == 0)
            {
                ...
                if (TCPTRANSFERKW == 1)
                {
                    __builtin_memcpy(ptr_buff_keywords, (char *) dcimg[ID].kw,
                ...
                for (int dgram = 0; dgram < n_udp_dgrams; ++dgram)
                {
                    long this_dgram_size =
                        dgram == n_udp_dgrams - 1 ? last_dgram_chunk + 2 : DGRAM_CHUNK_SIZE + 2;
                ...
                if (byte_sock_count != framesizeall + 2 * n_udp_dgrams)
                {
                    ...
                    snprintf(errmsg, 200,
                             ...
                             "expected %ld",
                             byte_sock_count, framesizeall + 2 * n_udp_dgrams);
                    printf("%s\n", errmsg);
                    fflush(stdout);
