...
static FPS_APP_INFO FPS_app_info = {
    .fps_name    = "streamdiff",
    .cmdkey      = "streamdiff",
    .description = "compute stream difference",
    .description_long =
        "Compute the pixel-wise difference between two image streams in real-time. Output stream = "
        "stream_A - stream_B. Operates as a continuous stream processor triggered by semaphore."
};
...
#define FPS_PARAMS(X)                                                                           \
    ...
    X(".mask", p_mask, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "optional mask (null=none)") \
 ...
 * Triggers on stream0.
 */
imageID MILK_HOT COREMOD_MEMORY_streamDiff(const char *IDstream0_name,
                                           ...
                                           long        semtrig)
{
    ...
    while (1)
    {
        ...
        if (ptrm == NULL)
        {
            MILK_IVDEP for (uint64_t ii = 0; ii < xysize; ii++)
            {
