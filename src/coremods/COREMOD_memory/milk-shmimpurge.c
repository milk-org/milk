...
#define SP_DESC "purge orphan shared memory streams and files"
...
static void print_help(const char *progname, int mh_color)
{
    ...
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-f, --filter <str>", mh_color ? MH_RST : "",
           "Only consider streams whose name contains <str>");
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-y, --force", mh_color ? MH_RST : "",
           "Skip confirmation prompt");
    printf("  %s%-25s%s %s\n", mh_color ? MH_OPT : "", "-n, --dry-run", mh_color ? MH_RST : "",
    ...
    printf("  %s$ milk-shmimpurge%s %s-n%s\n\n", mh_color ? MH_CMD : "", mh_color ? MH_RST : "",
           mh_color ? MH_OPT : "", mh_color ? MH_RST : "");
    const char *see_also[] = { "milk-stream-rm:remove shared memory streams",
...
int main(int argc, char *argv[])
{
    ...
    {
        if ((strcmp(argv[i], "-f") == 0 || strcmp(argv[i], "--filter") == 0) && i + 1 < argc)
        {
            filter = argv[++i];
        }
        else if (strcmp(argv[i], "-y") == 0 || strcmp(argv[i], "--force") == 0)
        {
            force = 1;
        }
    ...
    while ((de = readdir(d)) != NULL && total < SP_MAX_STREAMS)
    {
        ...
        if (filter != NULL && strstr(de->d_name, filter) == NULL)
        {
            continue;
        }
