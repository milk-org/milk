...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Reshape an image by merging one axis into another. For example, unfold a "
                        "3D cube (x, y, z) into a 2D image by merging z into y, producing "
                        "dimensions (x, y*z). The total pixel count is preserved."
};
...
#define FPS_PARAMS(X)                                                                   \
    ...
    X(".axisA", &axisA, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT, "axis to merged")       \
    X(".axisB", &axisB, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT, "merge into this axis") \
    X(".colsize", &colsize, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT, "column size")
