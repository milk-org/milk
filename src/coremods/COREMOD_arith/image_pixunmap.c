...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long =
        "Reverse-map a 2D image to a 1D array using an index map. Each pixel in the input image is "
        "placed at the position specified by the unmap table. Inverse operation of pixremap."
};
...
#define UNMAP_CASE_(DT, IACC, CT, OACC)                     \
    case DT:                                                \
        ...
        {                                                   \
            imgout.im->array.OACC[map_1Dpixindex[pixi]] +=  \
                imgin.im->array.IACC[map_2Dpixindex[pixi]]; \
        }                                                   \
        for (uint32_t ii = 0; ii < x1Dsize; ii++)           \
        {                                                   \
            imgout.im->array.OACC[ii] /= map_pixcnt[ii];    \
        }                                                   \
        break;
