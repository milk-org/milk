...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Multiply each slice of a 3D image cube by the corresponding element of a "
                        "1D vector. Applies element-wise scaling across the z-axis. Useful for "
                        "applying modal coefficients to a mode cube."
};
...
#define FPS_PARAMS(X)                                                                        \
    X(".iminname", iminname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image name") \
    X(".vecname", vecname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input vector name")  \
...
errno_t image_vect_multiply(IMGID imgin, IMGID imgvec, IMGID *imgout, uint32_t multaxis)
{
    DEBUG_TRACE_FSTART();
    ...
    if (multaxis == 0)
    {
        _Pragma("omp parallel for") for (uint64_t kk = 0; kk < zsize; kk++)
        {
            for (uint64_t jj = 0; jj < ysize; jj++)
            {
                uint64_t offset = kk * xysize + jj * xsize;
                _Pragma("omp simd") for (uint64_t ii = 0; ii < xsize; ii++)
                {
...
errno_t CLIADDCMD_COREMODE_arith__image_vecmult()
{
