...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Normalize each slice of a 3D image cube using a mask. For each slice, "
                        "compute the weighted mean over the mask region and divide. Produces a "
                        "cube where each slice has unit mean within the mask."
};
...
#define FPS_PARAMS(X)                                                                        \
    X(".in0name", inimname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image 0")     \
    X(".maskim", maskimname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image mask") \
    X(".outname", outimname, FPTYPE_STRING, 1, FPFLAG_DEFAULT_INPUT, "output image")         \
    X(".axis", &sliceaxis, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT, "norm axis")              \
    X(".auxin", auxin, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT,                           \
      "auxillary input image, in-place update")                                              \
    X(".RMS", &modeRMS, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "output RMS=1 over mask")

static errno_t image_slicenormalize_core(IMGID   inimg,
                                         IMGID   maskimg,
                                         IMGID  *outimg,
                                         uint8_t sliceaxis,
                                         IMGID   imgaux,
...
#define MASKMUL_(DT, ACC, CT)                                          \
    case DT:                                                           \
        valm = maskimg.im->array.F[pixim] * inimg.im->array.ACC[pixi]; \
        break;
...
#define NORMDIV_(DT, ACC, CT)                                                   \
    case DT:                                                                    \
        outimg->im->array.F[pixi] =                                             \
            (1.0 * inimg.im->array.ACC[pixi]) / normarray[pixcoord[sliceaxis]]; \
        break;
...
static MILK_HOT errno_t __attribute__((unused)) compute_function()
{
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_LOOPSTART
    {
        ...
        if (inimg.ID != -1)
        {
            ...
            image_slicenormalize_core(inimg, maskimg, &outimg, sliceaxis, imgaux, modeRMS,
                                      normarray, avarray, maskcntarray);
        }
