...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Save a shared memory image stream to a FITS file on disk. Preserves "
                        "keywords, data type, and multi-dimensional structure."
};
...
#define FPS_PARAMS(X)                                                                            \
    X(".in_name", &savefits_inimname, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "input image") \
    X(".out_fname", &savefits_outfname, FPTYPE_FILENAME, 1, FPFLAG_DEFAULT_INPUT,                \
      "output FITS file")                                                                        \
    X(".bitpix", &savefits_outbitpix, FPTYPE_INT32, 0, FPFLAG_DEFAULT_INPUT, "FITS bitpix")      \
    X(".in_header", &savefits_inheader, FPTYPE_FILENAME, 0, FPFLAG_DEFAULT_INPUT,                \
      "header import file")
                                 ...
                                 const char    *FITSIOext)
{
    ...
    int bitpix =
        (outputbitpix != 0) ? outputbitpix : ImageStreamIO_FITSIObitpix(imgin->md->datatype);
    if (bitpix == -1)
    {
        bitpix = FLOAT_IMG;
    }
    ...
    fits_write_img(fptr, ImageStreamIO_FITSIOdatatype(imgin->md->datatype), 1, nelements,
                   imgin->im->array.raw, &COREMOD_iofits_data.FITSIO_status);
 ...
 */
errno_t saveFITS_opt_trunc(const char    *inputimname,
                           int            truncate,
                           const char    *outputFITSname,
                           int            outputbitpix,
                           ...
                           const char    *FITSIOext)
{
    ...
    return saveFITS_opt_trunc_IMGID(&id, truncate, outputFITSname, outputbitpix, importheaderfile,
 ...
 */
errno_t saveFITS(const char    *inputimname,
                 const char    *outputFITSname,
                 int            outputbitpix,
                 const char    *importheaderfile,
                 IMAGE_KEYWORD *kwarray,
                 int            kwarraysize)
{
    return saveFITS_opt_trunc(inputimname, -1, outputFITSname, outputbitpix, importheaderfile,
                              kwarray, kwarraysize, "");
}
...
static MILK_HOT errno_t fpsexec(IMGID *imgin)
{
    ...
    return saveFITS_opt_trunc_IMGID(imgin, -1, savefits_outfname, savefits_outbitpix,
                                    savefits_inheader[0] ? savefits_inheader : NULL, NULL, 0, "");
}
