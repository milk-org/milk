...
static FPS_APP_INFO FPS_app_info = {
    .fps_name         = "ctsmstats",
    .cmdkey           = "ctsmstats",
    .description      = "monitors shared variable status",
    .description_long = "Collect and display statistics about process status values written to "
                        "shared memory streams. Histograms the status values over time."
};
...
errno_t CLIADDCMD_COREMOD_tools__statusstat()
{
...
imageID COREMOD_TOOLS_statusStat(const char *IDstat_name, long indexmax)
{
    ...
    for (unsigned short st = 0; st < indexmax; st++)
    {
...

    {
        ...
        for (long long k = 0; k < NBkiter; k++)
        {
            ...
            if (tdiffv > tdiffv1)
            {
                ...
                printf("\n");
                long cnttot = 0;
                for (unsigned short st_idx = 0; st_idx < indexmax; st_idx++)
                {
                ...
                for (unsigned short st_idx = 0; st_idx < indexmax; st_idx++)
                {
                    printf("STATUS  %5d"
                           "    %20ld"
                           "   %6.3f  \n",
                           st_idx, imgout.im->array.SI64[st_idx],
                           100.0 * imgout.im->array.SI64[st_idx] / cnttot);
                }
            }
        }
    }
