...
int main(int argc, char *argv[])
{
    ...
    /* Compile regex */
    regex_t regex;
    if (regcomp(&regex, regex_pattern,
    ...
    /* Connect to Valkey */
    FPS_VALKEY_CTX vctx;
    if (fps_valkey_connect(&vctx,
            valkey_host, valkey_port) != 0)
    {
        fprintf(stderr,
    ...
    KEYWORD_TREE_NODE *keywnode =
        (KEYWORD_TREE_NODE *) calloc(
            NB_KEYWNODE_MAX,
            sizeof(KEYWORD_TREE_NODE));

    track_list = (FPS_TRACK *) calloc(
        NB_FPS_MAX, sizeof(FPS_TRACK));
    ...
    while (keep_running)
    {
        ...
        for (int i = 0; i < NBfps; i++)
        {
            ...
            {
                if (strcmp(track_list[j].name,
                        fpsarray[i].md->name) == 0)
                {
                    tidx = j;
                    break;
                }
            ...
            if (tidx == -1)
            {
                ...
                /* Register in Valkey */
                fps_valkey_register_fps(
                    &vctx, track_list[tidx].name);

                /* Init + push all active params */
                for (int p = 0;
                     p < fpsarray[i].md->NBparamMAX;
                     p++)
                {
                    if (fpsarray[i].parray[p].fpflag
                        & FPFLAG_ACTIVE)
                    {
                        strncpy(
                            track_list[tidx]
                 ...
                 p++)
            {
                           ...
                           .params[p].cnt0)
                {
                    ...
                    const char *tn = get_type_name(
                        fpsarray[i].parray[p].type);
        ...
        for (int j = 0; j < track_list_cnt; j++)
        {
                ...
                && track_list[j].name[0] != '\0')
            {
                ...
                fps_valkey_unregister_fps(
                    &vctx, track_list[j].name);
