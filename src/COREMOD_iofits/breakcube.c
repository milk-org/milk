...
errno_t breakcube_addCLIcmd()
{
    ...
    RegisterCLIcommand("breakcube", __FILE__, break_cube_cli, "break cube into individual images (slices)",
                       "<input image>", "breakcube imc", "int break_cube(char *ID_name)");
        ...
        for (uint32_t ii = 0; ii < naxes[0]; ii++)
            for (uint32_t jj = 0; jj < naxes[1]; jj++)
            {
                data.image[ID1].array.F[jj * naxes[0] + ii] =
                    data.image[ID].array.F[kk * naxes[0] * naxes[1] + jj * naxes[0] + ii];
            }
    }
