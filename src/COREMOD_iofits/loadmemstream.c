            ...
            if (ID == -1)
            {
                *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE; // fail
                if (MEMLOADREPORT == 1)
                {
                    char msg[STRINGMAXLEN_FPS_LOGMSG];
                    SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                                   "%s EXITFAIL STREAM_LOAD_FORCE_CONFFITS: Image does not exist as conf FITS", sname);
                    functionparameter_outlog("LOADMEMSTREAM", msg);
                }
            }
            else
                    ...
                    else
                    {
                        ...
                        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE; // fail
                        if (MEMLOADREPORT == 1)
                        {
                            char msg[STRINGMAXLEN_FPS_LOGMSG];
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                                           "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: fscanf reached end of file, no "
                                           "matching characters",
                                           sname);
                            functionparameter_outlog("LOADMEMSTREAM", msg);
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname, *imLOC);
                ...
                {
                    {
                        ...
                        if (ID == -1)
                        {
                            *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE; // fail
                            if (MEMLOADREPORT == 1)
                            {
                                char msg[STRINGMAXLEN_FPS_LOGMSG];
                                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                                               "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: cannot load stream fname",
                                               sname);
                                functionparameter_outlog("LOADMEMSTREAM", msg);
                                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname, *imLOC);
                                functionparameter_outlog("LOADMEMSTREAM", msg);
                        ...
                        else
                        {
                            *imLOC = STREAM_LOAD_SOURCE_CONFNAME;
                            ...
                            {
                                char msg[STRINGMAXLEN_FPS_LOGMSG];
                                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s SUCCESS STREAM_LOAD_FORCE_CONFFITS",
                                               sname);
                                functionparameter_outlog("LOADMEMSTREAM", msg);
                                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname, *imLOC);
                                functionparameter_outlog("LOADMEMSTREAM", msg);
                ...
                if (fscanfcnt == EOF)
                {
                    if (ferror(fp))
                    {
                        ...
                        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE; // fail
                        ...
                        {
                            char msg[STRINGMAXLEN_FPS_LOGMSG];
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s EXITFAILURE fscanf error reading %s",
                                           sname, fname);
                            functionparameter_outlog("LOADMEMSTREAM", msg);
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname, *imLOC);
                            functionparameter_outlog("LOADMEMSTREAM", msg);
                    ...
                    else
                    {
                        ...
                        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE; // fail
                        if (MEMLOADREPORT == 1)
                        {
                            char msg[STRINGMAXLEN_FPS_LOGMSG];
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                                           "%s EXITFAILURE fscanf error reading %s. fscanf reached end of file, no "
                                           "matching characters",
                                           sname, fname);
                            functionparameter_outlog("LOADMEMSTREAM", msg);
                            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname, *imLOC);
