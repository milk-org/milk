...
static CLICMDARGDEF farg[] = {
    {CLIARG_STR, ".infname", "input file", "imfname", CLIARG_VISIBLE_DEFAULT, (void **)&infilename, NULL},
    {CLIARG_STR_NOT_IMG, ".outimname", "output image name", "outimname", CLIARG_VISIBLE_DEFAULT, (void **)&outimname,
     NULL},
    {CLIARG_LONG, ".errmode", "FITSIO errors mode \n(0:ignore) (1:warning) (2:error) (3:exit)", "1",
     CLIARG_HIDDEN_DEFAULT, (void **)&FITSIOerrmode, NULL}};

// CLI function initialization data
static CLICMDDATA CLIcmddata = {"loadfits", "load FITS format file", CLICMD_FIELDS_DEFAULTS};
...
errno_t load_fits(const char *restrict file_name, const char *restrict ID_name, int errmode, imageID *IDout)
{
    DEBUG_TRACE_FSTART();
...

    {
        ...
        if (fileOK == 0)
        {
            ...
            if (errmode == 1)
            {
                PRINT_WARNING("Image \"%s\" could not be loaded from file \"%s\"", ID_name, file_name);
                DEBUG_TRACE_FEXIT();
                return RETURN_SUCCESS;
            }
    ...
    if (bitpix == -32)
    {
        ...
        {
            int status = 0;
            fits_read_img(fptr, data_type_code(bitpix), fpixel, nelements, &nulval, data.image[ID].array.F, &anynul,
                          &status);
    ...
    if (bitpix == -64)
    {
        ...
        {
            int status = 0;
            fits_read_img(fptr, data_type_code(bitpix), fpixel, nelements, &nulval, data.image[ID].array.D, &anynul,
                          &status);
            FITSIO_CHECK_ERROR(status, errmode, "fits_read_img bitpix=%d", bitpix);
        }
    }
    ...
    if (bitpix == 16)
    {
        ...
        {
            int status = 0;
            fits_read_img(fptr, 20, fpixel, nelements, &nulval, data.image[ID].array.UI16, &anynul, &status);
            FITSIO_CHECK_ERROR(status, errmode, "fits_read_img bitpix=%d", bitpix);
        }
    }
    ...
    if (bitpix == 32)
    {
        create_image_ID(ID_name, naxis, naxes, _DATATYPE_INT32, data.SHARED_DFT, data.NBKEYWORD_DFT, 0, &ID);
        ...
        {
            int status = 0;
            fits_read_img(fptr, data_type_code(bitpix), fpixel, nelements, &nulval, larray, &anynul, &status);
            FITSIO_CHECK_ERROR(status, errmode, "fits_read_img bitpix=%d", bitpix);
        }
    ...
    if (bitpix == 64)
    {
        create_image_ID(ID_name, naxis, naxes, _DATATYPE_INT64, data.SHARED_DFT, data.NBKEYWORD_DFT, 0, &ID);
        larray = (long *)malloc(sizeof(long) * nelements);
        if (larray == NULL)
        {
            PRINT_ERROR("malloc error");
    ...
    if (bitpix == 8)
    {
        ...
        {
            int status = 0;
            fits_read_img(fptr, data_type_code(bitpix), fpixel, nelements, &nulval, barray, &anynul, &status);
            FITSIO_CHECK_ERROR(status, errmode, "fits_read_img bitpix=%d", bitpix);
        }
