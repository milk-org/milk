...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".in_name", "input image", "im1",
                               ...
                               // Note: an alternative way to specify an output image is
                               // FARG_OUTIM2D(outim)
                               { CLIARG_UINT32, ".cntindex", "counter index", "5",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &cntindex, &fpi_cntindex },
                               { CLIARG_UINT32, ".cntindexmax", "counter index max value", "100",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &cntindexmax, &fpi_cntindexmax },
                               { CLIARG_ONOFF, ".option.ex0mode", "toggle0", "0",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &ex0mode, &fpi_ex0mode },
                               { CLIARG_ONOFF, ".option.ex1mode", "toggle1 conditional on toggle0",
                                 "0", CLIARG_HIDDEN_DEFAULT, (void **) &ex1mode, &fpi_ex1mode } };
...
static errno_t customCONFcheck()
{
    if (data.fpsptr != NULL)
    {
        ...
        if (data.fpsptr->parray[fpi_ex0mode].fpflag & FPFLAG_ONOFF) // if ex0mode is in ON state
        {
            ...
            // FPFLAG_WRITECONF : Allow parameter to be written/changed while conf
            // process is running FPFLAG_RUNCONF   : Allow parameter to be
            // written/changed while run process is running
        }
        else // OFF state
        {
            data.fpsptr->parray[fpi_ex1mode].fpflag &= ~FPFLAG_USED;
...
static errno_t streamprocess(IMGID *inimg, IMGID *outimg)
{
    ...
    // Delaying creation of the image until here is necessary if the image size or
    // type needs to be determined within this function.
    //
    // We have called copyIMGID in compute_function(), so outimg metadata is
    // already filled up. Otherwise, we would edit these lines: outimg->naxis = 2;
    // outimg->size[0] = xsize;
    // outimg->size[1] = ysize;
...
static errno_t compute_function()
{
    ...
    // If we are sure we want outimg to be the same format (size, type etc) as
    // inimg, we can use:
