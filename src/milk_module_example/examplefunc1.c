...
// Within this translation unit, these point to the variables values shared
// between the FPS and the processes

static char *inimname;

// Make sure the type here matches the type flag in CLICMDARGDEF
static double *scoeff; // matches CLIARG_FLOAT64
// Include the following line, and add reference to pointer in CLICMDARGDEF
// entry to allow for dynamic access to parameter static long    fpi_scoeff =
// -1;

// Possible types are:
// static float *      for CLIARG_FLOAT32
// static double *     for CLIARG_FLOAT64
// static chat *       for CLIARG_IMG, CLIARG_FILENAME, CLIARG_FITSFILENAME,
// CLIARG_FPSNAME, CLIARG_STREAM, CLIARG_STR, CLIARG_STR_NOT_IMG static uint32_t
// *   for CLIARG_UINT32 static uint64_t *   for CLIARG_UINT64 static int32_t *
// for CLIARG_INT32 static int64_t *    for CLIARG_INT64, CLIARG_ONOFF,
// CLIARG_LONG
...
static CLICMDARGDEF farg[] = {
    { CLIARG_IMG, // type of argument
      ...
      CLIARG_VISIBLE_DEFAULT, // This will be exposed as a function argument in
                              // the milk CLI, which has to be entered
      (void **) &inimname, NULL },
...
static CLICMDDATA CLIcmddata = {
    ...
    "compute total of image example1", // brief (1-line) description of what the
                                       // function does
...
{
    // The preferred way to have images and streams as function args is to pass a
    // pointer to IMGID struct. Here, the function needs to change the IMGID
    // content (call to resolveIMGID). Accessing images through IMGID is faster
    // than through their names, as there is no need to resolve the image (find
    // out which ID corresponds to its name) each time the function is called.

    // Entering function, updating trace accordingly
    DEBUG_TRACE_FSTART();

    // Resolve image if not already resolved.
    // This is a low-overhead function if the image is already in memory and
    // imgptr already pointing to it. If not already connected, the function will
    // use imgptr->name to try to connect to it.
...
static errno_t compute_function()
{
    ...
    // The function mkIMGID_from_name takes the name as argument and forms an
    // IMGID from it. At this point the connection to the image has not been
    // established. This will be done on the first call of resolveIMGID inside the
    // compute function.
