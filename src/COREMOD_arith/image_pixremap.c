...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".insname", "input image name", "inim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &insname, NULL },
                               { CLIARG_IMG, ".map", "mapping image name", "mapim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &mapsname, NULL },
                               FARG_OUTIM_NAME(outim),
                               FARG_OUTIM_SHARED(outim) };

static CLICMDDATA CLIcmddata = { "pixremap", "pixel remapping of image", CLICMD_FIELDS_DEFAULTS };
...
static errno_t compute_function()
{
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_LOOPSTART
    {
        ...
        case _DATATYPE_INT8:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.SI8[map_outpixindex[pixi]] =
                    imgin.im->array.SI8[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_UINT8:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.UI8[map_outpixindex[pixi]] =
                    imgin.im->array.UI8[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_INT16:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.SI16[map_outpixindex[pixi]] =
                    imgin.im->array.SI16[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_UINT16:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.UI16[map_outpixindex[pixi]] =
                    imgin.im->array.UI16[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_INT32:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.SI32[map_outpixindex[pixi]] =
                    imgin.im->array.SI32[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_UINT32:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.UI32[map_outpixindex[pixi]] =
                    imgin.im->array.UI32[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_INT64:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.SI64[map_outpixindex[pixi]] =
                    imgin.im->array.SI64[map_inpixindex[pixi]];
            }
            break;
        ...
        case _DATATYPE_UINT64:
            for (uint64_t pixi = 0; pixi < nbpix; pixi++)
            {
                imgout.im->array.UI64[map_outpixindex[pixi]] =
                    imgin.im->array.UI64[map_inpixindex[pixi]];
            }
            break;
        }
