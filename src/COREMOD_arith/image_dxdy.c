...
imageID arith_image_dx_IMGID(IMGID *imgin, IMGID *imgout)
{
    ...
    for (uint32_t jj = 0; jj < ysize; jj++)
    {
        ...
        {
            imgout->im->array.F[jj * xsize + ii] = (imgin->im->array.F[jj * xsize + ii + 1] -
        ...
        imgout->im->array.F[jj * xsize + xsize - 1] =
            imgin->im->array.F[jj * xsize + xsize - 1] - imgin->im->array.F[jj * xsize + xsize - 2];
    }
...
imageID arith_image_dy_IMGID(IMGID *imgin, IMGID *imgout)
{
    ...
    for (uint32_t ii = 0; ii < xsize; ii++)
    {
        ...
        {
            imgout->im->array.F[jj * xsize + ii] = (imgin->im->array.F[(jj + 1) * xsize + ii] -
