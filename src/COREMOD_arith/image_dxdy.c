...
imageID arith_image_dx(const char *ID_name, const char *IDout_name)
{
    ...
    create_image_ID(IDout_name, naxis, naxes, datatype, data.SHARED_DFT, data.NBKEYWORD_DFT, 0, &IDout);
...
imageID arith_image_dy(const char *ID_name, const char *IDout_name)
{
    ...
    for (uint32_t ii = 0; ii < naxes[0]; ii++)
    {
        ...
        {
            data.image[IDout].array.F[jj * naxes[0] + ii] =
                (data.image[ID].array.F[(jj + 1) * naxes[0] + ii] - data.image[ID].array.F[(jj - 1) * naxes[0] + ii]) /
                2.0;
        }
