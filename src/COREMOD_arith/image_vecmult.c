...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".iminname", "input image name", "inim",
                               ...
                               { CLIARG_IMG, ".vecname", "input vector name", "vec",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &vecname, &fpi_vecname },
                               { CLIARG_STR, ".imoutname", "output image name", "outim",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &imoutname, &fpi_imoutname },
                               { CLIARG_UINT32, ".axis", "multiplication axis", "0",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &multaxis, &fpi_multaxis } };
...
static errno_t customCONFsetup()
{
    if (data.fpsptr != NULL)
    {
        data.fpsptr->parray[fpi_iminname].fpflag |= FPFLAG_STREAM_RUN_REQUIRED | FPFLAG_CHECKSTREAM;
    }
...
static CLICMDDATA CLIcmddata = { "imvecmult", "multiply image by vector", CLICMD_FIELDS_DEFAULTS };
