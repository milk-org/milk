            ...
            if (datatype == _DATATYPE_FLOAT)
                        ...
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            {
                                data.image[IDout]
                                    .array
                                    .CF[(jj - start[1]) * naxesout[0] +
                                        (ii - start[0])]
                                    .re = data.image[IDin]
                                              .array.CF[jj * naxes[0] + ii]
                                              .re;
                                data.image[IDout]
                                    .array
                                    .CF[(jj - start[1]) * naxesout[0] +
                        ...
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            {
                                data.image[IDout]
                                    .array
                                    .CD[(jj - start[1]) * naxesout[0] +
                                        (ii - start[0])]
                                    .re = data.image[IDin]
                                              .array.CD[jj * naxes[0] + ii]
                                              .re;
                                data.image[IDout]
                                    .array
                                    .CD[(jj - start[1]) * naxesout[0] +
            ...
            else if (datatype == _DATATYPE_UINT8)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.UI8[(jj - start[1]) * naxesout[0] +
                                           (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_UINT16)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.UI16[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_UINT32)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.UI32[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_UINT64)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.UI64[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_INT8)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.SI8[(jj - start[1]) * naxesout[0] +
                                           (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_INT16)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.SI16[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_INT32)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.SI32[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
            ...
            else if (datatype == _DATATYPE_INT64)
                {
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            data.image[IDout]
                                .array.SI64[(jj - start[1]) * naxesout[0] +
                                            (ii - start[0])] =
                    ...
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array.F[(kk - start[2]) * naxesout[0] *
                                                     naxesout[1] +
                                                 (jj - start[1]) * naxesout[0] +
                                                 (ii - start[0])] =
                                        data.image[IDin]
                    ...
                    for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                        for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array.D[(kk - start[2]) * naxesout[0] *
                                                     naxesout[1] +
                                                 (jj - start[1]) * naxesout[0] +
                                                 (ii - start[0])] =
                                        data.image[IDin]
                                            .array.D[kk * naxes[0] * naxes[1] +
                                                     jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_COMPLEX_FLOAT)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .UI8[(kk - start[2]) * naxesout[0] *
                                                 naxesout[1] +
                                             (jj - start[1]) * naxesout[0] +
                                             (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .UI8[kk * naxes[0] * naxes[1] +
                                                 jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_UINT16)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .UI16[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .UI16[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_UINT32)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .UI32[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .UI32[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_UINT64)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .UI64[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .UI64[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_INT8)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .SI8[(kk - start[2]) * naxesout[0] *
                                                 naxesout[1] +
                                             (jj - start[1]) * naxesout[0] +
                                             (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .SI8[kk * naxes[0] * naxes[1] +
                                                 jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_INT16)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .SI16[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .SI16[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_INT32)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .SI32[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .SI32[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else if (datatype == _DATATYPE_INT64)
                            ...
                            for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                                {
                                    data.image[IDout]
                                        .array
                                        .SI64[(kk - start[2]) * naxesout[0] *
                                                  naxesout[1] +
                                              (jj - start[1]) * naxesout[0] +
                                              (ii - start[0])] =
                                        data.image[IDin]
                                            .array
                                            .SI64[kk * naxes[0] * naxes[1] +
                                                  jj * naxes[0] + ii];
                                }
                }
            else
