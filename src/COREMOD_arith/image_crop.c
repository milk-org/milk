...
imageID arith_image_extract3D(const char *in_name, const char *out_name, long size_x, long size_y, long size_z,
                              long xstart, long ystart, long zstart);
...
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) + CLI_checkarg(5, CLIARG_LONG) + CLI_checkarg(6, CLIARG_LONG) ==
        0)
    {
        arith_image_extract2D(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
                              data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.numl,
...
static errno_t arith_image_extract3D__cli()
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) + CLI_checkarg(3, CLIARG_LONG) +
        ...
        0)
    {
        arith_image_extract3D(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
                              data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.numl,
                              data.cmdargtoken[6].val.numl, data.cmdargtoken[7].val.numl, data.cmdargtoken[8].val.numl);
...
errno_t image_crop_addCLIcmd()
{
    ...
    RegisterCLIcommand("extractim", __FILE__, arith_image_extract2D__cli, "crop 2D image",
                       "<input image> <output image> <sizex> <sizey> <xstart> <ystart>",
                       "extractim im ime 256 256 100 100",
                       "int arith_image_extract2D(const char *in_name, const char *out_name, long size_x, long size_y, "
                       "long xstart, long ystart)");

    RegisterCLIcommand("extract3Dim", __FILE__, arith_image_extract3D__cli, "crop 3D image",
                       "<input image> <output image> <sizex> <sizey> <sizez> <xstart> <ystart> <zstart>",
                       "extractim im ime 256 256 5 100 100 0",
                       "int arith_image_extract3D(const char *in_name, const char *out_name, long size_x, long size_y, "
...
imageID arith_image_crop(const char *ID_name, const char *ID_out, long *start, long *end, long cropdim)
{
    ...
    create_image_ID(ID_out, naxis, naxesout, datatype, data.SHARED_DFT, data.NBKEYWORD_DFT, 0, &IDout);
            ...
            for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                    for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                    {
                        data.image[IDout]
                            .array
                            .CF[(kk - start[2]) * naxesout[0] * naxesout[1] + (jj - start[1]) * naxesout[0] +
                                (ii - start[0])]
                            .re = data.image[IDin].array.CF[kk * naxes[0] * naxes[1] + jj * naxes[0] + ii].re;
                        data.image[IDout]
                            .array
                            .CF[(kk - start[2]) * naxesout[0] * naxesout[1] + (jj - start[1]) * naxesout[0] +
                                (ii - start[0])]
                            .im = data.image[IDin].array.CF[kk * naxes[0] * naxes[1] + jj * naxes[0] + ii].im;
                    }
        }
        else if (datatype == _DATATYPE_COMPLEX_DOUBLE)
        {
            for (uint32_t ii = start_c[0]; ii < end_c[0]; ii++)
                for (uint32_t jj = start_c[1]; jj < end_c[1]; jj++)
                    for (uint32_t kk = start_c[2]; kk < end_c[2]; kk++)
                    {
                        data.image[IDout]
                            .array
                            .CD[(kk - start[2]) * naxesout[0] * naxesout[1] + (jj - start[1]) * naxesout[0] +
                                (ii - start[0])]
                            .re = data.image[IDin].array.CD[kk * naxes[0] * naxes[1] + jj * naxes[0] + ii].re;
                        data.image[IDout]
                            .array
                            .CD[(kk - start[2]) * naxesout[0] * naxesout[1] + (jj - start[1]) * naxesout[0] +
                                (ii - start[0])]
                            .im = data.image[IDin].array.CD[kk * naxes[0] * naxes[1] + jj * naxes[0] + ii].im;
                    }
        }
        else if (datatype == _DATATYPE_UINT8)
        {
                              ...
                              long xstart, long ystart, long zstart)
{
    ...
    {
        PRINT_ERROR("malloc() error");
        printf("params: %s %s %ld %ld %ld %ld %ld %ld \n", in_name, out_name, size_x, size_y, size_z, xstart, ystart,
               zstart);
        exit(0);
    }
