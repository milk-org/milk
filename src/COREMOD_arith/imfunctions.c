...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.UI8[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.UI16[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.UI32[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.UI64[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.SI8[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.SI16[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.SI32[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.SI64[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = (float) pt2function((double) (imgin->im->array.F[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (uint_fast64_t ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.D[ii] = pt2function(imgin->im->array.D[ii]);
            }
        }
#ifdef _OPENMP
    }
...
errno_t arith_image_function_im_im__d_d(const char *__restrict ID_name,
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI8[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_UINT16)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI16[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_UINT32)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI32[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_UINT64)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI64[ii]), v0);
            }
        }
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI8[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_INT16)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI16[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_INT32)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI32[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_INT64)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI64[ii]), v0);
            }
        }
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.F[ii]), v0);
            }
        }
        if (datatype == _DATATYPE_DOUBLE)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.D[ii] = pt2function(imgin->im->array.D[ii], v0);
            }
        }
#ifdef _OPENMP
    }
...
errno_t arith_image_function_imd_im__dd_d(const char *__restrict ID_name,
                                          double v0,
                                          const char *__restrict ID_out,
                                          double (*pt2function)(double, double))
{
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI8[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_UINT16)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI16[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_UINT32)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI32[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_UINT64)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.UI64[ii]), v0, v1);
            }
        }
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI8[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_INT16)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI16[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_INT32)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI32[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_INT64)
        {
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.SI64[ii]), v0, v1);
            }
        }
...
#    pragma omp for
            ...
            {
                imgout->im->array.F[ii] =
                    (float) pt2function((double) (imgin->im->array.F[ii]), v0, v1);
            }
        }
        if (datatype == _DATATYPE_DOUBLE)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.D[ii] = pt2function(imgin->im->array.D[ii], v0, v1);
            }
        }
#ifdef _OPENMP
    }
...
errno_t arith_image_function_1_1_IMGID(IMGID *imgin, IMGID *imgout, double (*pt2function)(double))
{
    long ii;
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.UI8[ii]));
            }
        }
        if (datatype == _DATATYPE_UINT16)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.UI16[ii]));
            }
        }
        if (datatype == _DATATYPE_UINT32)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.UI32[ii]));
            }
        }
        if (datatype == _DATATYPE_UINT64)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.UI64[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.SI8[ii]));
            }
        }
        if (datatype == _DATATYPE_INT16)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.SI16[ii]));
            }
        }
        if (datatype == _DATATYPE_INT32)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.SI32[ii]));
            }
        }
        if (datatype == _DATATYPE_INT64)
        {
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.SI64[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.F[ii] = pt2function((double) (imgin->im->array.F[ii]));
            }
        }
...
#    pragma omp for
            ...
            for (ii = 0; ii < nelement; ii++)
            {
                imgout->im->array.D[ii] = (double) pt2function((double) (imgin->im->array.D[ii]));
            }
        }
#ifdef _OPENMP
    }
...
errno_t arith_image_function_2_1_IMGID(IMGID *inimg1,
                                       IMGID *inimg2,
                                       IMGID *outimg,
                                       double (*pt2function)(double, double))
{
...
#    pragma omp parallel if (nelement > OMP_NELEMENT_LIMIT)
        {
#endif
            if (datatype == _DATATYPE_FLOAT && datatype1 == _DATATYPE_FLOAT &&
                datatype2 == _DATATYPE_FLOAT)
            {
...
#    pragma omp for
    ...
    else
    {
        ...
        {
            ptr1array[ii] = get_pixel_double(inimg1->im, ii);
        }
    }
    ...
    else
    {
        ...
        {
            ptr2array[ii] = get_pixel_double(inimg2->im, ii);
        }
    }
    ...
    if (outimg->datatype == _DATATYPE_FLOAT)
    {
        for (uint64_t ii = 0; ii < nbpix; ii++)
        {
            outimg->im->array.F[ii] =
                (float) pt2function(ptr1array[inpix1[ii]], ptr2array[inpix2[ii]]);
        }
    }
...
errno_t arith_img_function_2_1(IMGID  inimg1,
                               IMGID  inimg2,
                               IMGID *outimg,
                               double (*pt2function)(double, double))
{
                                       ...
                                       complex_float (*pt2function)(complex_float, complex_float))
{
    ...
    create_image_ID(ID_out, naxis, naxes, datatype1, data.SHARED_DFT, NB_KEYWNODE_MAX, 0, &IDout);
    free(naxes);
...
#    pragma omp for
        ...
        for (ii = 0; ii < nelement; ii++)
        {
            data.image[IDout].array.CF[ii] =
                pt2function(data.image[ID1].array.CF[ii], data.image[ID2].array.CF[ii]);
        }
#ifdef _OPENMP
    }
#endif
...
#    pragma omp for
        ...
        for (ii = 0; ii < nelement; ii++)
        {
            data.image[IDout].array.CD[ii] =
                pt2function(data.image[ID1].array.CD[ii], data.image[ID2].array.CD[ii]);
        }
#ifdef _OPENMP
    }
#endif
                                    ...
                                    double (*pt2function)(double, double))
{
    ...
    imgout->datatype =
        (imgin->md->datatype == _DATATYPE_DOUBLE) ? _DATATYPE_DOUBLE : _DATATYPE_FLOAT;
    imgout->shared = data.SHARED_DFT;
    imgout->NBkw   = NB_KEYWNODE_MAX;
    imcreateIMGID(imgout);
    uint_fast64_t nelement = imgin->md->nelement;
...
#    pragma omp parallel for if (nelement > OMP_NELEMENT_LIMIT)
        ...
        for (ii = 0; ii < nelement; ii++)
        {
            ...
            if (imgout->datatype == _DATATYPE_FLOAT)
            {
...
#    pragma omp parallel for if (nelement > OMP_NELEMENT_LIMIT)
    ...
    for (ii = 0; ii < nelement; ii++)
    {
        ...
        {
            data.image[ID].array.F[ii] = (float) pt2function(v, f1);
        }
        else if (data.image[ID].md[0].datatype == _DATATYPE_DOUBLE)
        {
            data.image[ID].array.D[ii] = pt2function(v, f1);
        }
    }
    return EXIT_SUCCESS;
}
...
#    pragma omp parallel for if (nelement > OMP_NELEMENT_LIMIT)
    ...
    for (ii = 0; ii < nelement; ii++)
    {
        ...
        if (data.image[ID].md[0].datatype == _DATATYPE_FLOAT)
        {
            data.image[ID].array.F[ii] = (float) pt2function(v, f1, f2);
        }
        else if (data.image[ID].md[0].datatype == _DATATYPE_DOUBLE)
        {
            data.image[ID].array.D[ii] = pt2function(v, f1, f2);
        }
    }
    return (0);
}
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            _Pragma("omp parallel for if (nelement > OMP_NELEMENT_LIMIT)") for (uint64_t i = 0;    \
                                                                                i < nelement; i++) \
                po[i] = (float) funcname((double) p1[i]);                                          \
        }                                                                                          \
        else if (imgin->md->datatype == _DATATYPE_DOUBLE && imgout->datatype == _DATATYPE_DOUBLE)  \
        {                                                                                          \
