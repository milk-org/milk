...
static CLICMDARGDEF farg[] = { { CLIARG_IMG, ".in0name", "input image 0", "im0",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &inimname, NULL },
                               { CLIARG_IMG, ".maskim", "input image mask", "imm",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &maskimname, NULL },
                               { CLIARG_STR, ".outname", "output image", "im0n",
                                 CLIARG_VISIBLE_DEFAULT, (void **) &outimname, NULL },
                               { CLIARG_UINT32, ".axis", "norm axis", "0", CLIARG_VISIBLE_DEFAULT,
                                 (void **) &sliceaxis, &fpi_sliceaxis },
                               { CLIARG_IMG, ".auxin", "auxillary input image, in-place update",
                                 "auxin", CLIARG_HIDDEN_DEFAULT, (void **) &auxin, NULL },
                               { CLIARG_ONOFF, ".RMS", "output RMS=1 over mask", "OFF",
                                 CLIARG_HIDDEN_DEFAULT, (void **) &modeRMS, NULL } };

static CLICMDDATA CLIcmddata = { "normalizeslice", "image normalize over mask by slice",
...
errno_t image_slicenormalize(IMGID   inimg,
                             IMGID   maskimg,
                             IMGID  *outimg,
                             uint8_t sliceaxis,
                             IMGID   imgaux,
            ...
            for (uint32_t kk = 0; kk < sizescan[2]; kk++)
            {
                ...
                case _DATATYPE_UINT8:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.UI8[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_INT8:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.SI8[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_UINT16:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.UI16[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_INT16:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.SI16[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_UINT32:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.UI32[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_INT32:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.SI32[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_UINT64:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.UI64[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_INT64:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.SI64[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_FLOAT:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.F[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                case _DATATYPE_DOUBLE:
                    outimg->im->array.F[pixi] =
                        (1.0 * inimg.im->array.D[pixi]) / normarray[pixcoord[sliceaxis]];
                    break;
                }
            }
        }
        ...
        for (uint32_t ii = 0; ii < auxsizescan[0]; ii++)
        {
            ...
            for (uint32_t jj = 0; jj < auxsizescan[1]; jj++)
            {
                ...
                for (uint32_t kk = 0; kk < auxsizescan[2]; kk++)
                {
                    ...
                    imgaux.im->array.F[pixi] =
                        imgaux.im->array.F[pixi] / normarray[pixcoord[sliceaxis]];
                }
            }
        }
    }
...
static errno_t compute_function()
{
    ...
    {
        image_slicenormalize(inimg, maskimg, &outimg, *sliceaxis, imgaux, *modeRMS);
