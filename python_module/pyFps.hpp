...
class pyFps
{
   ...
   * @param name : the name of the shared memory file to connect
   */
    int connect() { return function_parameter_struct_connect(name_.c_str(), &fps_, FPSCONNECT_SIMPLE); };
   ...
   */
    int add_entry(std::string entry_name, std::string entry_desc, uint32_t fptype)
    {
        ...
        return function_parameter_add_entry(&fps_, entry_name.c_str(), entry_desc.c_str(), fptype, FPFLAG_DEFAULT_INPUT,
                                            nullptr);
    }
